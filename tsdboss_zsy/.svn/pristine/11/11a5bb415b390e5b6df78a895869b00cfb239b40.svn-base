...
<head>
	...
	<script type="text/javascript">
        ...
        function savedb(busitype){  
        	...
        	var pubParam = getPubProcParam(busitype); 
         	var param=pubParam+"&internetacct="+v_acct+"&paytype="+v_paytype+"&busifee="+v_busifee+"&workmemo="+v_workmemo+"&busiywarea="+tsd.encodeURIComponent($("#ywarea").val());
        	...
        	if (res[0][0]=="already"){
        		alert("该用户已经是\""+sinfo1+"\"状态，不能再办理"+sinfo+"业务！");
        		return;
        	}       	      
        	if(res[0][0]=="SUCCEED"){
        		if (parseFloat($("#txtBussFee").val()) > 0){
        			$("#jobid").val(res[0][1]);
        			setRepFileField();
        			printworkorder(busitype);//业务类型，取自于tsd_ini(tsection='radbusi_busifee' and tident='pause(或restore)' )        			
        		}
        		else{	
        			alert(sinfo+"成功！\r\n\n点击“确定”按钮办理下一笔业务。");
        		}
        		...
        		if(res[0][1]!=""){
        			...
        			alert(sinfo+"失败！请与系统管理员联系。");
        		}	
        	}     	
        }	
        function getPubProcParam(busitype){        	
...
<body onunload="unLock()">	
	...
	<DIV class="ui-layout-south">		
		<div style="margin-top:-5px; margin-bottom:-7px">
			<button class="tsdbutton" id="btnPause" onclick="save('pause');" style="width:70px;line-height:25px; margin-top: 1px; padding: 0px;" > 
				确认报停 
			</button>	
			<button class="tsdbutton" id="btnRestore" onclick="save('restore');" style="width:70px;line-height:25px; margin-top: 1px; padding: 0px;" > 
				确认复通 
			</button>			
