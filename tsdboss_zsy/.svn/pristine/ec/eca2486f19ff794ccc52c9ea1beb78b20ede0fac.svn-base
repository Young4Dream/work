<%----------------------------------------
	...
	modify: 
		 2009-11-26:  添加功能注释.
	      2009-12-02:  添加日志.
	      2009-12-04：  修改模块功能
	      2009-12-09：  修改号线功能，根据当前用户所在的部门显示电话和宽带信息
	      2009-12-10:  用户修改地址时，对小区号选择做一下限制，小区号为当前登陆用户所在的区域,对该模块添加国际化信息
	      2010-08-04：  修改用户显示信息，对用户选择设备时进行模块局区分
	      2010-08-05:  添加可编辑路由设置权限，editroute getLineFields()
	      2010-08-20:  添加号线日志信息
		  2010-11-5    youhongyu 添加注释功能	
		  2010-11-24   chenze  修改getTheValue方法，用于处理回调时处理路由段设置
		  2010-11-29   chenze  修改方法thisDataDownload，增加一个url参数exportflag=0，解决无法导出问题
		  2010-12-22   chenze  添加设置端子时同时更新端子状态
		  2010-12-25   youhongyu  导出提示修改
		  2011-6-9    youhongyu  号线设计修改
---------------------------------------------%>
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script src="script/telephone/linemanagement/userLineManager.js" type="text/javascript"></script>
		<script src="script/telephone/linemanagement/ZWUserLineManager.js" type="text/javascript"></script>	
...
function thisReady()
{
    var urlstr = tsd.buildParams(
    {
        ...
        tsdoper : 'query', //操作类型 
        datatype : 'xml', //返回数据格式 
        tsdpk : 'userLineManager.query', //执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
        tsdpkpagesql : 'userLineManager.querypage'//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
    });
    ...
    jQuery("#editgrid").jqGrid(
    {
        ...
        colNames : [Dh, Jhj, UserBM,bmzc, userAddr, Regdate, MoKuaiJu,'跨模块局','userID'], 
        colModel : [ 
        //{
        ...
        onSelectRow: function (ids){
        	if (ids != null) 
            {
                ...
                var line_userid =$("#editgrid").getCell(ids, "userid");
...
function queryDhInfos()
{
    ...
    if (dh != '')
    {
        ...
        $.ajax(
        {
            url : 'mainServlet.html?dh=' + dh+"&line_userid="+line_userid + urlstr, datatype : 'xml', cache : false, //如果值变化可能性比较大 一定要将缓存设成false
            timeout : 1000, async : false , //同步方式
            success : function (xml)
            {
                $(xml).find('row').each(function ()
                {
                    ...
                    if (UserName != undefined)
                    {
                        ...
                        var MoKuaiJu3 = $(this).attr("mokuaiju3");
                        var MoKuaiJu4 = $(this).attr("mokuaiju4");
                        ...
                        $('#dis_mokuaiju1').html(MoKuaiJu);
                        $('#dis_mokuaiju2').html(MoKuaiJu2);
                        $('#dis_mokuaiju3').html(MoKuaiJu3);
                        $('#dis_mokuaiju4').html(MoKuaiJu4);
...
function isThisHidden(str)
{
    ...
    document.getElementById('pagered_broad').style.display = str;
}
...
function queryRouteInfo()
{
    ...
    //{
        ...
        for (var i = 0 ; i < thiscolum.length; i++)
        {
            ...
            theinput += "<p style='margin-left:100px'><span class='tmpspanstyle'>" + thiscolum[i] + "</span><input class='n_value' type='text' id='" + thevalue + "' /><input  class='par_noo' id='" + thevalue + "_h' /><input class='src_value'  id='" + thevalue + "_y' /><button style='margin-left:10px' class='btn_2k3' "+flags+" id='" + thevalue + "b' onclick=getDeviceType('" + thevalue + "')><fmt:message key='userline.changedevice'/></button><button class='btn_2k3' style='margin-left:20px' "+flags+" id='" + thevalue + "c' onclick=clearTheValue('" + thevalue + "')><fmt:message key='userline.cleardata'/></button></p><hr/>" ;
        }
        ...
        success : function (msg)
        {
            if (msg == "true")
            {
                ...
                if (theflag == 1)
                {
                    ...
                    var line_userid = $("#line_userid").val();
                    reloadRouteData('userLineManager.querylineinfo', 'userLineManager.querylineinfopage', 
                    'editgrid_info', line_userid, column) 
                }
                else if (theflag == 2)
                {
                	/*
...
function fuheExe()
{
    ...
    if (queryName == "query")
    {
        fuheQueryUserLine('userLineManager.fuhequerybywhere', 'userLineManager.fuhequerybywherepage', thismkj, thiscolumn);
    }
}
...
function thisDataDownload(ds,tsdcf,cols,table,flagsql,table2) {
						...
						if(params=='&fusearchsql='){
							params +='1=1';
							tsdpkstr = 'userLineManager.export';
						}else{
							tsdpkstr = 'userLineManager.exportbywhere';
						}
...
.spanstyle{display:-moz-inline-box; display:inline-block; width:115px}
	...
	<body>
		...
		<div id="broaddiv" style="display: none;">
			<table id="editgrid_broad" class="scroll" cellpadding="0"
				cellspacing="0"></table>
			<div id="pagered_broad" class="scroll" style="text-align: left;"></div>
		</div>
		...
		<div class="neirong" id="addRountform" style="display: none; width: 800px">
			...
			<div class="midd">
				<form action="#" name="addRountforms" id="addRountforms"
					...
					<table width="100%" id="tdiv" border="0" cellspacing="0"
						...
						<tr>
							<td>
								<div style="padding-left: 20px;height: 0px;" >
									<label>路由类型：</label>
						...
						<tr>
							<td>								
								<div style="padding-left: 20px;height:30px;" >	
									<img style="margin-left:60px;_margin-left:30px;" src="style/images/public/tubiao_3.jpg" />
									<a href="#" onclick="switchZWPan();" id="changeToZW">切换到专网号线</a>							
								</div>
							</td>
						</tr>
						-->
						<tr>
							<td>
								<div id='addRountinput'
									style="height: 280px;overflow-y: auto; overflow-x: hidden;"></div>
							...
							<td>
								<div>
									<p style='margin-left:50px'>
										<span class='tmpspanstyle'>备注：</span>
										<textarea id="addRouteBz" rows="3" cols="60"></textarea>
									</p>
								</div>								
							</td>
						</tr>
			...
			<div class="midd_butt">
				<button type="submit" class="tsdbutton" id="modifyRount"
					style="margin-right: 10%" onclick="saveRouteInfo('modify',0);">
					<fmt:message key="global.edit" />
				</button>
				
				<button type="submit" class="tsdbutton" id="saveRount"
					style="margin-right: 10%" onclick="saveRouteInfo('add',0);">
					<fmt:message key="global.add" />
				</button>
				<button type="submit" class="tsdbutton" id="ParallelRount"
					style="margin-right: 10%" onclick="saveRouteInfo('Parallel',0);">
					并机
				</button>
		...
		<!-- 专网路由设置面板  start -->
		<div class="neirong" id="ZWaddRountform" style="display: none; width: 800px">
			<div class="top">
				<div class="top_01" id="thisdrag">
					<div class="top_01left">
						<div class="top_02">
					...
					<div class="top_01right">
						<a href="#" onclick="javascript:$('#ZWaddRountclose').click();">关闭</a>
					</div>
				</div>
				<div class="top02right">
					<img src="style/images/public/toptiaoright.gif" />
			...
			<div class="midd">
				<form action="#" name="ZWaddRountforms" id="ZWaddRountforms"
					onsubmit="return false;">
					<table width="100%" id="tdiv" border="0" cellspacing="0"
						cellpadding="0">
						<tr>
							<td>
								<div style="padding-left: 20px;height:30px;" >	
									...
									<!-- 
										<img style="margin-left:60px;_margin-left:30px;" src="style/images/public/tubiao_3.jpg" />
										<a href="#" onclick="switchPTPan();" id="changeToPT">切换到普通号线面板</a>
									 -->
										<span style="margin-left: 10px;margin-right: 2px;">路由序号:</span>
										<select id="ZWOrder" style="float: none;width: 80px;margin-left: 0px;" onchange="setMkj();">
										</select>
										<span style="margin-left: 10px;margin-right: 2px;">路由模块局:</span>
										<input id="ZWmokuaiju"  style="float: none;width: 110px;margin-left: 0px;" disabled="disabled"/>
										<span style="margin-left: 10px;margin-right: 2px;">号线设备类型:</span>
										<select id="ZWtype" style="float: none;width: 80px;margin-left: 0px;" >
											<option value="0">普通设备</option>
											<option value="1">RC设备</option>
										</select>
									</p>													
								</div>
								
								<div style="padding-left: 20px;height:25px;" >	
									<p>
										<a href="#" style="margin-left: 150px;" onclick="ZWprintRountPan(1,'add');">添加一个设备</a>
									</p>													
								</div>
							</td>
						</tr>
						<tr>
							<td>
								<div id='ZWaddRountinput'
									style="height: 250px;overflow-y: auto; overflow-x: hidden;"></div>
								<input type="hidden" id="ZWlineid"/>
								<input type="hidden" id="ZWRmemo"/>
								<input type="hidden" id="ZWRrounttype"/>
								<input type="hidden" id="ZWRdh"/>
							</td>
						</tr>
						<tr>
							<td>
								<div>
									<p style='margin-left:50px'>
										<span class='tmpspanstyle'>备注：</span>
										<textarea id="ZWAddRouteBz" rows="3" cols="60"></textarea>
									</p>
								</div>								
							</td>
						</tr>
			...
			<div class="midd_butt">
				<button type="submit" class="tsdbutton" id="ZWmodifyRount"
					style="margin-right: 10%" onclick="saveRouteInfo('modify',1);">
					<fmt:message key="global.edit" />
				</button>
				
				<button type="submit" class="tsdbutton" id="ZWsaveRount"
					style="margin-right: 10%" onclick="saveRouteInfo('add',1);">
					<fmt:message key="global.add" />
				</button>
				<button type="submit" class="tsdbutton" id="ZWParallelRount"
					style="margin-right: 10%" onclick="saveRouteInfo('Parallel',1);">
					并机
				</button>		
				<button type="button" class="tsdbutton" id="ZWaddRountclose"
					onclick="isThisHidden('block');clearText('ZWaddRountforms');">
					<fmt:message key="global.close" />
				</button>
			</div>
		</div>
		...
		<!-- 被选中电话在air_users的id -->
		<input type='hidden' id='line_userid' />
		...
		<!-- 并机用户信息显示 start-->
		<div class="neirong" id="BJRountform"
			style="display: none; width: 800px">
			<div class="top">
				<div class="top_01" id="thisdrag">
					<div class="top_01left">
					...
					<div class="top_01right">
						<a href="#" onclick="javascript:$('#BJRountclose').click();">关闭</a>
					</div>
				</div>
				<div class="top02right">
					<img src="style/images/public/toptiaoright.gif" />
			...
			<div class="" style="background-color: white;">
				<table id="editgrid_BJ" class="scroll" cellpadding="0" cellspacing="0" ></table>
				<div id="pagered_BJ" class="scroll" style="text-align: left;"></div>
			</div>
			<div class="midd_butt">
				<button type="button" class="tsdbutton" id="BJRountAdd" onclick="SelTheBJ()">
					选择
				</button>
				<button type="button" class="tsdbutton" id="BJRountclose">
					<fmt:message key="global.close" />
				</button>
			</div>
		</div>
		...
		<input type="hidden" id="ziduansF_BJ"/>
		...
		<input type="hidden" id="BJ_mkjacross"/><!-- 并机用户是否 跨模块局 -->
		<input type="hidden" id="BJ_userid"/><!-- 并机用户是air_user 的userid-->
		<input type="hidden" id="BJ_lineid"/><!-- 并机时，号线信息复制并给其他地址的air_line.lineid -->
		<input type="hidden" id="BJ_EndLineid"/><!-- 并机时，更新了号线的air_line.lineid -->
		<input type="hidden" id="BJ_typeid"/><!-- 类型：1：电话 2：宽带 -->	
		<input type="hidden" id="BJ_DevNum"/><!-- 并机时，更新号线前的设备个数 -->
