...
<%-- 
	File Name:  StopKeepNumber.jsp
    Function:   宽带业务受理（申请停复机）
    Author:     wenxuming
    Date:       2009-01-01
    Description: 宽带申请停复机
    Modify:     2010-03-21  
...
<html>
	<head>
		<title><fmt:message key="gjh.tjbh" /></title>
...
<!-- 导入的js文件 -->
		...
		<style type="text/css">
			...
			cas{float:left; width:100%; 
           height:26px;background:url(style/images/public/daohangbg.jpg) repeat-x; line-height:28px;}
		</style>
		<style type="text/css">
		 .button {
		...
		function lastApplyDate(UserName)
		{
			var res = fetchMultiArrayValue("StopKeepNum.timesdds",0,"&UserName="+UserName);
			...
			else
			{
				...
				var stoptime = res[0][1];
				
				$("#cc_time_tj").val(startTime);
				$("#cc_time_fj").val(stoptime);				
			}
		}
		...
		{
			//	
			var pime = $("#PauseStopTime1").val();
			if(pime==""){
			   $("#PauseTime").empty();
			}		
			var svalue = $("#tjbhzzzz").val();
			var res = "";
			if(svalue=="1")
			{
				res = fetchSingleValue("StopKeepNum.getDate",0,"&mons="+$("#PauseStopTime1").val());
			}
			else
			{
				res = fetchSingleValue("StopKeepNum.addDate",0,"&mons="+$("#PauseStopTime1").val()+"&UserName="+$("#UserName").val());
			}
			...
			else
			{
				...
				$(function(){
                	...
                	$("#profetype").change(function(){
                		if(svalue=="0")
                		{
                			//alert($("#UserName").val());
                			lastApplyDate($("#UserName").val());
                			if($("#profetype").val()=="申请停机")
                			{
                				$("#c_c_c td:eq(1)").css("display","block");
                				$("#c_c_c td:eq(2)").css("display","block");
                				$("#d_d_d").css("display","block");
                			}
                			else
                			{  
                				$("#c_c_c td:eq(1)").css("display","block");
                				$("#c_c_c td:eq(2)").css("display","none");
                				$("#d_d_d").css("display","none");
                			}
                		}
					...
					$(document).keydown(function(event){
						...
						if(panelStatus == "block")
						{
							...
							else if(event.keyCode==13)
							{
								...
								{
									userChoose($("#bu_muser").getCell(idx,"UserName"));
								}
							}
						}
					});
               ...
               function userChoose(UserName)
	           {
                 ...
                 if(param>0){
                    alert("<fmt:message key="broadband.gyhwwgzbnblcyw"/>");//该用户未完工，暂不能办理此业务！
                    return false;
                 }	
                 ...
                 if(par==1){
		        	...
		        	//alert(checkYE(UserName));
		        	if(!checkYE(UserName))
		            {
		            	alert("帐号余额为0，不能办理此业务。");
		            }
		            else if(sfgongfei(UserName))
		            {
		            	alert("公费用户不能办理些业务");
		            }
		            else
		            {	            
					$("#c_c_c td:eq(1)").css("display","none");
					$("#c_c_c td:eq(2)").css("display","none");
					$("#d_d_d").css("display","none");
        	...
        	//判断用户是否有余额
        	function checkYE(UserName)
        	{
        		var ress = fetchSingleValue("StopKeepNum.ye",0,"&UserName="+UserName);
        	...
        	//判断该用户是否为公费用户
        	function sfgongfei(UserName)
        	{
        		var ress = fetchSingleValue("StopKeepNum.gongfei",0,"&UserName="+UserName);
        		if(ress=="1")
        		{
        			return true;
        		}
        ...
        var usertype = getusertype(UserName);
        	var urlstr=tsd.buildParams({ packgname:'service',
							...
							$.ajax({
								url:'mainServlet.html?'+urlstr+"&busiclass="+'broadband'+"&nubmer="+UserName+"&busitype="+'pause'+"&usertype="+usertype,
								datatype:'xml',
								cache:false,//如果值变化可能性比较大 一定要将缓存设成false
								timeout: 1000,
								async: false ,//同步方式
			...
			*****************************************/
				function getUserPower(){
				 var urlstr=tsd.buildParams({ 	  packgname:'service',
												  ...
												  method:'exequery',
							 					  ds:'tsdBilling',//数据源名称 对应的spring配置的数据源
							 					  tsdpname:'StopKeepNumber.quanxian',//存储过程的映射名称
							 					  tsdExeType:'query',
							 					  datatype:'xmlattr'
						 					});
				/************************
    	...
    	var iStatusto;//传值用户类型全局
    	var PauseTimeTO;//停机时间全局变量
	...
	//////////////////////////////////////////////////////////////////
	///判断该用户的当前时间是否在停机保号开始时间和停机保号结束时间之中	
	//////////////////////////////////////////////////////////////////
	var resultto="";
	function sftjbh(UserName){
	   var urlstr=tsd.buildParams({ 	packgname:'service',//java包
						 					clsname:'ExecuteSql',//类名
						 					method:'exeSqlData',//方法名
						 					ds:'broadband',//数据源名称 对应的WEB-INF/classes/ApplicationContent.xml中配置的数据源
						 					...
						 					tsdoper:'query',//操作类型 
						 					datatype:'xmlattr',
						 					tsdpk:'radcheck.sftjbhtime',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
						 					tsdUserID:'ture'
						 				});	
			$.ajax({
					url:'mainServlet.html?'+urlstr+'&UserName='+UserName,
					...
					success:function(xml){
						...
						$(xml).find('row').each(function(){
						    ...
						    if(result==0){
						       $("#tjbhzzzz").val("1");
						       //办理停机保号
						       //$("#profetype").val("<fmt:message key="stopkeepnumber.bltjbh"/>");
						       $("#profetype").empty();
						       $("#profetype").append("<option value=\"申请停机\">"+'申请停机'+"</option>");
						       $("#profetype").attr("disabled","disabled");
						       
						       $("#ifstlabel").show();
						       //$("#iFeeStopType").show();
						       //$("#iFeeStopType").removeAttr("disabled");
						       //$("#PauseStopTime1").show();						       
						       //$("#timets").text("(<fmt:message key="stopkeepnumber.sjts"/>)");//选择该项系统将在选择的规定时间自动恢复停机，如果不选可通过营业厅来办理开机!
						       $("#timets").show();
						       //$("#tjstoptime").show();
						       //$("#stime").hide();
						       //$("#PauseTime").hide();		
						       $("#d_d_d").css("display","block");				       
						       //$("#iFeeStopType").val("<fmt:message key="stopkeepnuber.qxz"/>");//请选择
						       iStatusto="tjbh";
						    }else if(result==1){						  
						       $("#tjbhzzzz").val("0");
						       //$("#profetype").val("<fmt:message key="stopkeepnumber.hftjbh"/>");//恢复停机保号
						       $("#profetype").empty();
						       $("#profetype").append("<option value=\"申请复机\">"+'申请复机'+"</option>");
						       $("#profetype").append("<option value=\"申请停机\">"+'申请停机'+"</option>");
						       $("#profetype").removeAttr("disabled");
						       $("#ifstlabel").show();
						      ...
						      // $("#stime").show();
						       ...
						       $("#d_d_d").css("display","none");
						       $("#timets").hide();
						       //$("#iFeeStopType").attr("disabled","disabled");
						       iStatusto="hftjbh";
						    }
					   });	
					}            		
	...
	/////////////////////////////////////////////////////////////////
			...
			$.ajax({
					...
					success:function(xml){
						...
						$(xml).find('row').each(function(){						 
							...
							sftjbh(UserName);//判断该用户的当前服务器时间是否在停机保号开始时间和停机保号停止时间段中
							...
							if(idtype==8){$("#idtype").val("其他");}
							var idcard = $(this).attr("idcard");
							$("#idcard").val(idcard);
							var mobile = $(this).attr("mobile");
							$("#mobile").val(mobile);
							var PauseStartTime = $(this).attr("pausestarttime");//停机开始时间
							//$("#PauseTime").val(PauseStartTime);
							//if(iStatus==0||iStatus==1){
							//   $("#PauseTime").val(PauseStartTime);
							//}
							var PauseStopTime = $(this).attr("pausestoptime");//停机结束时间
							//if(iStatus==2||iStatus==3){
							//   $("#PauseTime").val(PauseStopTime);
							//}						
	           ...
	           ***对停机保号状态进行变更
	           *****************************************/
	           var resultqj;//返回选项时间是否大于当前时间 0为小于当前时间，1为大于当前时间；
	           //跟新计费规则变更带来的带宽变更
	           function userUpdate(){
	               ...
	               var PauseStopTime1 = $("#PauseStopTime1").val();//选择停机保号结束的时间	               
	               var speed = $("#speed").val();
	               var feei = $("#feei").val();
	               var Q = QJiFeeType;
	               var src=qjfeename;
	               speed = tsd.encodeURIComponent(speed);
	               ...
	               /////////////////////////////办理停机保号时执行如下操作////////////////////////////////////////////
	               if(confirm("<fmt:message key="stopkeepnumber.qddueiuser"/>"+[username]+"申请停机吗？")){
	               if(getinterface()=='YES'){
		               /*调用接口，如果返回为true则继续执行下列的开户超做，如果返回false则提示遇到异常开户失败*/			 			 			 
					   var jkres=TSDInterface.ZXISAM.changeStatus(username,2);
						 	jkres = eval("("+jkres+")");
						 	if(jkres.info!="true"&&jkres.info!=true){
						 		alert("接口出现["+jkres.resultInfo.infoStr+"]异常，无法执行停复机操作过程！");
						 		return false;
						 }
	               }
	               var urlstr=tsd.buildParams({packgname:'service',//java包
						 					...
						 					ds:'broadband',//数据源名称 对应的WEB-INF/classes/ApplicationContent.xml中配置的数据源
						 					tsdExeType:'query',
						 					tsdpname:'stopkeepnumer.save',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
						 					datatype:'xmlattr',
									 	    tsdUserId:true
						 				});			 				 					
			       $.ajax({
					url:'mainServlet.html?'+urlstr+'&userid='+userid+'&area='+area+'&depart='+depart+'&username='+username+'&feetype='+''+'&fee='+feei+'&feename='+queryFeeInfo()+'&func='+iStatusto+'&redate='+getPauseStopTime()+"&paymode="+paymode+"&ispay="+ispay+"&hth="+hth+"&ifurgent="+ifurgent+"&descrpt="+jobmemo,
					datatype:'xml',
				    cache:false,//如果值变化可能性比较大 一定要将缓存设成false
				    timeout: 1000,
					async: false ,//同步方式
					...
					$(xml).find('row').each(function(){
						var result = $(this).attr("result");
						if(result==null){
						   alert("<fmt:message key="gjh.operationfailed"/>");
						}else{
						    var payitem = $("#payitem").val();//得到数据加载到隐藏域中
							$("#payitemYCY").val(payitem);
							
						   //alert("<fmt:message key="gjh.operationSeccecssful"/>");
						  //日志提示
						  $("#jobidid").val(result);
						   $("#printfeeee").val(feei);
						   autoBlockForm("choosePrintJob","150","close","#FFFFFF",false);
						   
						  var loginfo = tsd.encodeURIComponent("申请停机");
						  loginfo += ";";
				 ...
				 }else if(profetype=='申请复机'){
				 ///////////////////////////////恢复停机保号时执行如下操作//////////////////////////////////////////////////	
				 if(confirm("<fmt:message key="stopkeepnumber.qddueiuser"/>"+[username]+"申请复机？")){
	              if(getinterface()=='YES'){
	              /*调用接口，如果返回为true则继续执行下列的开户超做，如果返回false则提示遇到异常开户失败*/			 			 			 
				   var jkres=TSDInterface.ZXISAM.changeStatus(username,1);
					 	jkres = eval("("+jkres+")");
					 	if(jkres.info!="true"&&jkres.info!=true){
					 		alert("接口出现["+jkres.resultInfo.infoStr+"]异常，无法执行停复机操作过程！");
					 		return false;
	              ...
	              var urlstr=tsd.buildParams({packgname:'service',//java包
			       ...
			       $.ajax({
					url:'mainServlet.html?'+urlstr+'&userid='+userid+'&area='+area+'&depart='+depart+'&username='+username+'&feetype='+''+'&fee='+feei+'&feename='+queryFeeInfo()+'&func='+iStatusto+'&redate='+PauseStopTime1+"&paymode="+paymode+"&ispay="+ispay+"&hth="+hth+"&ifurgent="+ifurgent+"&descrpt="+jobmemo,
					datatype:'xml',
				    cache:false,//如果值变化可能性比较大 一定要将缓存设成false
				    timeout: 1000,
					async: false ,//同步方式
					...
					$(xml).find('row').each(function(){
						...
						if(result==null){
						  ...
						  autoBlockForm("choosePrintJob","150","close","#FFFFFF",false);
						  var loginfo = tsd.encodeURIComponent("申请复机");
						  loginfo += ";";
		...
		var ids="";
		var idsto="";
	    //判断该用户是办理停机保号还是回复停机保号来取相应的杂费类型
		if(resultto==0){
		  bnames='pause';//停机保号
		}else if(resultto==1){
		 bnames='continue';//回复停机保号
		}
		var urlstr=tsd.buildParams({packgname:'service',//java包
						 					clsname:'ExecuteSql',//类名
						 					method:'exeSqlData',//方法名
		                   ...
		                   $.ajax({
					              url:'mainServlet.html?'+urlstr+'&bname='+tsd.encodeURIComponent(bnames),
					              datatype:'xml',
					              cache:false,//如果值变化可能性比较大 一定要将缓存设成false
					              timeout: 1000,
					              async: false ,//同步方式
		...
		var sim="";
		  function stoptime(){
		  var systemtype='broadbandstop';//代表的系统项
		  var propertytype='stopTime';//属性值
	      var urlstr=tsd.buildParams({ 	packgname:'service',//java包
						 					clsname:'ExecuteSql',//类名
						 					method:'exeSqlData',//方法名
						 					ds:'broadband',//数据源名称 对应的WEB-INF/classes/ApplicationContent.xml中配置的数据源
			...
			$.ajax({
					...
					success:function(xml){
							...
							$("#PauseStopTime1").html("<option value='' selected=true><fmt:message key="gjh.select"/></option>");				
						    $("#PauseStopTime1").html($("#PauseStopTime1").html()+sim);
						}
					});
				}
          ...
          function quereyshiftj(){
			...
			function kdLock(UserName)
			{
				...
				var res = fetchMultiPValue("kd.Lock",0,"&userid="+$("#userid").val()+"&account="+UserName+"&businesstype=pause&flag=in"+"&realname="+tsd.encodeURIComponent(rel));
				...
				<div id="input-Unit">
					<div id="inputtext">
						<div class="title">
							...
							<fmt:message key="broadband.yhsqtj" /><!-- 用户申请停复机 -->							
						</div>
						
						<div id="inputtext" style="margin-top: 5px;">
								<table id="c_c_c"><tr><td>
								<label for="iStatus" style="width: 70px">
								<!-- 业务类型 -->
									<fmt:message key="stopkeepnumber.ywlx"/>									
								</label>
								<select name="profetype" id="profetype"
									style="width: 80px"></select>
								</td><td style="display:none">
						        	<label for="cc_time_tj"><fmt:message key="broadband.sqtjtime" /><!-- 申请停机时间 --></label><input type="text" id="cc_time_tj" disabled="disabled" style="width: 130px"/></td>
									<td style="display:none"><label for="cc_time_fj"><fmt:message key="broadband.zdfjtiime" /><!-- 自动复机时间 --></label><input type="text" id="cc_time_fj" disabled="disabled" style="width: 130px"/>
								</td></tr></table>	
				
							<table id="d_d_d" style="display:none"><tr><td>
							 <label for="StartTime" id="tjstoptime" style="width: 110px">
							 <!-- 停机结束时间 -->
								 <fmt:message key="broadband.tjtimeyuef" /><!--  停机时间月份数 -->
							 </label>
							 <select name="PauseStopTime1" id="PauseStopTime1" onchange="getPauseStopTime()" style="width:50px;">
								
							</select>
							</td><td>
							<label for="PauseTime" id="stime" style="width:120px">
								<!-- 停机保号时间 -->
									  <fmt:message key="broadband.xzdfjtime" /><!--  新自动复机时间 -->
								</label>
								<input type="text" name="" id="PauseTime"
									class="required date"
						...
						<div id="inputtext" style="margin-top: 3px;">
							<p>
...
<div style="display: none;">					
					...
					<div style="display: none">
						...
						<input type="hidden" name="iFeeStopType" id="iFeeStopType"
							value="-1" />
						;												
						<input type="hidden" name="tjbhzzzz" id="tjbhzzzz" value="0" />
					</div>
</div>
					<div id="buttons">
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<div class="top_01" id="thisdrag">
					<div class="top_01left">
						...
						<div class="top_03" id="titlediv">
							<fmt:message key="broadband.SQTFJusernamequery" /><!-- 宽带申请停复机用户信息查询 -->
						</div>
						<div class="top_04">
