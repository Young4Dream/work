  ...
  <head>    
	<title>话费查询</title>
		...
		$(function(){
			...
			var column  = '';
			var thisdata = loadData('Jfgl_Dngyzb_two','<%=languageType%>',1,'');
			...
			jQuery("#editgrid").jqGrid({  	
				...
				datatype: "xml",
			   	colNames:['操作','汇总月份','合同号','用户名称','一级部门','二级部门','月租合计','话费合计','话费补贴','转账','序号3'],
			   	colModel:[
			   		...
			   		{name:'yuezu',index:'yuezu', width:70},
			   		{name:'hf',index:'hf', width:70},
			   		{name:'hfbutei',index:'hfbutei', width:70},					
			   		{name:'zhuanzhang',index:'zhuanzhang', width:70},
			   		{name:'xh3',index:'xh3', width:70}
			   	],
			   	rowNum:15,
			   	rowList:[15,30,128],
		...
		function modifyInfo(){
			...
			var zhuanzhang=$("#zhuanzhang").val();
			 ...
			 var urlstr=tsd.buildParams({ 	packgname:'service',//java包
						 					...
						 					tsdoper:'exe',//操作类型 
						 					tsdpk:'dngyhzquery.modify'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
						 				});
					urlstr+=params;
				$.ajax({
		...
		function queryInfo(){
			...
			var res = fetchSingleValue('dngyhzquery.listpage',6,'&hzyf='+hzyf);//是否已经汇总
			if(res=='0'){
			
				var flag=fetchMultiPValue('dngyhzquery.Hftj_dngyzb_two',6,'&hzyf='+hzyf);
		...
		function initGrid_Detail(yf){
			...
			///设置命令参数
			 var urlstr=tsd.buildParams({ 	
				...
				tsdoper:'query',//操作类型 
				datatype:'xml',//返回数据格式 
				tsdpk:'dngyhzquery.list',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
				tsdpkpagesql:'dngyhzquery.listpage'//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
			});
			
			$("#editgrid").setGridParam({url:'mainServlet.html?'+urlstr+"&hzyf="+yf}).trigger("reloadGrid");
		}
		...
		********************************************************/
		function fuheQuery()
		{
				...
				if(params=='&fusearchsql='){
			 	...
			 	var urlstr1=tsd.buildParams({  packgname:'service',//java包
											  ...
											  tsdoper:'query',//操作类型 
											  datatype:'xml',//返回数据格式 
											  tsdpk:'dngyhzquery.fuheQueryByWhere',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
											  tsdpkpagesql:'dngyhzquery.fuheQueryByWherepage'
											});
		...
		{
			var reportfilename = "jfgl_Dngyzb_two";
  ...
  <body>
	...
	<table id="queryparam">
		<tr>
			<td width="25%">查询月份</td>
			<td>
				<input type="text" id="hzyf"  onfocus="WdatePicker({startDate:'%y-%M',dateFmt:'yyyyMM',alwaysUseStartDate:true})" style="width: 150px;margin-top: 5px;margin-bottom: 5px;height: 20px;margin-left: 15px;line-height: 20px;"/>
	...
	<div id="infolist" style="display: none;">
		...
		<div id="buttons">
			...
			<button type="button" id='gjquery1' onclick="openDiaQueryG('query','Jfgl_Dngyzb_two');">
				<!--高级查询-->
				<fmt:message key="oper.queryA"/>
			</button>
			<button type="button" id='gjquery1' onclick="ghPrintPrew();">
				预览打印
			</button>
