<%------------------------------------------ 
    File Name:  trunk/boss-bs/src/tsdboss/WebRoot/WEB-INF/template/telephone/business/BulkAllCJ.jsp
    Function:   电话批量拆机
    Author:     wenxuming
    Date:       2012-06-26
    Description: 
...
<html>
	<head>
		...
		{	    
			...
			$("#Bz5new_yhdang").html("<option value=''>--请选择--</option><option value='局端'>局端</option><option value='远端'>远端</option>");
		}); 
		...
		**********/
	    ...
	    var newaddress = $("#newAllAdress").val().replace(/(^\s*)|(\s*$)/g,"");
	    if(newaddress==""){
	    		alert("请输入新地址!");
	    		$("#newAllAdress").select().focus();
	    		return;
	    	}
	    var bz1new_yhdang = $("#bz1new_yhdang").val().replace(/(^\s*)|(\s*$)/g,"");
	    var Bz5new_yhdang = $("#Bz5new_yhdang").val();	
	    var cPayType = $("#cPayType").val();
			...
			params+="&newaddress="+tsd.encodeURIComponent(newaddress);	
			params+="&bz1new_yhdang="+tsd.encodeURIComponent(bz1new_yhdang);//专（光）线路	
			params+="&Bz5new_yhdang="+tsd.encodeURIComponent(Bz5new_yhdang);	//区域端点
			params += "&zwbz=";
			...
			loginfo += area;
			loginfo += ")(移机新地址:";
			loginfo += $("#newAllAdress").val();
			loginfo += ")";
			loginfo = tsd.encodeURIComponent(loginfo);
			params += "&ywbz="+loginfo;
			params+="&Dhmc="+Dhmc;
			if(confirm("你确定对合同号["+$("#this_hth").text()+"]帐户下的电话进行批量移机么？"))
			{
				var res = fetchMultiPValue("ywsl_Bulk_yj",6,params);							
	  ...
	  function getdhcountvalue(){		
			...
			$("#dhcount").text("当前移机电话数量："+rescount+"个");
		}
		</script>
	</head>
	<body>
	   ...
	   <div id="input-Unit" style="margin-bottom:0px;">			
		...
		<table id="tdiv" border="0"
			...
			<tr>
				...
				<td width="20%" class="tdstyle">
					<input type="text" id="newAllAdress" style="width:180px;"/> 
				</td>
				<td class="labelclass">
					<label for="sarea">
						<span id="">专（光）线路</span>
				...
				<td width="20%" class="tdstyle">
					<input id="bz1new_yhdang" name="bz1_yhdang" style="width:180px"/>
				</td>
				<td class="labelclass">
					<label for="sarea">
						<span id="">区域端点</span>
				...
				<td width="20%" class="tdstyle">
					<select id="Bz5new_yhdang" name="Bz5new_yhdang" style="width:180px" ></select>
				</td>
			</tr>
			<tr>
				<td class="labelclass">
				...
				<td width="20%" class="tdstyle">
					<input type="text" id="newLxr" style="width:180px;"/> 
				</td>
				<td class="labelclass">
					<label for="sarea">
						<span id="">联系电话</span>
				...
				<td width="20%" class="tdstyle">
					<input id="newLxdh" name="newLxdh" style="width:180px"/>
				</td>
			</tr>
		</table>
	   <div class="title">
		 		...
		 		<tr>
		 			<td style="width:460px;">
						...
						<tr>
							...
							<td class="wenzix">
								<select name="cPayType" id="cPayType" style="width: 140px" class="you1" onChange="payWayChange(this)"></select>
							</td>
							<td class="wenzi">
	...
	<div id="buttons" style="text-align:center">
		<button id="kdcx" style="width:120px;" onClick="cxJob()" disabled>批量移机</button><button id="kdback" style="width:120px;" onClick="respage()"><fmt:message key="AddUser.Reset" /></button>
	</div>		
