...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	*/
	...
	var tables = ["feilu","jcfeilu","fljm","charge_type"];
	...
	function loadFeilu(models)
	{
		var urlstr=tsd.buildParams({ packgname:'service',//java包
									...
									tsdoper:'query',//操作类型 
									datatype:'xml',//返回数据格式 
									tsdpk:'Rate1Feilu.query',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
									tsdpkpagesql:'Rate1Feilu.querypage'//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
									});
   ...
   * 计次费率
	...
	function loadJcFeilu(models)
	{
		var urlstr=tsd.buildParams({ packgname:'service',//java包
									...
									tsdoper:'query',//操作类型 
									datatype:'xml',//返回数据格式 
									tsdpk:'Rate1Jcfeilu.query',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
									tsdpkpagesql:'Rate1Jcfeilu.querypage'//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
									});
	...
	function loadFljm(models)
	{
		var urlstr=tsd.buildParams({ packgname:'service',//java包
									...
									tsdoper:'query',//操作类型 
									datatype:'xml',//返回数据格式 
									tsdpk:'Rate1Fljm.query',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
									tsdpkpagesql:'Rate1Fljm.querypage'//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
									});
	...
	function loadChargeType(models)
	{
		var urlstr=tsd.buildParams({ packgname:'service',//java包
									...
									tsdoper:'query',//操作类型 
									datatype:'xml',//返回数据格式 
									tsdpk:'Rate1Charge_Type.query',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
									tsdpkpagesql:'Rate1Charge_Type.querypage'//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
									});
	...
	function getI118n()
	{
		...
		//alert(tabStatus-1);
		Dat = loadData(tables[tabStatus-1],$("#languageType").val(),1,"<fmt:message key='Set1rate.modifydelete' />");	
	...
	function InitialDropdownList()
	{
		...
		$("#NetName_1,#NetName_2").append("<option value=\"<fmt:message key='Set1rate.choose' />\"><fmt:message key='Set1rate.choose' /></option>");
		...
		$("#Jmlx_1").append("<option value=\"\"><fmt:message key='Set1rate.choose' /></option>");
		...
		$("#Jrlx_3,#HoliDay_Type_2").append("<option value=\"<fmt:message key='Set1rate.choose' />\"><fmt:message key='Set1rate.choose' /></option>");
		...
		$("#Call_Type_2,#Thlx_4").append("<option value=\"<fmt:message key='Set1rate.choose' />\"><fmt:message key='Set1rate.choose' /></option>");
	...
	{	
		$(".top_03").html("<fmt:message key='Set1rate.modify' />");
		if($("#editright").val()=="false")
		{
			alert("<fmt:message key='Set1rate.nomodifyright' />");
			//return false;
		}
		else
		{
	...
	{	
		...
		for(j=0;j<primaryKeys[tabStatus-1].length;j++)
		{
			...
			if($(controlBom).val()=="" || $(controlBom).val()=="<fmt:message key='Set1rate.choose' />")
			{
				var tmp = "";
				tmp = $(controlBom).parent().prev().text();
				alert($.trim(tmp) + "<fmt:message key='Set1rate.canotbeempty' />");
				$(controlBom).select();
				$(controlBom).focus();
				return false;								
			}
		...
		if(parseInt(res)>0)
		{
			alert("<fmt:message key='Set1rate.recordexist' />");
			//第一个主键输入框获得焦点
		...
		for(var k=0;k<lables.length;k++)
		{
			...
			if($("#"+tmp).attr("dtype")!=undefined && $("#"+tmp).attr("dtype")!='int' && $("#"+tmp).attr("dtype")!='double')
			{
				...
				{
					alert($.trim($(lables[k]).text()) + "<fmt:message key='Set1rate.falseform' />" + getType($("#"+tmp).attr("dtype")) + ".");
					$("#"+tmp).select();
					$("#"+tmp).focus();
					return false;
				}
		...
		var params = "";
		//日志信息
		var loginfo = "<fmt:message key='Set1rate.setonerate' />";
	...
	function getType(type)
	{
		if(type=="int")
		{
			return "<fmt:message key='Set1rate.integer' />";
		}
		else if(type=="double")
		{
			return "<fmt:message key='Set1rate.number' />";
		}
		if(type=="datetime")
		{
			return "<fmt:message key='Set1rate.date' />";
		}
		else
		{
			return "<fmt:message key='Set1rate.char' />";
		}
	}
	/**
	 * 修改数据
	...
	function ModifyData()
	{
		...
		if(!fuheM)
		{
			...
			if(res=="true"||res==true)
			{
				alert("<fmt:message key='Set1rate.modifydata' />");
	...
	function deleteRow()
	{
		...
		{
			alert("<fmt:message key='Set1rate.nodeleteright' />");
			//return false;
		}
		else
		{//生成参数串
			...
			//alert(params);		
			//删除
			if(confirm("<fmt:message key='Set1rate.willyoudeletethisrecord' />"))
			{
	...
	function openAddForm()
	{
		...
		toggleBtn(1);
		clearText("addform"+tabStatus,2);
		$(".top_03").html("<fmt:message key='Set1rate.add' />");		
	...
	function openSearchForm()
	{
		...
		$(".top_03").html("<fmt:message key='Set1rate.seek' />");
		//隐藏星号
	...
	function fuheDel()
	{
		...
		if(confirm("<fmt:message key='Set1rate.deletealldata' />"))
		{
		 	...
		 	$.ajax({
					...
					success:function(msg){
						if(msg=="true"){
							...
							loginfo += tables[tabStatus-1];
							loginfo += ";";
							loginfo += "<fmt:message key='Set1rate.condition' />:";
							loginfo = tsd.encodeURIComponent(loginfo);
					
							loginfo += $("#fusearchsql").val().replace(new RegExp("'","g"),"\"");
							writeLogInfo("","batchdelete",loginfo);
														
							alert("<fmt:message key='Set1rate.deletesucess' />");
	...
	function fuheModify()
	{
		...
		for(var k=0;k<lables.length;k++)
		{
			...
			if($("#"+tmp).val()!="" && $("#"+tmp).val()!="<fmt:message key='Set1rate.choose' />")
			{
		...
		$.ajax({
			...
			success:function(msg){
				if(msg=="true"){//日志
					...
					loginfo += queryStr;
					loginfo += tsd.encodeURIComponent(";<fmt:message key='Set1rate.condition' />:");
	...
	function DownSure()
	{
		getTheCheckedFields('tsdCDR','mssql',tables[tabStatus-1]);
	}
	/**
	 * 数据导出
	 * @param 
			...
			<div class="top_01" id="thisdrag">
				<div class="top_01left">
					...
					<div class="top_03" id="titlediv">
						<fmt:message key="Set1rate.setonerate&ratetable" />
					</div>
					<div class="top_04">
				...
				<div class="top_01right">
					<a href="javascript:;" onclick="javascript:$('#addform1Close').click();"><fmt:message key="Set1rate.close" /></a>
				</div>
			</div>
	...
	<form class="neirong" id="addform2" style="display:none;width:720px;">
		<div class="top">
			<div class="top_01" id="thisdrag">
				...
				<div class="top_01right">
					<a href="javascript:;" onclick="javascript:$('#addform2Close').click();"><fmt:message key="Set1rate.close" /></a>
				</div>
			</div>
			<div class="top02right">
				<img src="style/images/public/toptiaoright.gif" />
		...
		<div class="top">
			<div class="top_01" id="thisdrag">
				<div class="top_01left">
					...
					<div class="top_03" id="titlediv">
						<fmt:message key="Set1rate.setonerate&ratereduce" />
					</div>
					<div class="top_04">
						<img src="style/images/public/neibut03.gif" />
					</div>
				...
				<div class="top_01right">
					<a href="javascript:;" onclick="javascript:$('#addform3Close').click();"><fmt:message key="Set1rate.close" /></a>
				</div>
			</div>
			<div class="top02right">
				<img src="style/images/public/toptiaoright.gif" />
			...
			<table style="width:100%">
				<tr>
					...
					<td width="25%" class="tabinputclass">
						<input type="text" name="Jmlx_3" id="Jmlx_3"  class="textclass" maxlength="1" ></input>
					</td>
					<td class="labelclass" width="25%">
		...
		<div class="top">
			<div class="top_01" id="thisdrag">
				<div class="top_01left">
					...
					<div class="top_03" id="titlediv">
						<fmt:message key="Set1rate.setonerate&feestyle" />
					</div>
					<div class="top_04">
						<img src="style/images/public/neibut03.gif" />
					</div>
				...
				<div class="top_01right">
					<a href="javascript:;" onclick="javascript:$('#addform4Close').click();"><fmt:message key="Set1rate.close" /></a>
				</div>
			</div>
			<div class="top02right">
				<img src="style/images/public/toptiaoright.gif" />
				...
				<tr>
					<td class="labelclass">
						<label for="Fuf_Jmbl_4"><!-- 服务费减免比率 -->
							<span id="Fuf_Jmbl_label_4"></span>
							<span id="ST9_lf" style="display:none;"></span>
							<span id="ST9_ld" style="display:none;"></span>
						</label>
					</td>
					<td class="tabinputclass">
	...
	<div id="modT" name='modT' style="display: none"  class="neirong">
		...
		<div class="top_01">
				<div class="top_01left">
					...
					<div class="top_03" ><fmt:message key="Set1rate.functionname" /></div>
	...
	<div class="neirong" id="thefieldsform" style="display: none;width:800px">
		<div class="top">
			<div class="top_01" id="thisdrag">
					<div class="top_01left">
						<div class="top_02"><img src="style/images/public/neibut01.gif" /></div>
						<div class="top_03" id="titlediv"><fmt:message key="Set1rate.chooseexportdata" /></div>
						<div class="top_04"><img src="style/images/public/neibut03.gif" /></div>
					</div>
					<div class="top_01right"><a href="#" onclick="javascript:$('#closethisinfo').click();"><fmt:message key="Set1rate.close" /></a></div>
			</div>
			<div class="top02right"><img src="style/images/public/toptiaoright.gif" /></div>	
		</div>
		<div class="midd" >
		...
		<div class="midd_butt">
			<button type="submit" class="tsdbutton" id="query" onclick="checkedAllFields()">
				<fmt:message key="Set1rate.selectall" />
			</button>
