		...
		<script type="text/javascript">
			...
			window.onload = function(){
				...
				btnMmfSave_click = function(){
					...
					//yyl add
					var relatemenuid1 = document.getElementById('relatemenuid').value;
					var relatefields1 = document.getElementById('relatefields').value;
					var deletechecksql = document.getElementById('deletechecksql').value;
					var addTip1 = document.getElementById('addTip').value;
					var modTip1 = document.getElementById('modTip').value;
					...
					if (manformtitle.innerHTML=="<fmt:message key='menuManager.updateMenu' />") {
						...
						//根据ID修改菜单内容
						var parentid=manform.nowNode.parentNode.id;
						if (parentid=="root"){
							parentid=0;
						}
						Ext.Ajax.request({
							...
							params: {
								...
								smenuurl: smenuurl,
								iparentid: parentid,//获取当前节点的父节点id
								tableName:tableName1,
								printName:printName1,		
								relatemenuid:relatemenuid1,		
								relatefields:relatefields1,			
								deletechecksql:deletechecksql,
								addtip:addTip1,	
								modtip:modTip1,	
						...
						Ext.Ajax.request({
							...
							success: function(response, options){
								...
								if(maximenuid!=0){
								    ...
								    //alert("parentid=="+manform.nowNode.parentNode.imenuid.value);
									//manform.isvisible.value
									//将新节点插入到选择节点上面
									newnode = manform.nowNode.parentNode.insertBefore(newnode,manform.nowNode);
									var pnodeid=manform.nowNode.parentNode.id;//获取当前节点的父节点id
									if(pnodeid=="root"){
										pnodeid=0;
									}
									//3.将新节点插入到数据库
				...
				loadUpdateMenu = function(p_menuid,p_node){
					...
					//yyl add 清空新增字段
					document.getElementById("relatemenuid").value = "";
					document.getElementById("relatefields").value = "";
					document.getElementById("deletechecksql").value = "";
					document.getElementById("addTip").value = "";
					document.getElementById("modTip").value = "";
					...
					Ext.Ajax.request({
						...
						success:function(response,options){
							...
							if(_q.selectValue('@smenuurl',node,"") == 'pubMode/SingleTabE.jsp'){
								...
								//yyl add 新增字段赋值
								document.getElementById("relatemenuid").value = _q.selectValue('@relatemenuid'.toLowerCase(),node,"");
								document.getElementById("relatefields").value = _q.selectValue('@relatefields'.toLowerCase(),node,"");
								document.getElementById("deletechecksql").value = _q.selectValue('@deletechecksql'.toLowerCase(),node,"");
								document.getElementById("addTip").value = _q.selectValue('@addtip'.toLowerCase(),node,"");
								document.getElementById("modTip").value = _q.selectValue('@modtip'.toLowerCase(),node,"");
		...
		<form id='operform' name="operform" onsubmit="return false;" action="#" >
			<table width="100%" id="tdiv" border="0" cellspacing="0" cellpadding="0" style="line-height:33px; font-size:12px;">
				 <tr>			    
			 	 ...
			 	 <tr>			    
				 	<!-- 子表名 -->
				    <td width="20%" align="right"  class="labelclass"><label id=''>子表名</label></td>
			    	<td width="30%" align="left" style="border-bottom:1px solid #A9C8D9;">
			    		<input type="text" name="relatemenuid" id="relatemenuid" maxlength="50" class="textclass"  />   
			    	</td>
					<!-- 关联字段-->
					<td width="20%" align="right"  class="labelclass"><label id=''>关联字段</label></td>
				    <td width="30%" align="left" style="border-bottom:1px solid #A9C8D9;" title="格式为： 子表中字段=<数据库表中字段>，若有多个关联字段中间用英文逗号隔开">
				    	<input type="text" name="relatefields" id="relatefields" maxlength='50' class="textclass" />
				    </td>
			 	 </tr>
			 	 <tr>			    
				 	<!-- 删除校验 -->
				    <td width="20%" align="right"  class="labelclass"><label id=''>删除校验SQL</label></td>
			    	<td width="80%" colspan="3" align="left" style="border-bottom:1px solid #A9C8D9;" title="关联到数据库表中的字段需要用<>括起来，中间不可有空格;若有多个关联sql中间用英文分号隔开">
			    		<textarea name="deletechecksql" id="deletechecksql" style="height:28px" class="textclass"/></textarea>
			    	</td>
			 	 </tr>
			 	 <tr>			    
				 	<!-- 新增操作提示消息 -->
				    <td width="20%" align="right"  class="labelclass"><label id=''>新增操作提示消息</label></td>
			    	<td width="30%" align="left" style="border-bottom:1px solid #A9C8D9;"  title="用于在新增操作时提示信息">
			    		<input type="text" name="addTip" id="addTip" maxlength="50" class="textclass"  />   
			    	</td>
					<!-- 修改操作提示消息-->
					<td width="20%" align="right"  class="labelclass"><label id=''>修改操作提示消息</label></td>
				    <td width="30%" align="left" style="border-bottom:1px solid #A9C8D9;" title="用于在修改操作时提示信息">
				    	<input type="text" name="modTip" id="modTip" maxlength='50' class="textclass" />
				    </td>
			 	 </tr>
			    		...
			    		<div id="thedeptm" >
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<tr>	
									...
									<td height="20" width="10%" color="#666666">
										<input type='checkbox' id='right' name='right' value='import' style='width:15px;height:15px;border:0px;'/>
										<!-- 将查询保存为模板 -->
										<label style='text-align: left;'>外部话费导入</label>
									</td>
