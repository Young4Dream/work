...
<!-- 导入css文件结束 -->
		...
		$(function(){			
			...
			//setUserRight();	
			Dat = loadData("radjiaofei",$("#lanType").val(),"2");
			//Dat.colModels[1].hidden=true;
			
			Dat.setWidth({"AcctPayTime":130,"thisPayMoney":120,"lastPayMoney":120,"PayMoney":60,"Operator":60,"feeendtime":120,"feestarttime":120,"heji":150});
						
			Datdh1 = loadData("jiaofei",$("#lanType").val(),"2");
			
			Datdh2 = loadData("jiaofei_detail",$("#lanType").val(),"2");
		...
		*/
		...
		function xzbzAdd()
		{
			if(confirm("<fmt:message key='Writeoff.writeoffConfirm' />?"))
			{
				...
				setTimeout("autoBlockForm('addKDBzForm',5,'addKDBzClose','#ffffff',false)",100);
				$("#addKDBzAddUnt").click(function(){
					if($.trim($("#addKDBzText").val())=="")
					{
						alert("<fmt:message key="writeOff.tuifeiMessage"/>");//必须填写退费备注信息
						$("#addKDBzText").focus();
						$("#addKDBzText").select();
					}
					else if($("#addKDBzText").val().length>100)
					{
						alert("<fmt:message key="writeOff.bzLenghMax"/>");//备注长度过长，最大允许100个字符
						$("#addKDBzText").focus();
						$("#addKDBzText").select();
					}
					else
		...
		function searchForDh_d(jid)
		{
			...
			param += "&CancelType=1";
			param += "&Hth=";
			param += tsd.encodeURIComponent($("#ghSearchBox").val());
			param += "&Area=";
			param += tsd.encodeURIComponent($("#area").val());
			param += "&times=";
			param += getPower();
			if(jid!=undefined)
			{
			...
			var res = fetchMultiPValue("writeoff.checkXiaoZhangHth",1,param);
			
			if(res.length<=0)
			{
				alert("<fmt:message key="Writeoff.noJiaofeiMsg"/>");//在可销帐的时限内没有缴费记录
				$("#ghSearchBox").select().focus();
				return false;
			}
			else
			{
				$("#gghh").data("dh",res[0][1]);
				$("#gghh").data("lsz",res[0][2]);
				...
				var urlM=tsd.buildParams({packgname:'service',//java包
									clsname:'Procedure',//类名
									method:'exequery',//方法名
				...
				var urlS=tsd.buildParams({packgname:'service',//java包
									...
									tsdcf:'mssql',//指向配置文件名称
									datatype:'xml',//返回数据格式 
									tsdpname:'WriteOff.getHthhf'
									});
				$("#ghgrid").setGridParam({url:"mainServlet.html?"+urlM+"&Func=GetJiaofei"+pm}).trigger("reloadGrid");
				$("#ghdgrid").setGridParam({url:"mainServlet.html?"+urlS+"&Func=GetHthhf"+pm}).trigger("reloadGrid");
		...
		*/
			...
			jQuery("#ghgrid").jqGrid({
				...
				colModel:Datdh1.colModels, 
				        ...
				        loadComplete:function(){
				        	...
				        	var lsz = $("#gghh").data("lsz");
				        	$.ajax({
				        		url:'mainServlet.html?'+urlstr + "&lsz=" + encodeURIComponent(lsz),
				        		cache:false,
				        		async:true,
				        		timeout:10000,
				        		success:function(data){
				        			var info = "<br /> <h2>本次缴费信息</h2>";
				        			var dtype = $(data).find("row:first").attr("dtype");
				        			if(dtype!=undefined)
				        			{
				        				if(dtype=="1")
				        				{
				        					info += "大客户号：" + $(data).find("row:first").attr("inputdata") + "，";
				        					info += "本次缴费账号数：" + $(data).find("row:first").attr("sl") + "，";
				        				}
				        				info += "应缴话费：" + $(data).find("row:first").attr("yjf") + "，";
				        				info += "滞纳金：" + $(data).find("row:first").attr("znj") + "，";
				        				info += "实收金额：" + $(data).find("row:first").attr("byck") + "，";
				        				info += "收款人员：" + $(data).find("row:first").attr("skry") + "，";
				        				info += "区域：" + $(data).find("row:first").attr("area") + "，";
				        				info += "付费方式：" + $(data).find("row:first").attr("pay_flag");
				        			}
		...
		function ghXiaoZhang()
		{
			...
			{				
				var res = fetchMultiPValue("writeoff.xiaozhang",1,params);
				if(res.length>0)
				{
					...
					//writeLogInfo("营收销账","固话销账",loginfo);						
				}
			}	
		}
  ...
  <body>				   
	...
	<div id="gh">
		<div id="ghBar">
			...
			<select id="chooseForFJ">
			</select>
			<fmt:message key="writeOff.telNum"/><!-- 电话号码 -->:
			<input type="text" class="inputbox" id="ghSearchBox" name="ghSearchBox" /><button class="tsdbutton" id="ghSearchByUserName"><fmt:message key="writeOff.query"/><!-- 查找 --></button>
			<div id="gghh"></div>
			<div id="ggdd"></div>
            <div id="ghxzinfo" style="font-size:14px;color:#000"></div>
			<div id="buttons" style="text-align:center">
				<button id="ghxz" style="width:120px;" disabled><!-- 销帐 -->返销账</button><button id="ghback" style="width:120px;"><!-- 返回 --><fmt:message key="writeOff.reset"/><!-- 重置 --></button>
			</div>
		</div>
	</div>
