...
<!-- 导入css文件开始 -->
	...
	function search()
	{
		...
		var params = "&mokuaiju=" + cjkEncode(mokuaiju) + "&qfje=" + cjkEncode(qfje) + "&tjys=" + cjkEncode(tjys) + "&hzyf=" + cjkEncode(hzyf);
  ...
  <!-- 
   ...
   -->
	...
	<fieldset style="margin-left:120px;margin-right:120px;margin-top:100px;font-size:14px;">
		<legend style="font-size:14px">未缴费清单</legend>
		<div style="font-size:14px;">
			<div id="tj_way_area" style="display:inline;">
				选择营业区域：
				<select id="stations" name="stations" style="width:120px;"></select>
			</div>
			未缴费金额>=<input type="text" id="wjfje" value="0" style="width:80px" />元,
			未缴费月数>=<input type="text" id="wjfys" value="1" style="width:80px" />月,
			查询时间<=<input type="text" id="wjfsj" style="width:80px" value="<%=hzdf.format(cld.getTime()) %>" onFocus="WdatePicker({startDate:'%y-%M',dateFmt:'yyyyMM'})" />
