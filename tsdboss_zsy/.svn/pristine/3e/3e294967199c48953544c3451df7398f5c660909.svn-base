...
<head>
	...
	<script type="text/javascript">
		...
		function genAcc() {
			...
			while(true) 
			{
				var radacc = 'tmp' + Math.round(Math.random() * 100000);
				var param = '&radacc=' + radacc;
				var res = fetchSingleValue('Rad.checkAcc', 6, param, 'workflow');
				if (res == 0 || res == '0') {
					$("#txtacct").val(radacc);
					return;
				}
		...
		function getServiceArea(){
		 	...
		 	if(res!=undefined&&res!=""){
			 	...
			 	$("#bandservicearea").html(str);
			 	$("#bandservicearea").val("丰和园小区");
		 	}else{
		 		$("#bandservicearea").html("<option value=''>--请选择--</option>");
		 	}
		}	
		...
		//开户按钮调用的函数
		function save(){
			if (NullCheck("#txtacct", "宽带账号")) {return};
			...
			if (NullCheck("#bandservicearea", "所属区域")) {return};
		   ...
		   var hao= $("#txtcardid").val();
		   var lxr = $("#txtlinkman").val();
		   var lxdh= $("#txtlinkphone").val();
		   if(hao==null||hao==undefined||hao==""){
		      alert("证件号不能为空！");
		      return;
		   }
		    if(lxr==null||lxr==undefined||lxr==""){
		      alert("联系人不能为空！");
		      return;
		   }
		   if(lxdh==null||lxdh==undefined||lxdh==""){
		      alert("联系电话不能为空！");
		      return;
		   }
			...
			if(confirm("是否跳转到号线管理系统")){
                        openMenu('4310','pubMode/SingleTabE.jsp','6','');
                    }	;		
		}
        //调用宽带开户的后台存储过程
        function savedb(){  
        	...
        	var v_servicearea = tsd.encodeURIComponent($("#bandservicearea").val());
        	...
        	var v_busifee = $("#txthj").val().replace(/(^\s*)|(\s*$)/g,"");
        	...
        	var enddate = $("#enddate").val().replace(/(^\s*)|(\s*$)/g,"");
         	var param=pubParam+"&internetacct="+v_acct+"&pwd="+v_pwd+"&username="+v_user+"&useraddr="+v_addr
        		...
        		+"&usertype="+v_yhlb+"&userattr="+v_yhxz+"&servicearea="+v_servicearea+"&pkgid="+v_pkgid+"&paytype="+v_paytype+"&callpaytype="+v_callpaytype
	...
	function getDhnull(){
				...
				$("#Dhnull").empty();
				s="<tr style='line-height: 22px; font-weight:bold; text-align:center;'>";
				s+="<td>电话号：<input type='text' id='cx'/><input  type='button' value='查询' onclick='selectnulldh();'/></td>";
				s+="</tr>";
				var res = fetchMultiArrayValue('patch.getDhnull',6,"","patch");
				if(res != undefined && res!="" && res.length>0){
				for(var i=0;i<res.length;i++){
					s+="<tr style='line-height: 22px; font-weight:bold; text-align:center;' onDblClick=\"setdh('"+res[i]+"')\">";
					s+="<td>"+res[i]+"</td>";
					s+="</tr>";
					}
					$("#Dhnull").append(s);
				        autoBlockForm('nulldh', 200, 'close', "#ffffff", false);
				}
	}
	function  selectnulldh(){
	 var dhvalue=$("#cx").val();
	 var s="";
				$("#Dhnull").empty();
				s="<tr style='line-height: 22px; font-weight:bold; text-align:center;'>";
				s+="<td>电话号：<input type='text' id='cx'/><input  type='button' value='查询' onclick='selectnulldh();'/></td>";
				s+="</tr>";
				var res = fetchMultiArrayValue('patch.dhvalue',6,"&dh="+dhvalue,"patch");
				if(res != undefined && res!="" && res.length>0){
				for(var i=0;i<res.length;i++){
					s+="<tr style='line-height: 22px; font-weight:bold; text-align:center;' onDblClick=\"setdh('"+res[i]+"')\">";
					s+="<td>"+res[i]+"</td>";
	...
	/* 已装电话选择框  */	
    function getDhyhdang1(){
            var s="";
				$("#Dhnull").empty();
				s="<tr style='line-height: 22px; font-weight:bold; text-align:center;'>";
				s+="<td>电话号：<input type='text' id='cxyh'/><input  type='button' value='查询' onclick='selectyhdh();'/></td>";
				s+="<td> </td>";
				s+="</tr>";
				s+="<tr style='line-height: 22px; font-weight:bold; text-align:center;'>";
				s+="<td>用户名</td>";
				s+="<td>电话号</td>";
				s+="</tr>";
				var res = fetchMultiArrayValue('patch.getDhyhdang',6,"","patch");
				if(res != undefined && res!="" && res.length>0){
				for(var i=0;i<res.length;i++){
					s+="<tr style='line-height: 22px; font-weight:bold; text-align:center;' onDblClick=\"setdh('"+res[i][0]+"')\">";
					s+="<td>"+res[i][1]+"</td>";
					s+="<td>"+res[i][0]+"</td>";
					s+="</tr>";
					}
    ...
    function  selectyhdh(){
	 ...
	 var s="";
				...
				s+="<td>电话号</td>";
				s+="</tr>";
				var res = fetchMultiArrayValue('patch.getyhdangDh',6,"&dh="+dhvalue,"patch");
				if(res != undefined && res!="" && res.length>0){
				for(var i=0;i<res.length;i++){
					s+="<tr style='line-height: 22px; font-weight:bold; text-align:center;' onDblClick=\"setdh('"+res[i][0]+"')\">";
					s+="<td>"+res[i][1]+"</td>";
	...
	function getDhyhdang(){
		$("#dhpage").empty();
		$("#dhpage").append("<table id=\"yhdh\" class=\"scroll\"></table><div id=\"dh_pager\" class=\"scroll\"></div>");
		var urlstr = tsd.buildParams({packgname:'service',//java包
							 					...
							 					tsdoper:'query',//操作类型 
							 					datatype:'xml',
							 					tsdpk:'patch.getDhyhdang',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
							 					tsdpkpagesql:'patch.getDhyhdangpage',
							 					tsdUserID:'ture'
							 				});
		$("#yhdh").jqGrid({
			...
			}).navGrid('#dh_pager', {edit: false, add: false, add: false, del: false, search: false}, //options 
			...
			autoBlockForm('yhdangdh',5,'kdMultiSearchClose',"#FFFFFF",false);
		  }
	</SCRIPT> 
</head>
<body onunload="unIfLock();">
	...
	<DIV class="ui-layout-center">	
		<div id="input-Unit" style="margin-top:-10px">
			...
			<font color="red">注意：宽带账号和密码可以不填写，工单完工时填写！</font><br/>
			宽带帐号:<input type="text" id="txtacct" name="txtacct" style="width:90px;" value="temp"/>&nbsp;
			<input id="btngetAcc" type="button" value="获取账号" onclick="showAccInfo();"/>
			<input id="btngetAcc" type="button" value="账号检测" onclick="isExists();"/><br /><br />			
			住宅电话:<input type="text" id="txtdh" name="txtdh" style="width:152px;"/>
             <input id="btnSave" type="button" value="同步电话信息" onclick="getDhinfo();"/> 
<!--            <input id="Dhyhdang"  type="button" value="已安装电话" onclick="getDhyhdang();"/>-->
            <input id="Dhyhdang" type="button" value="未安装电话" onclick="getDhnull();"/>
            <br />
            <br />			
			...
			用户密码:<input type="text" id="txtpwd" name="txtpwd" style="width:86px;" value="123456"/><font color="red">*</font> &nbsp
			确认密码:<input type="text" id="txtpwd1" name="txtpwd1" style="width:86px;" value="123456"/><font color="red">*</font> <br /><br />						
			...
			所属区域:
			<select id="bandservicearea" style="width:162px;">
	...
	<DIV class="ui-layout-east">
		<div id="input-Unit" style="margin-top:-10px; margin-left:-80px;">
			<div class="title_rad">				
				基本资料
			</div>
			...
			证件号码:<input type="text" id="txtcardid" name="txtcardid" style="width:120px;"/> <font color="red">*</font><br /><br />					
			联 系 人:<input type="text" id="txtlinkman" name="txtlinkman" style="width:120px;"/><font color="red">*</font> &nbsp&nbsp&nbsp
			联系电话:<input type="text" id="txtlinkphone" name="txtlinkphone" style="width:120px;"/><font color="red">*</font> <br /><br />
			移动电话:<input type="text" id="txtmobile" name="txtmobile" style="width:120px;"/><script>writespace(50)</script> <br /><br />
			营业区域:<input type="text" id="txtarea" name="txtarea" style="width:320px;background:silver;" readonly="true" /> 
...
<!-- 未安装电话div  -->
	<div class="neirong" id="nulldh" style="width:500px; display: none">
		<div class="top">
			<div class="top_01" id="thisdrag">
				<div class="top_01left">
					<div class="top_02">
		...
		<div class="midd" style="background-color:#FFFFFF;text-align:left;height:300px;">		
			<div id="page" style="overflow-y:scroll;height:100%;width: 100%;">					
				<table border="1" cellpadding="0" bordercolor="#9AC0CD" id="Dhnull" style="width:400px;">
					
														
				</table>
			</div>
		</div>
		<div class="midd_butt" style="width:490px;height:38px;">  
			<!-- <button id="hthChooseFormSave" onclick="caclbusifee();" class="tsdbutton" 
				style="margin-left: 20px;">
				确定
			</button> -->
	...
	<!-- 未安装电话div  -->
	<div class="neirong" id="yhdangdh" style="width:500px; display: none">
		<div class="top">
			<div class="top_01" id="thisdrag">
				<div class="top_01left">
					<div class="top_02">
		...
		<div class="midd" style="background-color:#FFFFFF;text-align:left;height:300px;">		
			<div id="dhpage" style="overflow-y:scroll;height:100%;width: 100%;">					
