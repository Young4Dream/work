...
function readJQgird(oper,urlstr,callback){
		...
		var col=[[],[]];
		//拼接要执行的字符串
		var evalstr = 'col=getRb_Field(tablealiasName,"\'id\'",oper,"100","ziduansF"';		
		...
		//yyl add
		if(subrelatefields != null && subrelatefields != 'null' && subrelatefields != ''){
			fuhesql +=" and " + subrelatefields.replace(/@/gm,"'");
		}
		//////////////////////////////////////////////
		/////设置命令参数
		var urlstr2=tsd.buildParams({ 	  packgname:'service',//java包
										  ...
										  tsdoper:'query',//操作类型 
										  datatype:'xml',//返回数据格式 
										  tsdpk:'publicmode.queryByOrderq',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
			...
			rowNum:10, //默认分页行数
			        ...
			        width:document.documentElement.clientWidth-200,
			       	loadComplete:function(){	
								...
								$("#editgrid").focus();
								///自动适应 工作空间
								 var reduceHeight=$("#navBar").height()+$("#navBar").height()+$("#operTitle").height()+$("#buttons").height()+$("#pagered").height()+tariffBarHeight+8;
								...
								*********************************/										
								...
								//yyl add初始化页面，若子表信息存在，全部加载子表
								if(relatetablename != ''){
									$("#ifrm").show();
									//通过表名获取menuid和menuname
									var urlstr='';
									...
									urlstr=tsd.buildParams({ 	  
															  ...
															  tsdoper:'query',//操作类型 
															  datatype:'xml',//返回数据格式 
															  tsdpk:'publicmode.querySubTabMenuid'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
									});	
									$.ajax({
										url:"mainServlet.html?"+urlstr+"&tablename=singletabedit s1 left join sys_menu s2 on s1.menuid = s2.imenuid &whereinfo= lower(s1.tablename) = lower('"+relatetablename+"')",
										datatype:'xml',
										cache:false,//如果值变化可能性比较大 一定要将缓存设成false
										timeout: 1000,
										async: false ,//同步方式
										success:function(xml){
											...
											imenuname = imenuname.substring(4,imenuname.indexOf("/}"));
											
											$("#ifrm").attr("src","mainServlet.html?pagename=pubMode/SingleTabE.jsp&imenuname="+imenuname+"&imenuid="+relatemenuid+"&subrelatefields=1=1");			
										}
									});
								}
								...
								//yyl add点击父类之后隐藏父类框
								if(subrelatefields != null && subrelatefields != 'null' && subrelatefields != ''&& subrelatefields != '1=1'){
									
									$(".HeaderButton",parent.document).click();
								}
								...
								if(relatetablename != ''){
									...
									//alert(relatefields);
									if(relatefields != ''){
										if(relatefields.indexOf("=") < 0 || relatefields.indexOf("<") < 0 || relatefields.indexOf(">") < 0 || relatefields.indexOf("<") < relatefields.indexOf("=") || relatefields.indexOf(">") < relatefields.indexOf("<") ){
											alert("关联字段格式错误");
											return;
										}
										var relateFields = relatefields.split(",");
										var relateFieldsLength = relateFields.length;
										for(var k = 0; k <relateFieldsLength; k++){
											if(k != 0){
												relateFieldsStr +=" and ";
											}
											
											//添加key值
											relateFieldsStr +=relateFields[k].substring(0,relateFields[k].indexOf("<"));
											
											var relateField = relateFields[k].substring(relateFields[k].indexOf("<")+1,relateFields[k].length-1)
											//添加value值
											var keylen2=keyGridTop.length;		
											for(var m=0;m<keylen2;m++){
												if(keyGridTop[m].toLowerCase().indexOf(relateField.toLowerCase())==3){
													relateFieldsStr +="@"+$("#editgrid").getCell(rowid,keyGridTop[m])+"@";
									...
									$.ajax({
										...
										success:function(xml){
											...
											imenuname = imenuname.substring(4,imenuname.indexOf("/}"));
											$("#ifrm").attr("src","mainServlet.html?pagename=pubMode/SingleTabE.jsp&imenuname="+imenuname+"&imenuid="+relatemenuid+"&subrelatefields="+relateFieldsStr);			
										}
									});
...
function initData(menuid){
	...
	$.ajax({
		url:"mainServlet.html?"+urlstr+"&sumfields=addtip,modtip&tablename=singletabedit&initialization=1=1&fusearchsql=menuid="+menuid,
		datatype:'xml',
		cache:false,//如果值变化可能性比较大 一定要将缓存设成false
		timeout: 1000,
		async: false ,//同步方式
...
function showRightBut(){
		...
		if(rights!=''){
			...
			for(var i = 0 ; i <rightsA.length;i++){
				...
				if(rightsA[i]=='add'){				//添加
					...
					$("#print1").show();
					$("#print2").show();
				}else if(rightsA[i]=='import'){		//外部话费导入
					$("#import1").show();
					$("#import2").show();
				}		
		...
		//mingliang Add去掉最后的"|"
		if("" != operstr && "|" == operstr.charAt(operstr.length-1)){
...
function saveObj(saves){
		//yyl add正则表达式验证	
		var regixCheckRet = regexCheck();
		if(regixCheckRet==false){return false;}
		//通过表单值构造数据操作参数
...
function regexCheck(){
	...
	for(var i=0;i<obj.rows.length;i++){
		if(obj.rows[i].regex != null && obj.rows[i].regex != ''){
			var fieldVal = $("#"+obj.rows[i].field_name).val();
			var regex = eval("/"+obj.rows[i].regex.replace(/#/gm,"$")+"/");
			if(fieldVal != ''){
				if(!regex.test(fieldVal)){
						alert(obj.rows[i].field_alias.substring(4,obj.rows[i].field_alias.indexOf("/}"))+"校验失败");
						return false;
					}
			}
		}
		...
		*/
		...
		if(deleteright=="true"){	
			...
			if(confirm(deleteinformation)){
					...
					//yyl add 
					var keyArray = keyGridRow.split(",");
					
					var errorCount = 0;
					if(deletechecksql != null && deletechecksql != ''){
					
						var deletechecksqlSub = deletechecksql.toLowerCase();
						for(var inx = 1; inx < arguments.length; inx++){
							deletechecksqlSub = deletechecksqlSub.replace(new RegExp("<"+keyArray[inx-1].toLowerCase()+">", "g"), "'"+arguments[inx]+"'");
						}
						var deletechecksqlArray = deletechecksqlSub.split(";");
						
						for(var inx = 0; inx < deletechecksqlArray.length; inx++){
							//yyl add查询是否有关联
							var urlstr='';
							var dsstr = 'tsdBilling';
							var tsdcfstr = 'oracle';
							urlstr=tsd.buildParams({ 	  
													  ...
													  tsdoper:'query',//操作类型 
													  datatype:'xml',//返回数据格式 
													  tsdpk:'publicmode.deleteCheckSql'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
							});	
							$.ajax({
								url:"mainServlet.html?"+urlstr+"&deletechecksql="+deletechecksqlArray[inx],
								datatype:'xml',
								cache:false,//如果值变化可能性比较大 一定要将缓存设成false
								timeout: 1000,
								async: false ,//同步方式
...
function openadd(){
		...
		if(undefined != $("#OPERATOR").val() && "" == $("#OPERATOR").val()){
		...
		//yyl add 若存在子表，新增子表时关联字段
		if(subrelatefields != null && subrelatefields != 'null' && subrelatefields!= '1=1'){
			var relateFields = subrelatefields.replace(/@/gm,"").split("and");
			for(var n = 0; n < relateFields.length; n++){
				var keyVal = relateFields[n].split("=");
				if(keyVal.length >= 2){
...
function openRowModify(){
		...
		*/						
		...
		if(editright=="true"){				
				...
				//mingliang Add修改时候将创建操作员设置为不可编辑
				if(undefined != $("#OPERATOR").val()){
					$("#OPERATOR").attr("disabled","disabled");
				}
				if(undefined != $("#operator").val()){
					$("#operator").attr("disabled","disabled");
...
function saveBuildParams(){
	...
	for(var i=0;i<obj.rows.length;i++){
		if(obj.rows[i].webselectable == 'F'){ continue;}	//隐藏的不拼接，不获取字段名获取值传递	----mingliang添加
...
function modifyBuildParams(type){
	...
	for(var i=0;i<obj.rows.length;i++){
				if(obj.rows[i].webselectable == 'F'){ continue;}	//隐藏的不在页面拼接，也不获取字段名获取参数----mingliang添加
...
function openDiaPrint(){
	...
	groupid=groupid.replace(new RegExp("~","g"),",");
	var userdept= $("#departname").val();
	printThisReport1(menuid,tablename,getPriParams(),userid,groupid,userdept,2);
}
...
function openwbhfImport(){	
	wbhfImport('tsdBilling','mssql',tablealiasName,languageType);	
}
...
function formatKey(){
		...
		*/
		...
		for(var i=0;i<obj.rows.length;i++){	
			...
			var dataDict = obj.rows[i].datadict;	//表的字段名
				if(null != dataDict && '' != dataDict && (-1 != dataDict.indexOf(":") || -1 != dataDict.indexOf(","))){
					continue;//不需要配置数据字典并且使键值对形式的
				}
				knum++;		
				var fieldname = obj.rows[i].field_name;	//表的字段名
...
function getValbyRel(val,field,type,prefixkey,suffixWkey,opentype){
	...
	for(var i=0; i<relfield.length;i++){
		...
		if(type=='1'){
				...
				//yyl add
				if($("#"+fieldnamestr).attr("onchange") == null){
					$("#"+fieldnamestr).attr("disabled","disabled");
				}
				//级联字段为多选
				...
				//级联字段为单选
					...
					//yyl modify var strarea = '<option value="" >'+selectinfo+'</option>';
					//$("#"+fieldnamestr).html(strarea+area);
					$("#"+fieldnamestr).html(area);
				}
...
function printTable(jsonid,formid,prefixkey,suffixWkey,suffixNkey,relatedInfokey,type){
	...
	for(var i=0;i<obj.rows.length;i++){
		...
		//alert(obj.rows[i].webselectable);
		if(obj.rows[i].keytype=='3'){//keyType == 3隐藏属性
				$tr_1 += '<input type="hidden" name="'+fieldnamestrP+'" id="'+fieldnamestrP+'" value=""/>';				
		}else if(obj.rows[i].keytype !='3' && obj.rows[i].webselectable == 'T'){	//	obj.rows[i].webselectable == 'T'   -->把不显示的字段不拼接在页面上		
				//是否是下拉选择
				...
				//text 输入框类型
					...
					//yyl modify 增加宽度
					var showwidth = obj.rows[i].showwidth;
					if(showwidth > 400){
						$tr_1 += '<td width="560" colspan="3" align="left" style="border-bottom:1px solid #A9C8D9;"><textarea  style="height:170px;width:100%;overflow:hidden" type="text" '+conditionstr+' name="'+fieldnamestrP+'" id="'+fieldnamestrP+'" class="textclass"></textarea></td>';
						j++;
					}else{
		...
		success:function(xml){
			...
			{	
				...
				$(xml).find('row').each(function(){
					...
					//alert(str);	
					//yyl add
					relatetablename = $(this).attr("relatemenuid");
					relatefields = $(this).attr("relatefields");
					deletechecksql=$(this).attr("deletechecksql");
