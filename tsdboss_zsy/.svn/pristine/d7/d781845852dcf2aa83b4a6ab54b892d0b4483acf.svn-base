...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
    	...
    	<title><fmt:message key="tariff.hujiao" /></title><!--定义呼叫类型-->
			...
			function getUserPower(){
				 var urlstr=tsd.buildParams({ 	  packgname:'service',
												  ...
												  method:'exequery',
							 					  ...
							 					  tsdpname:'CallType.getPower',//存储过程的映射名称
							 					  tsdExeType:'query',
							 					  datatype:'xmlattr'
						 					});
				/************************
				...
				**************/
				...
				if(spower!=''&&spower!='all@'){
						...
						editfields = getFields('call_type_num');
						
						editfields_son = getFields('call_type');
						flag = true;
				}
		...
		<script type="text/javascript">	
			 ...
			 /**
			 * 生成可用序号、清空序号下拉列表框
			 	可用序号：在添加面板中展示，从1-40之间，未在数据表中存在的，可以被使用
			 	已用序号：在查询面板中使用，从1-40之间，如果已经在数据表中存在的，不能再被使用		 	
			 * @param type：1：显示可用序号  ；2：显示已用序号 ；type为空或其它时：清空序号下拉列表信息
			 * @return 
			 */
			function getNumforTab(type){
				//情况序号下拉框
				$("#Num").empty();
				...
				if(type==1){								
						...
						for(i=1;i<=40;i++){							
								...
								$("#Num").html($("#Num").html()+area);							
						}
						
						//从数据库中取出该表中的所有序号，从页面的序号下拉列表框中去掉数据库中已存在的序号
						var urlstr=tsd.buildParams({  
									...
									tsdpk:'CallTypeNum.getAllNum',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
							...
							success:function(xml){
								$(xml).find('row').each(function(){
									var num = $(this).attr("num");
										$("#Num option[value='"+num+"']").remove();
								});
							}
						});
					
				}else if(type==2){ //显示已用序号					
						
						//从数据库中取出该表中的所有序号，显示在页面的序号下拉列表框中
						var urlstr=tsd.buildParams({  
									packgname:'service',//java包
									clsname:'ExecuteSql',//类名
									method:'exeSqlData',//方法名
			...
			/**
			 * 明细表中添加修改面板中的呼叫类型值改变触发事件，
			 	根据呼叫类型获取call_type_num表中其对应的num值
			 * @param calltype :呼叫类型
			 * @return 
			 */
			function getNum_son(calltype){
					var num ='';
					//从数据库中取出该呼叫类型对应的序号
					var urlstr=tsd.buildParams({  
								packgname:'service',//java包
								...
								tsdoper:'query',//操作类型 
								datatype:'xmlattr',//返回数据格式 
								tsdpk:'CallTypeNum.getNumbyCT',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
								tsdpkpagesql:''//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
		 			});
		 				
					$.ajax({
						url:'mainServlet.html?'+urlstr +"&Call_Type="+tsd.encodeURIComponent(calltype),
						datatype:'xml',
						cache:false,//如果值变化可能性比较大 一定要将缓存设成false
						timeout: 1000,
						async: false ,//同步方式
						...
						$.ajax({
							...
							success:function(xml){
									$(xml).find('row').each(function(){
										...
										$("#GD_Head").html($("#GD_Head").html()+area);
									});
							}
						});
				}
			 ...
			 */	
			 function getJbfzy(){
						var urlstr=buildParamsSqlByS('tsdCDR','query','xmlattr','CallTypeNum.getJbfzy','');						
						$.ajax({
							url:'mainServlet.html?'+urlstr,
							datatype:'xml',
							cache:false,//如果值变化可能性比较大 一定要将缓存设成false
			 ...
			 */
			 function getKemu(){
						var urlstr=buildParamsSqlByS('tsdBilling','query','xmlattr','CallTypeNum.getKemu','');
			 ...
			 * @return 
			 */
			 function getCall_Type_son(){
						var urlstr=buildParamsSqlByS('tsdCDR','query','xmlattr','CallType.getCall_Type','');						
						$.ajax({
							url:'mainServlet.html?'+urlstr,
							datatype:'xml',
							cache:false,//如果值变化可能性比较大 一定要将缓存设成false
							...
							success:function(xml){
									$(xml).find('row').each(function(){
		                   				var area="<option value='"+$(this).attr("Call_Type".toLowerCase())+"'>"+$(this).attr("Call_Type".toLowerCase())+"</option>";
										$("#Call_Type_son").html($("#Call_Type_son").html()+area);
									});
							}
						});
				}
			 ...
			 function getNetName_son(){
						...
						$.ajax({
							...
							success:function(xml){
									...
									$(xml).find('row').each(function(){	
										var NetName_son =$(this).attr("NetName".toLowerCase());
										if(NetName_son=='undefined'||NetName_son==undefined){										
											return;
										}
									    
		                   				var area="<option value='"+NetName_son+"'>"+NetName_son+"</option>";
										$("#NetName_son").html($("#NetName_son").html()+area);
									});
							}
						});
				}
				...
				function removeDisabled(){
					var fields = getFields("call_type_num");
				...
				/**
				 * 将定义呼叫类型明细弹出面板输入框设置为可用
				 * @param 
				 * @return 
				 */
				function removeDisabled_son(){
					var fields = getFields("call_type");
					var fieldarr = fields.split(",");					
					for(var j = 0 ; j <fieldarr.length-1;j++){
						$('#'+fieldarr[j]).removeAttr("disabled");
					}
	...
	*/
	jQuery(document).ready(function () {		
			...
			//初始化面板
			getGD_Head();//初始化明细话单归档表前缀下拉列表看
			getJbfzy();//初始基本话费转移到呼叫类型下拉列表看
			getKemu();//初始收费科目下拉列表看			
			getCall_Type_son();//初始化呼叫类型下拉列表看
			getNetName_son();//初始化计费网名下拉列表看
			
			
			getUserPower();//取得当前用户的权限
			//用户权限判定，初始化用户可操作权限
			...
			//获取主表数据表对应字段国际化信息	
			var thisdata = loadData('call_type_num','<%=languageType %>',1);				 					
			
			var col=[[],[]];
			col=getRb_Field('call_type_num','Num',"<fmt:message key='CallType.modifydeletedetail' />",'97','ziduansF1','Call_Type');//得到jqGrid要显示的字段
			var column = $("#ziduansF1").val();
			column=column.toLowerCase();
			column=column.replace(",gd_head,",",hdtype,");
			$("#ziduansF1").val(column);
			
			var Call_Typeg = thisdata.getFieldAliasByFieldName('Call_Type');
			var GD_Headg = thisdata.getFieldAliasByFieldName('GD_Head');
			var Jbfzyg = thisdata.getFieldAliasByFieldName('Jbfzy');
			var Kemug = thisdata.getFieldAliasByFieldName('Kemu');
			var Numg = thisdata.getFieldAliasByFieldName('Num');
			var Thscg = thisdata.getFieldAliasByFieldName('Thsc');
					
			
			/***给页面添加面板标签赋值**/			
			$("#Call_Typeg").html(Call_Typeg);
			$("#GD_Headg").html(GD_Headg);
			$("#Jbfzyg").html(Jbfzyg);
			$("#Kemug").html(Kemug);
			$("#Numg").html(Numg);
			$("#Thscg").html(Thscg);
			...
			var urlstr=tsd.buildParams({ packgname:'service',//java包
											  ...
											  tsdoper:'query',//操作类型 
											  datatype:'xml',//返回数据格式 
											  tsdpk:'CallTypeNum.query',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
											  tsdpkpagesql:'CallTypeNum.querypage'//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
											});		
			...
			jQuery("#editgrid").jqGrid({
				...
				*/   
				...
				colModel:col[1], 
				       	...
				       	caption:'<fmt:message key="tariff.hujiao" />', 
				        width:document.documentElement.clientWidth-60,
				       	loadComplete:function(){
										...
										addRowOperBtnimage('editgrid','openRowModify','Num','click',1,"style/images/public/ltubiao_01.gif",editinfo,"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");//修改
										addRowOperBtnimage('editgrid','deleteRow','Num','click',2,"style/images/public/ltubiao_02.gif",deleteinfo,"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;",'Call_Type');//删除
										addRowOperBtnimage('editgrid','openRowInfo','Num','click',3,'style/images/public/ltubiao_03.gif',"<fmt:message key='CallType.detail' />","&nbsp;&nbsp;&nbsp;");//详细
										...
										*依赖于addRowOperBtn(,,,,,)函数 顺序不可调换
										addRowOperBtn('editgrid',editinfo,'openRowModify','Num','click',1);
										addRowOperBtn('editgrid',deleteinfo,'deleteRow','Num','click',2,'Call_Type');
										*/
										...
										ondblClickRow: function(ids) {
											if(ids != null) {
												var Num =$("#editgrid").getCell(ids,"Num");
												openRowInfo(Num);
											}
										}
...
/**
 * 查看呼叫类型详细信息
 * @param key 每条信息查看表识 字段Num的值
 * @return 
 */
function openRowInfo(key){
		markTable(0);//显示红色*号		
	 	$(".top_03").html('<fmt:message key="CallType.detailmessage" />');//设置编辑框的标题		
		isDisabledN('call_type_num','',''); //将修改面板的输入框全部	置为不可用
		$("#gridinfo").val(1);
		openpan();//显示修改面板	
...
/**
 * 加载明细表信息 根据呼叫类型的信息显示该呼叫类型明细
 * @param ids 呼叫类型jqgrid行标识，
 * @return 
 */
function reLoadMingxi(ids){
		...
		if(ids != null) {			
							...
							var num_meet = $("#editgrid").getCell(ids,"Num");
							$("#num_meet").val(num_meet);
							...
							var urlstr_son=buildParamsSqlByS('tsdCDR','query','xml','CallType.query','CallType.querypage');
...
/**
 * 在grid中展示呼叫类型明细表信息
 * @param ids 显示呼叫类型jqgrid行标识，
 * @return 
 */
function readyMingxi(){
			//获取主表数据表对应字段国际化信息			
			var thisdata = loadData('call_type','<%=languageType %>',1);
			
			var col=[[],[]];
			col=getRb_Field('call_type','Bjzg',"<fmt:message key='CallType.modifydeletedetail' />",'97','ziduansF2','NetName');//得到jqGrid要显示的字段			
			...
			var urlstr_son=tsd.buildParams({ packgname:'service',//java包
											  ...
											  tsdoper:'query',//操作类型 
											  datatype:'xml',//返回数据格式 
											  tsdpk:'CallType.query',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
											  tsdpkpagesql:'CallType.querypage'//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
											});			
			jQuery("#editgrid_son").jqGrid({
				...
				*/   
				...
				colModel:col[1], 
				       	...
				       	//hidegrid: false, 
				       	sortorder: 'asc',//默认排序方式 
				       	caption:'<fmt:message key="tariff.hujiao" /><fmt:message key="tariff.sublist"/>', 
				        width:document.documentElement.clientWidth-40,
				       	loadComplete:function(){
										...
										addRowOperBtnimage('editgrid_son','openRowInfo_son','Bjzg','click',3,'style/images/public/ltubiao_03.gif',"<fmt:message key='CallType.detail' />","&nbsp;&nbsp;&nbsp;",'NetName');//详细
...
/**
 * 从呼叫类型，取出一条数据显示在addform面板
 * @param key 每条信息查看表示 字段Num的值
 * @return 
 */
function queryByID(key){ 
		...
		var urlstr=tsd.buildParams({  packgname:'service',//java包
									  ...
									  tsdoper:'query',//操作类型
									  datatype:'xmlattr',//返回数据格式 
									  tsdpk:'CallTypeNum.queryByKey'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
									});	
		$.ajax({
			url:'mainServlet.html?'+urlstr+'&Num='+key,
			datatype:'xml',
			cache:false,//如果值变化可能性比较大 一定要将缓存设成false
			timeout: 1000,
			async: false ,//同步方式
			success:function(xml){
				...
				$(xml).find('row').each(function(){
					...
					var GD_Head = $(this).attr("GD_Head".toLowerCase());
					$("#GD_Head").val(GD_Head);
					oldstr.push(GD_Head);
					...
					$("#Kemu").val(Kemu);	
					oldstr.push(Kemu);
					var Thsc = $(this).attr("Thsc".toLowerCase());
					$("#Thsc").val(Thsc);	
					oldstr.push(Thsc);
...
function queryByID_son(key,key1){ 
		...
		$("#Bjzg_son").val(key);
		$("#NetName_son_old").val(key1);
		$("#Bjzg_son_old").val(key);
		var urlstr=tsd.buildParams({  packgname:'service',//java包
									  clsname:'ExecuteSql',//类名
									  method:'exeSqlData',//方法名
									  ds:'tsdCDR',//数据源名称 对应的 WEB-INF/classes/ApplicationContent.xml中配置的数据源
									  ...
									  tsdoper:'query',//操作类型 
									  datatype:'xmlattr',//返回数据格式 
									  tsdpk:'CallType.queryByKey'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
									});		 					
		$.ajax({
...
function saveObj(saves){			
		...
		Call_Type=Call_Type.replace(/(^\s*)|(\s*$)/g,"");//去掉前后空格
		...
		var urlstr=tsd.buildParams({
			...
			tsdoper:'query',//操作类型
			datatype:'xmlattr',//返回数据格式
			tsdpk:'CallTypeNum.queryCallType'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
		});
			...
			success:function(xml){
				...
				$(xml).find('row').each(function(){
					...
					if(dbCall_Type!='undefined'&&dbCall_Type!=undefined){
						...
						if(dbCall_Type==Call_Type){									
							var strCall_Type = $("#Call_Typeg").html();
							alert(strCall_Type+"<fmt:message key='tariff.isExist'/>");
							$("#Call_Type").focus();
							flag = true;
						}
					}
				...
				if(flag==false){
						...
						var urlstr1 =tsd.buildParams({  
							...
							tsdoper:'exe',//操作类型
							datatype:'xml',//返回数据格式
							tsdpk:'CallTypeNum.save'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
						});
						urlstr1 +=params;
							...
							async: false ,//同步方式
								success:function(msg){
									if(msg=="true"){
										...
										//写入日志操作
										var str ="(call_type_num)<fmt:message key='global.add'/>。"+$("#Numg").html()+": "+$("#Num").val()+";"+$("#Call_Typeg").html()+": "+$("#Call_Type").val()+";"+$("#GD_Headg").html()+": "+$("#GD_Head").val()+";"+$("#Thscg").html()+": "+$("#Thsc").val()+";"+$("#Jbfzyg").html()+": "+$("#Jbfzy").val()+";"+$("#Kemug").html()+": "+$("#Kemu").val();
										...
										var area="<option value='"+Call_Type+"'>"+Call_Type+"</option>";
										$("#Call_Type_son").html($("#Call_Type_son").html()+area);
									}
								}
						});
...
var closeflash_son = false;//用于判断添加操场是保存新增，或保存退出。保存新增closeflash_son=true ；保存退出closeflash_son=false
/**
 * 对呼叫类型明细进行保存操作
 * @param saves 判断保存类型 1：保存新增 ；2保存退出
 * @return 
			...
			success:function(xml){
				...
				***************************/
					...
					if(params==false){return false;}
						...
						tsdoper:'exe',//操作类型
						datatype:'xml',//返回数据格式
						tsdpk:'CallType.save'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
						});
						urlstr+=params;
						//此处的url参数主要修改tsdpname（对应存储过程的名称）、tsdExeType(操作类型)
						$.ajax({
						...
						async: false ,//同步方式
							success:function(msg){
								if(msg=="true"){
										...
										//写入日志操作
										var str ="(call_type)<fmt:message key='tariff.sublist'/><fmt:message key='global.add'/>。"+$("#NetNameg_son").html()+": "+NetName+";"+$("#Bjzgg_son").html()+": "+Bjzg+";"+$("#Call_Typeg_son").html()+": "+$("#Call_Type_son").val();
										writeLogInfo("","add",tsd.encodeURIComponent(str));				
...
function modifyObj(){
			...
			**修改子表部分
			*******************/
			var CallTypeM = $("#CallTypeM").val();
			var Call_Type = $("#Call_Type").val();
			Call_Type=Call_Type.replace(/(^\s*)|(\s*$)/g,"");
			if(CallTypeM!=Call_Type){
				$("#meetid").val(Call_Type);
				
			 	var params_son = "&Call_Type="+tsd.encodeURIComponent(Call_Type);
			 	params_son +="&CallTypeM="+tsd.encodeURIComponent(CallTypeM);
			 	var urlstr_son=tsd.buildParams({
											  ...
											  tsdoper:'exe',//操作类型 
											  datatype:'xml',//返回数据格式 
											  tsdpk:'CallTypeNum.modifyson'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
											});
				urlstr_son+=params_son;
			 	$.ajax({
					url:'mainServlet.html?'+urlstr_son,
					cache:false,//如果值变化可能性比较大 一定要将缓存设成false
					timeout: 1000,
					async: false ,//同步方式
					success:function(msg){
						if(msg=="true"){
							...
							//写入日志
							var str ="(call_type)<fmt:message key='tariff.sublist'/><fmt:message key='global.edit'/>。"+$("#Call_Typeg_son").html()+": "+CallTypeM+">>>"+Call_Type;
		 ...
		 var urlstr=tsd.buildParams({     packgname:'service',//java包
										  ...
										  tsdoper:'exe',//操作类型 
										  datatype:'xml',//返回数据格式 
										  tsdpk:'CallTypeNum.modify'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
										});
		 	urlstr+=params;
		 	var	key=$("#Num").val();
			$.ajax({
				...
				success:function(msg){
					if(msg=="true"){
						...
						//写入日志
						var str ="(call_type_num)<fmt:message key='global.edit'/>。"+$("#Numg").html()+": "+$("#Num").val()+";"+$("#Call_Typeg").html()+": "+oldstr[0]+">>>"+$("#Call_Type").val()+";"+$("#GD_Headg").html()+": "+oldstr[1]+">>>"+$("#GD_Head").val()+";"+$("#Thscg").html()+": "+oldstr[2]+">>>"+$("#Thsc").val()+";"+$("#Jbfzyg").html()+": "+oldstr[3]+">>>"+$("#Jbfzy").val()+";"+$("#Kemug").html()+": "+oldstr[4]+">>>"+$("#Kemu").val();
						writeLogInfo("","modify",tsd.encodeURIComponent(str));							
						querylist(0);
						closeo();
						//fuheQuery();	
...
/**
 * 对呼叫类型明细进行修改 通过隐藏域modifyM 的值来判断调用单条修改，还是批量修改
 * @param
 * @return 
 */
function modifyObj_son(){

		var modifyM = $("#modifyM").val();//获取表示判断是进行单条记录修改操作，还是批量修改。
		if(modifyM=="row"){
			modifyObjRow_son();//对呼叫类型明细进行单条记录修改
		}
		else if(modifyM=="batch"){
			fuheModify_son();//对呼叫类型明细进行批量修改
		}
}
/**
 * 对呼叫类型明细单条记录修改
...
function modifyObjRow_son(){
			 ...
			 var urlstr=tsd.buildParams({     packgname:'service',//java包
										  ...
										  tsdoper:'exe',//操作类型 
										  datatype:'xml',//返回数据格式 
										  tsdpk:'CallType.modify'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
										});
		 	urlstr+=params;
		 	$.ajax({
				url:'mainServlet.html?'+urlstr,
				...
				success:function(msg){
					if(msg=="true"){
						...
						var oldstr = logoldstr.split(',');
						//写入日志
						var str ="(call_type)<fmt:message key='tariff.sublist'/><fmt:message key='global.edit'/>。";
							str +=$("#Bjzgg_son").html()+": "+$("#Bjzg_son_old").val()+">>>"+$("#Bjzg_son").val()+";";
							str +=$("#NetNameg_son").html()+": "+$("#NetName_son_old").val()+">>>"+$("#NetName_son").val()+";";
							str +=$("#Call_Typeg_son").html()+": "+oldstr[0]+">>>"+$("#Call_Type_son").val();
						//str = transfer(str);
						writeLogInfo("","modify",tsd.encodeURIComponent(str));	
...
/**
 * 呼叫类型jqgrid删除按钮操作，删除该条呼叫类型记录之前需把其明细表的信息先删除
 * @param key  Num值
 * @param key1 呼叫类型值
 * @return 
 */
			 	...
			 	jConfirm(deleteinformation,operationtips,function(x){
			 		 if(x==true){				 		 
						...
						var urlstr=tsd.buildParams({ packgname:'service',//java包
													  ...
													  tsdoper:'exe',//操作类型 
													  datatype:'xml',//返回数据格式 
													  tsdpk:'CallTypeNum.deleteson'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
													});
						var urlstr='mainServlet.html?'+urlstr+'&Call_Type='+tsd.encodeURIComponent(key1); 
						$.ajax({
							url:urlstr,
							cache:false,//如果值变化可能性比较大 一定要将缓存设成false
							timeout: 1000,
							...
							success:function(msg){
								if(msg=="true"){
									...
									//写入日志操作
									var str="(call_type)<fmt:message key='tariff.sublist'/><fmt:message key='tariff.deleteb'/>。"+"<fmt:message key='global.conditions'/>:"+$("#Call_Typeg").html()+": "+key1;
						...
						var urlstr=tsd.buildParams({ packgname:'service',//java包
													  ...
													  tsdoper:'exe',//操作类型 
													  datatype:'xml',//返回数据格式 
													  tsdpk:'CallTypeNum.delete'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
													});
						var urlstr='mainServlet.html?'+urlstr+'&Num='+key; 
						$.ajax({
							url:urlstr,
							cache:false,//如果值变化可能性比较大 一定要将缓存设成false
							timeout: 1000,
							...
							success:function(msg){
								if(msg=="true"){
									...
									//写入日志操作
									var str ="(call_type_num)<fmt:message key='global.delete'/>。"+$("#Numg").html()+": "+$("#Num").val()+";"+$("#Call_Typeg").html()+": "+$("#Call_Type").val()+";"+$("#GD_Headg").html()+": "+$("#GD_Head").val()+";"+$("#Thscg").html()+": "+$("#Thsc").val()+";"+$("#Jbfzyg").html()+": "+$("#Jbfzy").val()+";"+$("#Kemug").html()+": "+$("#Kemu").val();
									//str = transferApos(str);
			 	...
			 	jConfirm(deleteinformation,operationtips,function(x){
			 		 if(x==true){
						 ...
						 var urlstr=tsd.buildParams({  packgname:'service',//java包
													  ...
													  tsdoper:'exe',//操作类型 
													  datatype:'xml',//返回数据格式 
													  tsdpk:'CallType.delete'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
													});
						...
						$.ajax({
							...
							success:function(msg){
								if(msg=="true"){
									...
									//写入日志操作
									var str="(call_type)<fmt:message key='tariff.sublist'/><fmt:message key='global.delete'/>。"+$("#Bjzgg_son").html()+": "+$("#Bjzg_son").val()+";"+$("#NetNameg_son").html()+": "+$("#NetName_son").val()+";"+$("#Call_Typeg_son").html()+": "+$("#Call_Type_son").val();
									writeLogInfo("","delete",tsd.encodeURIComponent(str));				
			...
			if(key==1){
					...
					//此处的url参数主要修改tsdpname（对应存储过程的名称）、tsdExeType(操作类型)			 	
				 	var urlstr1=tsd.buildParams({  packgname:'service',//java包
												  ...
												  tsdoper:'query',//操作类型 
												  datatype:'xml',//返回数据格式 
												  tsdpk:'CallTypeNum.fuheQueryByWhere',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
												  tsdpkpagesql:'CallTypeNum.fuheQueryByWherepage'
												});
					var column = $("#ziduansF1").val();
				...
				//此处的url参数主要修改tsdpname（对应存储过程的名称）、tsdExeType(操作类型)			 	
			 	var urlstr1=tsd.buildParams({  packgname:'service',//java包
											  ...
											  tsdoper:'query',//操作类型 
											  datatype:'xml',//返回数据格式 
											  tsdpk:'CallType.fuheQueryByWhere',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
											  tsdpkpagesql:'CallType.fuheQueryByWherepage'
											});
				var column = $("#ziduansF2").val();	
				var link = urlstr1 + params;	
			...
			jConfirm(deleteinformation,operationtips,function(x){
					...
					params+="&Call_Type="+meetid;							    
				 	...
				 	var urlstr1=tsd.buildParams({  packgname:'service',//java包
												  ...
												  tsdoper:'exe',//操作类型 
												  datatype:'xml',//返回数据格式 
												  tsdpk:'CallType.fuheDeleteBy'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
				 	...
				 	$.ajax({
							...
							success:function(msg){
								if(msg=="true"){
									...
									setTimeout($.unblockUI, 15);
									//写入日志操作
									var str ="(call_type)<fmt:message key='tariff.sublist'/><fmt:message key='tariff.deleteb'/>。"+"<fmt:message key='global.conditions'/>:"+$("#fusearchsql").val();
									str = transferApos(str);
									writeLogInfo("","Bulk Delete",tsd.encodeURIComponent(str));		
									//fuheQuery_son();	
									querylist_son(0);
...
function fuheModify_son(){
			...
			params+="&Call_Type="+meetid; 
			 ...
			 var urlstr=tsd.buildParams({  packgname:'service',//java包
										  ...
										  tsdoper:'exe',//操作类型 
										  datatype:'xml',//返回数据格式 
										  tsdpk:'CallType.fuheModifyBy'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
										});
		 	urlstr+=params;
		 	alert(urlstr);
			$.ajax({
				...
				success:function(msg){
					if(msg=="true"){
						...
						//写入日志操作
						//$("#logoldstr").val();
						var str ="(call_type)<fmt:message key='tariff.sublist'/><fmt:message key='tariff.modifyb'/>。"+"<fmt:message key='global.newVal'/>:"+$("#logoldstr").val()+" <fmt:message key='global.conditions'/>:"+$("#fusearchsql").val();
						str = transferApos(str);
...
function zhOrderExe(sqlstr){
		...
		if(key==1){
		 	...
		 	var urlstr1=tsd.buildParams({  packgname:'service',//java包
										  ...
										  tsdoper:'query',//操作类型 
										  datatype:'xml',//返回数据格式 
										  tsdpk:'CallTypeNum.queryByOrder',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
										  tsdpkpagesql:'CallTypeNum.queryByOrderpage'
										});
			var column = $("#ziduansF1").val();
			var link = urlstr1 + params;	
			$("#editgrid").setGridParam({url:'mainServlet.html?'+link+'&column='+column}).trigger("reloadGrid");
		...
		if(key==2){
			...
			params+="&Call_Type="+meetid;  
		 	...
		 	var urlstr1=tsd.buildParams({  packgname:'service',//java包
										  ...
										  tsdoper:'query',//操作类型 
										  datatype:'xml',//返回数据格式 
										  tsdpk:'CallType.queryByOrder',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
										  tsdpkpagesql:'CallType.queryByOrderpage'
										});
			var link = urlstr1 + params;
			var column = $("#ziduansF2").val();		
			$("#editgrid_son").setGridParam({url:'mainServlet.html?'+link+'&column='+column}).trigger("reloadGrid");
...
/**
 * 打开简单查询面板  	
 * @param  	key 判断是对呼叫类型jqgrid或是呼叫类型明细jqgrid操作；
 			key=1：呼叫类型 ；key=2：呼叫类型明细
 * @return 
 */
function openQuery(key){ 	
		...
		switch(key){
			case 1:	
				...
				break;
			case 2:	
				removeDisabledN('call_type','','_son');
				//$("#gridinfo").val(2);	
				openpan();				
				$('#jdquery2').show();
				clearText('operformT2');
 ...
 * @param  key 判断是对呼叫类型jqgrid或是呼叫类型明细jqgrid操作；key=1：呼叫类型 ；key=2：呼叫类型明细
 * @return 
 */
function openadd(key){
 ...
 /**
 * 呼叫类型jqgrid上修改按钮操作 ，打开修改面板并加载将要修改的数据
 * @param  key num值
 * @return 
 */
function openRowModify(key){
 ...
 /**
 * 呼叫类型明细jqgrid上修改按钮操作 ，打开修改面板并加载将要修改的数据
 * @param  key 被叫字冠值
 * @param  key1 计费网名值 
 * @return 
 */
...
{	
 ...
 /**
 * 根据简单查询输入条件进行简单查询
 * @param  key 判断是对呼叫类型或是呼叫类型明细操作；key=1：呼叫类型 ；key=2：呼叫类型明细
 * @return 
 */	
function QbuildParams(key){
		 	if(key==1){ 
			 	var Call_Type = $("#Call_Type").val();  Call_Type=Call_Type.replace(/(^\s*)|(\s*$)/g,"");
			 	var GD_Head = $("#GD_Head").val();
			 	var Jbfzy = $("#Jbfzy").val();
			 	var Kemu = $("#Kemu").val();
			 	var Thsc = $("#Thsc").val();
			 	var numm = $("#Num").val();
		 		...
		 		var paramsStr='1=1 ';
		 		if(numm!=''){
			 		paramsStr+="and NUM = '"+numm+"' " ;
			 	}			 		
			 	if(Call_Type!=''){
			 	 	paramsStr+="and Call_Type like '%"+Call_Type+"%' ";
			 	}
			 	if(GD_Head!=''){
			 		paramsStr+="and GD_Head like '%"+GD_Head+"%' " ;
			 	}
			 	if(Jbfzy!=''){
		 	...
		 	if(key==2){
			 	...
			 	//Bjzg, Call_Type, NetName ,Num
			 	var Bjzg = $("#Bjzg_son").val(); Bjzg=Bjzg.replace(/(^\s*)|(\s*$)/g,""); //去掉两边空
 ...
 /**
 * 对呼叫类型添加、修改面板，通过表单值构造数据操作参数 请根据业务参数的多少和属性进行修改
...
function buildParams(){
			...
			tsd.QualifiedVal=true;				
		 	var params='';
		 	var Call_Type = $("#Call_Type").val();  Call_Type=Call_Type.replace(/(^\s*)|(\s*$)/g,"");//去掉两边空格
		 	var GD_Head = $("#GD_Head").val();
		 	var Jbfzy = $("#Jbfzy").val();
		 	var Kemu = $("#Kemu").val();
		 	var Num = $("#Num").val();
	 		...
	 		if(GD_Head!=null&&GD_Head!=""){
	 			params+="&GD_Head="+tsd.encodeURIComponent(GD_Head);
	 		}
		 	else{	
		 		//请输入明细话单归档表前缀!
	 			alert("<fmt:message key='tariff.wornGDHead'/>");
	 			$("#GD_Head").focus();
	 			return false;
	 		} 
	 		if(Thsc!=null&&Thsc!=""){
	 			params+="&Thsc="+tsd.encodeURIComponent(Thsc);
	 		}
	 		else{	
	 			//请输入通话时长!
	 			alert("<fmt:message key='tariff.wornThsc'/>");
	 			$("#Thsc").focus();
	 			return false;
	 		}	
 ...
 /**
 * 对呼叫类型明细添加、修改面板，通过表单值构造数据操作参数 请根据业务参数的多少和属性进行修改
 	@oper 操作类型 modify|save
 * @param 
 * @return 
 */
 function buildParams_son(){
			...
			tsd.QualifiedVal=true;				
		 	...
		 	//Bjzg, Call_Type, NetName ,Num
		 	var Bjzg = $("#Bjzg_son").val();
		 	Bjzg=Bjzg.replace(/(^\s*)|(\s*$)/g,"");//去掉两边空格
		 	var Call_Type = $("#Call_Type_son").val();
		 	var NetName = $("#NetName_son").val();
		 	var NetName_old = $("#NetName_son_old").val();
		 	var Bjzg_old = $("#Bjzg_son_old").val();
			 	
		 	//var Num = $("#Num_son").val();
		 	
		 	params+="&Bjzg="+tsd.encodeURIComponent(Bjzg);			 		
		 	params+="&NetName="+tsd.encodeURIComponent(NetName);
		 	params+="&Bjzg_old="+tsd.encodeURIComponent(Bjzg_old);			 		
		 	params+="&NetName_old="+tsd.encodeURIComponent(NetName_old);
		 	
		 	
	 		if(Call_Type!=null&&Call_Type!=""){
	 			params+="&Call_Type="+tsd.encodeURIComponent(Call_Type);	 
	 			
	 			var numget =getNum_son(Call_Type);
		 		params+="&Num="+numget;
...
function MbuildParams_son(){			
 ...
 /**
 * 高级查询参数获取
 	隐藏域gridinfo参数用于判断是对呼叫类型或是呼叫类型明细操作；gridinfo=1：呼叫类型 ；gridinfo=2：呼叫类型明细
 * @param 
 * @return 
 */
function fuheQbuildParams(){
			...
			if(modinfo==1){
		 	...
		 	var key = $("#gridinfo").val();
		 	if(key==1){
		 		var fusearchsql = encodeURIComponent($("#querysql").val());
...
<script type="text/javascript">
 /**
 * 关闭表格面板
 	隐藏域gridinfo判断是对呼叫类型或是呼叫类型明细操作；gridinfo=1：呼叫类型 ；gridinfo=2：呼叫类型明细
 * @param 
 * @return 
 */
function closeo(){
...
function openpan(){		
		var key= $("#gridinfo").val();
		if(key=='1'){				
				autoBlockFormAndSetWH('panTab1',60,5,'closeo1',"#ffffff",true,800,'');//弹出查询面板
				$("#jdquery1").hide();$("#readd1").hide();$("#save1").hide();$("#modify1").hide();$("#reset1").hide();$("#clearB1").hide();
		}else if(key=='2'){					
				autoBlockFormAndSetWH('panTab2',60,5,'closeo2',"#ffffff",true,800,'');//弹出查询面板		
 ...
 */
function reset2(){			
		var key =$("#Bjzg_son").val();
		var key1 =$("#NetName_son").val();		
		queryByID_son(key,key1);	
}	
</script>
<script type="text/javascript">
...
function openwinQ(key,key1)
{
	...
	if(key==1){
		...
		openDiaQueryG(key1,'call_type','2');
	}
 ...
 function openSaveModT(key)
{
	...
	$("#gridinfo").val(key);
	if(key==1){
		openModT('call_type_num','1');					
	}
	else if(key==2){
		openModT('call_type','2');
	}
 ...
 */
 function saveExoprt(){
	 	var key = $('#gridinfo').val();
	 	if(key==2||key=='2'){			 						
	 		getTheCheckedFields('tsdCDR','mssql','call_type');
	 	}
	 	else{
	 		getTheCheckedFields('tsdCDR','mssql','call_type_num');
 ...
 *标识当前操作的面板，点击页面导出数据按钮时操作
	 隐藏域gridinfo判断是对呼叫类型或是呼叫类型明细操作；gridinfo=1：呼叫类型 ；gridinfo=2：呼叫类型明细
 * @param key 判断是对呼叫类型或是呼叫类型明细操作；1：呼叫类型 ，2：呼叫类型明细
 * @return 
 */
  ...
  <body >   
	...
	<div id="zong" >
		...
		<div id="buttons">
			...
			<button type="button" id="openadd1" onclick="openadd(1);" disabled="disabled"><!--  disabled="disabled" -->
			...
			<button type="button" id="export1" onclick="thisDownLoad('tsdBilling','mssql','call_type_num','<%=languageType %>',6);openExport(1);" 
			disabled="disabled"> <!--  disabled="disabled" -->
				<!--导出--><fmt:message key="global.exportdata" />
			</button>	
			<!--打印				
			<button type="button" id="print1"  onclick="openExport(1);printThisReport1('<%=request.getParameter("imenuid")%>','call_type_num',getPriParams(),'<%=(String)session.getAttribute("userid")%>','<%=request.getParameter("groupid").replaceAll("~", ",")%>','<%=(String)session.getAttribute("departname")%>')"  disabled="disabled">
	...
	<!-- 用户操作标题以及放一些快捷按钮-->		
		<div id="buttons" style="display: none;">
			...
			<button type="button" id="export2" onclick="thisDownLoad('tsdBilling','mssql','call_type_num','<%=languageType %>',6);openExport(1);"  
			disabled="disabled"> <!--  disabled="disabled" -->
				<!--导出--><fmt:message key="global.exportdata"/>
			</button>		
			<!--打印		
			<button type="button" id="print2"  onclick="openExport(1);printThisReport1('<%=request.getParameter("imenuid")%>','call_type_num',getPriParams(),'<%=(String)session.getAttribute("userid")%>','<%=request.getParameter("groupid").replaceAll("~", ",")%>','<%=(String)session.getAttribute("departname")%>')" disabled="disabled">
				<fmt:message key="tariff.printer" />
			</button>
			-->	
		</div>	
	...
	<!-- 用户操作标题以及放一些快捷按钮-->		
		<div id="buttons">
			...
			<button type="button" id="export3" onclick="thisDownLoad('tsdBilling','mssql','call_type','<%=languageType %>',6);openExport(2);" 
			disabled="disabled"> 
				<!--导出--><fmt:message key="global.exportdata" />
			</button>	
			<!--打印			
			<button type="button" id="print3" onclick="openExport(2);printThisReport1('<%=request.getParameter("imenuid")%>','call_type',getPriParams(),'<%=(String)session.getAttribute("userid")%>','<%=request.getParameter("groupid").replaceAll("~", ",")%>','<%=(String)session.getAttribute("departname")%>')"  disabled="disabled">
				<fmt:message key="tariff.printer" />
			</button>
			-->	
	</div>	
	...
	<!-- 用户操作标题以及放一些快捷按钮-->		
		<div id="buttons" style="display: none;">
			...
			<button type="button" id="export4"  onclick="thisDownLoad('tsdBilling','mssql','call_type','<%=languageType %>',6);openExport(2);" 
			disabled="disabled"> 
				<!--导出--><fmt:message key="global.exportdata" />
			</button>
			<!--打印					
			<button type="button" id="print4" onclick="openExport(2);printThisReport1('<%=request.getParameter("imenuid")%>','call_type',getPriParams(),'<%=(String)session.getAttribute("userid")%>','<%=request.getParameter("groupid").replaceAll("~", ",")%>','<%=(String)session.getAttribute("departname")%>');"  disabled="disabled">
				<fmt:message key="tariff.printer" />
			</button>
			-->
	  </div>
...
<div id="modT" name='modT' style="display: none"  class="neirong">
	...
	<div class="top_01">
			<div class="top_01left">
				...
				<div class="top_03" ><fmt:message key="CallType.functionname" /></div>
	...
	<div class="top02right"><img src="style/images/public/toptiaoright.gif" /></div>	
		...
		<input type="hidden" ></input>
			<table width="100%" id="tdiv" border="0" cellspacing="0" cellpadding="0" style="line-height:33px; font-size:12px;">
			  ...
			  <tr>
			   <!--  num -->	 
			  	<td width="25%" align="right"  class="labelclass"><label id="Numg" ></label></td>
			    <td width="25%" align="left" style="border-bottom:1px solid #A9C8D9;">					
					<select name="Num" id="Num" class="textclass" >									
					</select>
					<label class="addred" ></label>
				</td>	  
					
			  <!--  呼叫类型 -->	   
			    <td width="25%" align="right" class="labelclass"><label id="Call_Typeg" ></label></td>							
			    <td width="25%" align="left" style="border-bottom:1px solid #A9C8D9;">
			    	<input type="text" name="Call_Type" id="Call_Type" maxlength="16" onpropertychange="TextUtil.NotMax(this)" class="textclass"></input>						
					<label class="addred" ></label></td>
		
			</tr>
			<tr>
				 <!-- 通话时长 -->
			    <td width="25%" align="right"  class="labelclass"><label  id="Thscg"></label></td>
			    <td width="25%" align="left" style="border-bottom:1px solid #A9C8D9;">
			    	<input type="text" name="Thsc" id="Thsc" onfocus="WdatePicker({startDate:'00:00:00',dateFmt:'HH:mm:ss',alwaysUseStartDate:true});" class="textclass"></input>
			    	<label class="addred" ></label></td>	
			    	
			    <!-- 明细话单归档表前缀 -->	
			    <td width="25%" align="right"  class="labelclass"><label id="GD_Headg"></label></td>
			    <td width="25%" align="left" style="border-bottom:1px solid #A9C8D9;">
				  	<select name="GD_Head" id="GD_Head" class="textclass">
						<option value="" ><fmt:message key="global.select"/></option>					
					</select><label class="addred" ></label></td>	
			</tr>
			<tr>
			  <!-- 收费科目 -->   
			    <td width="25%" align="right" class="labelclass"><label id="Kemug" ></label></td>							
			    <td width="25%" align="left" style="border-bottom:1px solid #A9C8D9;">
			 ...
			 <!-- 基本话费转移到呼叫类型 -->
			    <td width="25%" align="right"  class="labelclass" style="line-height:20px;"><label id="Jbfzyg"></label></td>
			    <td width="25%" align="left" style="border-bottom:1px solid #A9C8D9;">
			    	<select name="Jbfzy" id="Jbfzy" class="textclass">
						<option value="" ><fmt:message key="global.select"/></option>						
					</select></td>			  
			</tr>			
	    ...
	    <!-- 取消     --><button class="tsdbutton" id="reset1" style="width:63px;heigth:27px;" onclick="reset1();" >取消</button>
...
<div class="neirong" id="panTab2" style="display: none">
	...
	<div class="top02right"><img src="style/images/public/toptiaoright.gif" /></div>	
			...
			<table width="100%" id="tdiv" border="0" cellspacing="0" cellpadding="0" style="line-height:33px; font-size:12px;">							
			  ...
			  <!-- 被叫字冠 -->		   
			   ...
			   <!--  计费网名 -->
			    ...
			    <td width="22%" align="left" style="border-bottom:1px solid #A9C8D9;">
			    	<select name="NetName_son" id="NetName_son" class="textclass">
						<option value="" ><fmt:message key="global.select"/></option>						
					</select>
					<label class="addred" ></label>
				<input type="hidden" id="NetName_son_old" />
				<input type="hidden" id="Bjzg_son_old" />
				</td>	
			    ...
			    <td width="20%" align="left" style="border-bottom:1px solid #A9C8D9;">
				  	<select name="Call_Type_son" id="Call_Type_son" class="textclass" >
						<option value="" ><fmt:message key="global.select"/></option>						
					</select>
				  	</td>				  
			</tr>			
	    ...
	    <!-- 取消     --><button class="tsdbutton" id="reset2" style="width:63px;heigth:27px;" onclick="reset2();" ><fmt:message key="CallType.cancel" /></button>
			...
			<div style="display: none">		
				   ...
				   <input type="hidden" id="useridd" value="<%=userid %>"/>							
					...
					<form name="childform" id="childform">
				 	...
				 	<!-- 标识目前正在处理的是那个表 -->
				  	<input type="hidden" id="gridinfo" name="gridinfo"/>
				  	<!-- 标识面板查询的表 -->
			...
			<div class="neirong" id="thefieldsform" style="display: none;width:800px">
				<div class="top">
					<div class="top_01" id="thisdrag">
							<div class="top_01left">
								<div class="top_02"><img src="style/images/public/neibut01.gif" /></div>
								<div class="top_03" id="titlediv"><fmt:message key="CallType.checkexportdata" /></div>
								<div class="top_04"><img src="style/images/public/neibut03.gif" /></div>
							</div>
							<div class="top_01right"><a href="#" onclick="javascript:$('#closethisinfo').click();"><fmt:message key="CallType.close" /></a></div>
					</div>
					<div class="top02right"><img src="style/images/public/toptiaoright.gif" /></div>	
				</div>
				<div class="midd" >
				...
				<div class="midd_butt">
					<button type="submit" class="tsdbutton" id="query" onclick="checkedAllFields()">
						<fmt:message key="CallType.chooseall" />
					</button>
