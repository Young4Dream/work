	...
	<title>话费查询</title>
		...
		$(function(){
			...
			jQuery("#editgrid").jqGrid({  	
				...
				datatype: "xml",
			   	colNames:['汇总月份','合同号','用户名称','电话','电话数量','话费指标','月租合计','话费合计','累计金额','余额累计'],
			   	colModel:[
			   		{name:'hzyf',index:'hzyf', width:60},
			   		{name:'hth',index:'hth', width:60},
			   		{name:'yhmc',index:'yhmc', width:120},
			   		{name:'dh',index:'dh', width:90},
			   		{name:'dhsl',index:'dhsl', width:80},
			   		{name:'hfzb',index:'hfzb', width:80},					
			   		{name:'zfs',index:'zfs', width:90},
			   		{name:'hfhj',index:'hfhj', width:90},
			   		{name:'leji',index:'leji', width:90},
			   		{name:'noleiji',index:'noleiji', width:90}
			   	],
			   	rowNum:15,
			   	rowList:[15,30,128],
		...
		function queryInfo(){
			...
			var flag=fetchMultiPValue('hfquery.Hftj_Hfzb',6,'&hzyf='+hzyf+"&isflag="+$("#isflag").val());
			if(flag=="true"){
		...
		function initGrid_Detail(yf){
			...
			///设置命令参数
			 var urlstr=tsd.buildParams({ 	packgname:'service',//java包
				...
				tsdoper:'query',//操作类型 
				datatype:'xml',//返回数据格式 
				tsdpk:'hfQuery.list',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
				tsdpkpagesql:'hfQuery.listpage'//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
			});
  ...
  <body>
	...
	<table id="queryparam">
		<tr>
			<td width="25%">查询月份</td>
			<td>
				<input type="text" id="hzyf" onfocus="WdatePicker({startDate:'%y-%M',dateFmt:'yyyyMM',alwaysUseStartDate:true})" readonly="readonly"  style="width: 150px;margin-top: 5px;margin-bottom: 5px;height: 20px;margin-left: 15px;"/>
			</td>
		</tr>
		<tr>
			<td width="25%">负数累计</td>
	...
	<div id="infolist" style="display: none;">
		...
		<div id="buttons">
			...
			<button type="button" id='gjquery1' onclick="openDiaQueryG('query','jfgl_hthhftj');">
				<!--高级查询-->
