...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	var tables = ['callpay_shedule_base','callpay_shedule_owemonth','callpay_shedule_interval','callpay_shedule_ringinterval','callpay_shedule_callcount','callpay_shedule_telnoday','callpay_shedule_enabletime','callpay_cjys'];
	var pkeys = ["Rate1Feilu","Rate1Jcfeilu","Rate1Fljm","Rate1Charge_Type"];
	var fuheM = false;
	
	var firstLoad = [true,true,true,true,true,true,true,true];
	var mNames = [
				'催缴基本策略','催缴策略_欠费月数','催缴策略_催缴间隔','催缴策略_外拨间隔','催缴策略_催缴次数','催缴策略_分段催缴','催缴策略_催缴时间','催缴策略_催缴月份'
				];
	//用于数据格式的验证
	var regularExp = [];
	regularExp["integer"] = /^[-]{0,1}\d+$/;
	regularExp["double"] = /^[-]{0,1}\d+|[-]{0,1}\d+\.\d+$/;
	//修改数据时保存原记录
		...
		$("#editgrid").jqGrid({
				...
				width:document.documentElement.clientWidth-60,
		       	loadComplete:function(){
					...
					{height:300,reloadAfterSubmit:true,closeAfterEdit:true}, // edit options 
					{height:300,reloadAfterSubmit:true,closeAfterAdd:true}, // add options 
	...
	function InitialDropdownList()
	{
		...
		//模块局	2012-10-12 cyl start
		
		var mokuaiju = fetchMultiValue("CallPay.queryMO",6,"");
		var mkhtml = "";
		
		if(mokuaiju!=undefined && mokuaiju[0]!=undefined)
		{
			for(var i=0; i<mokuaiju.length; i++)
			{
				mkhtml = mkhtml + "<option value=\"" + mokuaiju[i] + "\">" + mokuaiju[i] + "</option>";
			}
		}
		
		$("#Ctrl6_MoKuaiJu").html(mkhtml);
		//$("#Ctrl6_Sel_MoKuaiJu").html("<option value=\"\">请选择</option>" + kemuhtml);
		//cyl end
	...
	function disablePrimary()
	{
		...
		//中石油项目添加催缴月份选项卡 20160107 add by zxy
		if (tabStatus == 8){
			$("#cjyfxuhao").attr("disabled","disabled");
	...
	function openRowModify()
	{
		...
		else
		{
			...
			toggleBtn(0);
			disablePrimary();
			//中石油项目催缴月份选项卡  20160107 add by zxy
			if (tabStatus == 8){
			$("#cjyfxuhao").val(arguments[0]);
			}
			//生成参数串
	...
	/**
	*保存催缴月份 20160116 add by zxy
	*/
    function addmonth()
	{
	  var xuhao = $("#cjyfxuhao").val();
	  var months = $("#callpaymonths").val();
	  
	   //拼接主键参数，用于验证数据是否已存在
		var queryStr = "&Xuhao=";
		tsd.QualifiedVal=true;
		queryStr += tsd.encodeURIComponent(xuhao);
		if(tsd.Qualified()==false){$("cjyfxuhao").select();$("#cjyfxuhao").focus();return false;}
		queryStr += "&tablename=callpay_cjys";
		
		var res = fetchSingleValue("CallPay.existed",6,queryStr);
		if(parseInt(res)>0)
		{
			alert("序号"+xuhao+" 已存在,请重新选择.");
			$("cjyfxuhao").select();
			$("#cjyfxuhao").focus();
			return false;
		}
		...
		var queryStr = "&callpaymonths=";
		tsd.QualifiedVal=true;
		queryStr += tsd.encodeURIComponent(months);
		if(tsd.Qualified()==false){$("callpaymonths").select();$("#callpaymonths").focus();return false;}
		queryStr += "&tablename=callpay_cjys";
		
		var res = fetchSingleValue("CallPay.monthexisted",6,queryStr);
		if(parseInt(res)>0)
		{
			alert("催缴月份"+months+" 已存在,请重新选择.");
			$("callpaymonths").select();
			$("#callpaymonths").focus();
			return false;
		}
		//清空queryStr;
		...
		tsd.QualifiedVal=true;
		queryStr += tsd.encodeURIComponent(xuhao);
		queryStr += "&callpaymonths=";
		queryStr += tsd.encodeURIComponent(months);
		
		
		var res = executeNoQuery("CallPay.add8",6,queryStr);
	...
	//中石油催缴月份选项卡 20160107 add by zxy	
	function addmonthmodifydata(){
		
		var xuhao = $("#cjyfxuhao").val();
		var callpaymonths = $("#callpaymonths").val();
		
		var queryStr = "&callpaymonths=";
		tsd.QualifiedVal=true;
		queryStr += tsd.encodeURIComponent(callpaymonths);
		if(tsd.Qualified()==false){$("callpaymonths").select();$("#callpaymonths").focus();return false;}
		queryStr += "&tablename=callpay_cjys";
		...
		if(parseInt(res)>0)
		{
			alert("催缴月份"+callpaymonths+" 已存在,请重新选择.");
			$("callpaymonths").select();
			$("#callpaymonths").focus();
			return false;
		}		
		...
		var querystr = "";
		querystr += "&callpaymonths="+callpaymonths;
		querystr += "&Xuhao="+xuhao;
	...
	function initCallSwitch(){
		$("#gridd").empty();
		$("#gridd").append('<table id="editgrid" class="scroll" cellpadding="0" cellspacing="0"></table>');
		$("#gridd").append('<div id="pagered" class="scroll" style="text-align: left;"></div>');
		
		var res = fetchSingleValue('dbsql_queryCallSwitch',6,'');
		if(res=='T'){
	...
	function changeSwitch(){
		var res = fetchSingleValue('dbsql_queryCallSwitch',6,'');
		var tsd_checked = $(":radio:checked").val();
		if(res == tsd_checked){
			return false;
		}
		if(tsd_checked == 'F'){		
			if(confirm("是否将催缴功能关闭?")){
				executeNoQuery("dbsql_updateCallSet",6,'&tvalues=F');
		 		initCallSwitch();
		 		alert("修改成功");
			}else{
				initCallSwitch();
			...
			if(confirm("是否将催缴功能打开?")){
		 		executeNoQuery("dbsql_updateCallSet",6,'&tvalues=T');
		 		initCallSwitch();
		 		alert("修改成功");
			}else{
				initCallSwitch();
...
<%-- 		<button type="button" id="export1" onclick="DownFile()"> 
	...
	<div id="tabs">	
		<ul>
			<li><a href="#yangshi" onclick="initCallSwitch()"><span>催缴启动配置</span></a></li>
			...
			<li><a href="#gridd" onclick="tabsChg(8)"><span>催缴策略_催缴月份</span></a></li>
		</ul>
		<div id="yangshi">
			<table >
				<tr>
					<td>
						<div style="float:left;line-height:23px;">催缴系统开关：</div>
						<div id="eitherport" style="float:left;margin-left:6px;">
							<input id='T' type='radio' name='callSwitch' value="T" onclick="changeSwitch();"/>开
							<input id='F' type='radio' name='callSwitch' value="F" onclick="changeSwitch();"/>关		
					   </div>
				   </td>
			  </tr>
			<tr>
				<td style="color:red;">说明：单击催缴开关单选按钮，设置开关状态。</td>
			</tr>
			</table>
		</div>
 		<div id="gridd">
...
<%-- 	<div id="buttons">
	...
	<!-- Number 8 -->
	<form class="neirong" id="cl_from8" style="display:none;width:720px;">
		<input type="hidden" />
		<div class="top">
			<div class="top_01" id="thisdrag">
				...
				<div class="top_01right">
					<a href="javascript:;" onclick="javascript:$('#cl_from8Close').click();">关闭</a>
				</div>
			</div>
		...
		<div class="midd" style="background-color:#FFFFFF;text-align:left;overflow-x:hidden;">
			<table style="width:100%">
				<tr>
					<td class="labelclass" width="16%"><span>序号</span></td>
					<td width="34%" class="tdstyle">
						<select id="cjyfxuhao" class="textclass" >
					...
					<td class="labelclass" width="16%"><span>催缴月份</span></td>
					<td width="34%" class="tdstyle">
						<select id="callpaymonths" class="textclass" >
		...
		<div class="midd_butt">
			<button type="button" class="tsdbutton" id="cl_from8save" onclick="addmonth()">
				保存添加
			</button>
			<button type="button" class="tsdbutton" id="cl_from8exit" onclick="addmonthforexit()">
				保存退出
			</button>
			<button type="button" class="tsdbutton" id="cl_from8modify" onclick="addmonthmodifydata()">
				修改
			</button>			
			<button type="button" class="tsdbutton" id="cl_from8Close">
				关闭
			</button>
		</div>
	</form>	
	...
	<form class="neirong" id="cl_from6" style="display:none;width:720px;">
		...
		<div class="midd" style="background-color:#FFFFFF;text-align:left;overflow-x:hidden;">
			<table style="width:100%">
				...
				<tr>					
					<!-- 2012-10-12 cyl start -->
					<td class="labelclass" width="16%">
						<label for="Ctrl6_MoKuaiJu"><!-- 模块局 -->
							<span id="Ctrl6_Lbl_MoKuaiJu"></span>
						</label>
					</td>
					<td width="34%" class="tdstyle">
						<select id="Ctrl6_MoKuaiJu"  class="textclass" >   
						</select>
					</td>
					<!-- cyl end -->
				</tr>
		...
		<div class="midd" style="background-color:#FFFFFF;text-align:left;overflow-x:hidden;">
			<table style="width:100%">
				<tr>
					...
					<!--  
					<td class="labelclass" width="16%">
						<label for="Ctrl6_Sel_MoKuaiJu">
							<span id="Ctrl6_Sel_Lbl_MoKuaiJu"></span>
						</label>
					</td>
					<td width="34%" class="tdstyle">
						<select name="Ctrl6_Sel_MoKuaiJu" id="Ctrl6_Sel_MoKuaiJu" class="textclass" ></select>
					</td>
					-->
				</tr>
				<tr>
