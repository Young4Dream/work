	...
	<script type="text/javascript" language="javascript">
		...
		********************************************************/ 
		function printThisReport1(menuid,tablename,param,userid,groupid,userdept,reportfalg){
				//存放报表名称								
        		var ReportName=tablename;//getReportName(menuid,tablename);
        		...
        		if(ReportName == undefined||ReportName == 'undefined'||ReportName == ''){
        			...
        			if(reportfalg == undefined||reportfalg == 'undefined'||reportfalg == ''){
        				...
        				//进行打印操作		        				
        				basepath = basepath + 'ReportServer?reportlet=/com/tsdreport/'+ReportName+'.cpt'+param+'&userid='+userid+'&groupid='+groupid+'&userdept='+userdept+"&______" + new Date();
        				//window.open(basepath);
        				window.showModalDialog(basepath,window,"dialogWidth:800px; dialogHeight:530px; center:yes; scroll:no");
        			}
		...
		*/
		...
		function initOptions(){
			...
			if(isAutoCalculate != undefined){
				if(isAutoCalculate == 'Y'){
					$('#ghDJLbl,#ghDJ,#ghYzLbl,#ghYz,#ghycjeLbl,#ghycje').show();
					$('#isAutoCalculate').val(isAutoCalculate);
				}else{
					$('#ghDJLbl,#ghDJ,#ghYzLbl,#ghYz,#ghycjeLbl,#ghycje').hide();
				}
			}
		}
...
parameters:       flag: 1:直接打印 2:不打印 3：预览打印 
...
function ghPrint(flag)
{
	...
	*/
	...
	//alert(reportfile+'_'+rptArr.length)
	if(rptArr.length>1){
		/**
		...
		*/
		...
		printRep(reportfile,"&lsz=" + sGhlsz.replace(/\'/g,''),flag);
	}
}
		//加载复选费用列表
   ...
   if(/.*[\u4e00-\u9fa5]+.*$/.test(txtval)){ 
     ...
     if((key < 48||key > 57) && key != 46 ){
       ...
       if(key == 46){
		...
		//2016-01-26增加绑定宽带的宽带到期时间提示
		function kdEndDate(dh){
			var res = fetchSingleValue("charge_phone.kdEndDate",6,"&DH="+dh);
			if(res != '' && res !='undefined' && res != undefined){
				$(".kdenddate").html(" ,宽带到期时间："+res);
			}else{
				$(".kdenddate").html("");
			}
		}
		
		//20151230 add by zxy 中石油项目营收查询添加根据用户名称和地址进行模糊查询
		function ghSearchby(){
			
			var idx = parseInt($("#ghSearchWay").val(),10);
			if (idx == 1 )//如果是电话号码或合同号查询直接调用原查询方法
			{
				btnFindClick('','','normal');
				kdEndDate($.trim($("#ghSearchBox").val()));
			}
			else //如果是用户名称或地址模糊查询则通过弹出查询div选择后查询 
			{
				...
				{
					alert("请输入要查询的用户名称或地址");
					$("#ghSearchBox").select();
					$("#ghSearchBox").focus();
					return false;
				}
				
				var temp = fetchSingleValue("Revenue.querybyC",6,"&UN="+param);
				if(temp=="0")
				{
		...
		function kdMultiUser(UN)
		{
			...
			var urlstr = tsd.buildParams({packgname:'service',//java包
								 					...
								 					tsdoper:'query',//操作类型 
								 					datatype:'xml',
								 					tsdpk:'Revenue.queryby',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
								 					tsdpkpagesql:'Revenue.querybyC',
								 					tsdUserID:'ture'
								 				});
		...
		<div id="navBar">&nbsp;&nbsp; 
     	...
     	<div id="gh" align="left" >
				...
				<table style="margin-top: 5px">
					<tr valign="middle">
						<td>
							...
							查询方式
							<select id="ghSearchWay" style="width: 120px;" onchange="ghClearInfo()">
							<option value="1">电话号码</option>
							<option value="2">用户名或地址</option>
							</select>&nbsp;&nbsp;
						</td>
						<td>
						...
						<td>
							&nbsp;
							<button class="tsdbutton" id="ghMultiHth" onclick="ghMultiHthJfPay('multi')" style="margin-bottom: 0px;padding:2px;display: none;">
								多合同号
							</button>
						</td>
						<!-- 
			...
			<div id="ghFP">
				...
				<div id="ghYHYE" style="width: 100%;" tip1="<fmt:message key="charge_phone.RealTimeBalance"/>" tip2="<fmt:message key="charge_phone.InTheCurrent"/>"><!-- 当前月费用：  元 -->
					...
					<br />
					<hr width="100%" style="height:1px;border:none;border-top:1px dashed #0066CC;" />
				</div>
			</div>
			...
			<div id="ghStatus">
				<div id="ghStatustb" style="width: 100%;float: left">
					...
					<span id="ghDJLbl" style="display: none">递交金额：</span>
					...
					<br />
					<!-- <span id="ghycjeLbl" style="display: none">预存金额：</span>
					<input type="text" id="ghycje" name="ghycje" readonly="readonly" style="width: 120px;font-weight:bold;color:#000;display: none" />
					 -->
					<span id="ghYzLbl" style="display: none">应找金额：</span>
				...
				<table  width="640px" cellpadding="5px" cellspacing="2px" style="display: none;" >
					<tr>
						<td colspan="8" align="center">
							<span class="spanstylept" style="margin-left: 70px;">
								<input type="checkbox" id="ghPayNotPrint" onclick="$('#ghSs').select().focus()" />
								打印
							</span>
							<span class="spanstylept" style="margin-left: 70px;">
								<input type="radio" id="prePayReport" name="ghPayReport" value="prepay" onclick="$('#ghSs').select().focus()"/>
								分月打印
							</span>
							<span class="spanstylept" style="margin-left: 70px;">
								<input type="radio" id="comPayReport" name="ghPayReport" value="combined" onclick="$('#ghSs').select().focus()"/>
								合并打印
							</span>
			...
			<div id="ghButtons" style="text-align: center;">
				...
				<div>
					<button class="tsdbtn"  id="ghUnpaymentPrint" style="display:none;width: 100px;display: none;"  onclick="ghOpenUnpaymentDiv();"
						 disabled="disabled">
						自动冲账打印<!-- 未收费打印 -->
					</button>
					...
					<button class="tsdbtn" id="ghsave" style="width: 80px;" disabled="disabled" onclick="onSave();">
						<fmt:message key="charge_phone.pay"/><!-- 缴费 -->
					</button>
					<span style="visibility: hidden;">__</span>
