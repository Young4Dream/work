	...
	function ghSearch(){
		...
		else
		{
			...
			{	
				...
				var tempkd="";
				if(keys[idx]=="Dh"){
					tempkd = fetchSingleValue("Revenue.querykdC",6,"&p="+keys[idx]+"&UN="+param);
					temp = fetchSingleValue("Revenue.queryFuzzyC",6,"&p="+keys[idx]+"&UN="+param);
					if(temp=="0"&&tempkd=="0")
					{
						...
						$("#ghSearchBox").select();
						$("#ghSearchBox").focus();
					}else if(temp=="0"&&tempkd!="0"){
						kdMultiUserkdxc(keys[idx],param);
					}else
					    {
...
/////////////////////////查询宽带相关信息////////////////////
	function kdMultiUserkdxc(flag,UN)
	{
		$("#grid").empty();
		...
		var urlstr = tsd.buildParams({packgname:'service',//java包
							 					...
							 					tsdoper:'query',//操作类型 
							 					datatype:'xml',
							 					tsdpk:'Revenue.querykd',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
							 					tsdpkpagesql:'Revenue.querykdC',
							 					tsdUserID:'ture'
							 				});
		...
		$("#bu_muser").jqGrid({
				...
				colModel:[{name:'viewOperation',index:'viewOperation',width:70},
				{name:'Yhmc',index:'Yhmc',width:100},
				{name:'Dh',index:'Dh',width:80},
				{name:'Hth',index:'Hth',width:80},
				{name:'Yhdz',index:'Yhdz',width:330}],
		        ...
		        ondblClickRow:function(idx){
		        	userChoose($("#bu_muser").getCell(idx,"Yhmc")==undefined?"":$("#bu_muser").getCell(idx,"Yhmc"),$("#bu_muser").getCell(idx,"Dh"),$("#bu_muser").getCell(idx,"Yhdz")==undefined?"":$("#bu_muser").getCell(idx,"Yhdz"),$("#bu_muser").getCell(idx,"Hth"));
		        	$("#YwArea_yhdang").removeAttr("disabled");	
		        }
		...
		$.ajax({
			...
			success:function(xml){
				$(xml).find("row").each(function(){									
					...
					$("#dhlx_yhdang").val($(this).attr("dhlx")==undefined?"":$(this).attr("dhlx"));
					//用户地址
					...
					$("#tfjstatus_zh").val($(this).attr("tjbz")==undefined?"":getdatevalue("&cloum=showname&tablename=hwjbdef&key=hwjb='"+tsd.encodeURIComponent($(this).attr("tjbz"))+"'"));					
					$("#Tjbz_yhdang").val($(this).attr("tjbz")==undefined?"":getdatevalue("&cloum=showname&tablename=hwjbdef&key=hwjb='"+tsd.encodeURIComponent($(this).attr("tjbz"))+"'"));						
					//调级策略
					...
					*/
					$("#CustType_yhdang").val($(this).attr("custtype")==undefined?"":getdatevalue("&cloum=custtype&tablename=custtype&key=custid='2'" ));//客户类型
					...
					$("#Bz2_yhdang").val($(this).attr("bz2")==undefined?"":$(this).attr("bz2"));
					//并机地址
		...
		var params = "&Hth=" + Hth;
			...
			success:function(xml){
				$(xml).find("row").each(function(){
					...
					$("#ZnjBz").val($(this).attr("znjbz")==undefined?"":getdatevalue("&cloum=bz&tablename=znjbz&key=znjbz='0'"));//$(this).attr("znjbz")));
					//补贴类别
					...
					$("#Bz3").val($(this).attr("bz3")==undefined?"":$(this).attr("bz3"));
					$("#Bz4").val($(this).attr("bz4")==undefined?"":$(this).attr("bz4"));
					$("#sBm4").val($(this).attr("bm4")==undefined?"":$(this).attr("bm4"));	
					$("#sBz10").val($(this).attr("bz10")==undefined?"":$(this).attr("bz10"));			
					...
					$("#CustType").val($(this).attr("custtype")==undefined?"":getdatevalue("&cloum=custtype&tablename=custtype&key=custid='2'"));//客户类型
    ...
    //查看是否选择单位合同号，如果选择者弹出窗口显示单位同好和单位名称进行选择
		...
		******/
        ...
        function querybusinessfee(){
			...
			//该方法只适用于申请停复机 中石油定制 modify 2015-12-11
			 if($("#cYingJiao").length>0 ){
			 	$("#cYingJiao").click();
			 }
			}
        ...
        function Bytc_Modify_show(Dh) {
            $("#bytctabs").empty();
               var res1="";
        	    var selectedPackageInfo="";
        	    var i=0;
                var res = fetchMultiArrayValue("selecttypesql",6,"&user_tel=" +Dh);
                if(res==""){
                	alert("无资费");
                }else{
                   res1 = fetchMultiArrayValue("dbsql_phone.query_package_detail_zf",6,"&id="+res+"&dh="+Dh);	
                }
