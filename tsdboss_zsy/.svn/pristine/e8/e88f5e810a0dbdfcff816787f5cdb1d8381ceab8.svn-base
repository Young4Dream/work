	...
	<script type="text/javascript">
        ...
        function savedb(){  
        	...
        	var pubParam = getPubProcParam(); 
         	var param=pubParam+"&internetacct="+v_acct+"&paytype="+v_paytype+"&backfee="+$("#backfee").val()+"&busifee="+v_busifee+"&workmemo="+v_workmemo+"&busiywarea="+tsd.encodeURIComponent($("#ywarea").val());
        	...
        	if(res[0][0]=="SUCCEED"){
        		if (parseFloat($("#txtBussFee").val()) > 0){
        			$("#jobid").val(res[0][1]);
        			setRepFileField();
        			printworkorder('delete');//业务类型，取自于tsd_ini(tsection='radbusi_busifee' and tident='r_delete' )        			
        		}
        		else{	
		...
		function setValue(arr){      	
        	...
        	if(res!=undefined){
        		if (res[0][0] == '-1' || res[0][0] == '1'){
	        		if(parseInt(res[0][2])>0){
	        			alert("该用户为预付费用户，费用尚未结清，请先到收费窗口进行退费！");
		        		$("#queryvalue").val('');
		        		$("#txtacct").val('');
		        		initValue();
		        		return ;
	...
	<DIV class="ui-layout-center">	
		<div id="input-Unit" style="margin-top:-10px">
			...
			<div style="background:#F5DEB3;">
				...
				退费金额:<input type="text" id="backfee" style="width:90px;"/><font color="red">*</font>
				<span style="display: none;">付费方式:<select id="radpayitem" style="width: 90px;" ></select><font color="red">*</font> &nbsp</span>		
