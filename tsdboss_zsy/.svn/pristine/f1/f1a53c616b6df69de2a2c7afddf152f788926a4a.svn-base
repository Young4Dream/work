...
<!-- 导入css文件开始 -->
	...
	$(function(){
		...
		$("#tj_tjway").change(function(){
			var val = $(this).val();
			if(val=="1")
			{
				$("#tj_way_dhhead").css("display","inline");
				$("#tj_way_hmfw,#tj_way_area,#tj_way_noparam").css("display","none");
			}
			else if(val=="2")
			{
				$("#tj_way_hmfw").css("display","inline");
				$("#tj_way_dhhead,#tj_way_area,#tj_way_noparam").css("display","none");
			}
			else if(val=="3")
			{
				$("#tj_way_area").css("display","inline");
				$("#tj_way_hmfw,#tj_way_dhhead,#tj_way_noparam").css("display","none");
			}
			else
			{
				$("#tj_way_noparam").css("display","inline");
				$("#tj_way_hmfw,#tj_way_area,#tj_way_dhhead").css("display","none");
			}//tj_way_dhhead，tj_way_hmfw，tj_way_area，tj_way_noparam
		});
	});
	
	function search()
	{
		var tjway = $("#tj_tjway").val();
		var dhhead = $("#dhhead").val();
		var dhstart = $("#tj_dhstart").val();
		var dhend = $("#tj_dhend").val();
		...
		else if(tjway=="2")
		{
			...
			{
				alert("请输入起始电话号码");
				$("#tj_dhstart").select().focus();
				return;
			}
			if($.trim(dhend)=="")
			{
				alert("请输入结束电话号码");
				$("#tj_dhend").select().focus();
				return;
			}
		}
		...
		var params = "&dhhead=" + cjkEncode(dhhead) + "&dhstart=" + cjkEncode(dhstart) + "&tjway=" + cjkEncode(tjway) + "&dhend=" + cjkEncode(dhend);
		params += "&area=" + cjkEncode(stations);
	...
	function setUserRight()
	{
		...
		if(typeof allRi == 'undefined' || allRi.length == 0)
		{
			...
			var areaa = $("#area").val();
			if(areaa.indexOf("-")>-1)
				areaa = areaa.substring(2,areaa.length);			
			$("#stations").val(areaa);
  ...
  <!-- 
   ...
   -->
	<br />
	<fieldset style="margin-left:120px;margin-top:100px;font-size:14px;">
		<legend style="font-size:14px">空号查询</legend>
		查询方式：<select id="tj_tjway" style="display:inline;">
			<option value="1">按号码前缀查询</option>
			<option value="2">按号码范围查询</option>
			<option value="3">按营业区查询</option>
			<option value="4">查询全部</option>
		</select>
		<div id="tj_way_dhhead" style="display:inline;">
			号码前缀：
			<input type="text" name="dhhead" id="dhhead" style="width:120px;" />
		</div>
		<div id="tj_way_hmfw" style="display:none;">
			电话号码：
			<input type="text" name="tj_dhstart" id="tj_dhstart" style="width:120px;" />
			至
			<input type="text" name="tj_dhend" id="tj_dhend" style="width:120px;" />
			
		</div>
		<div id="tj_way_area" style="display:none;">
			选择营业区域：
			<select id="stations" name="stations" style="width:120px;"></select>
		</div>
		<div id="tj_way_noparam" style="display:none;">
			不限制条件
		</div>
