  ...
  return fmt;   
				...
				********************************************************/
				function fuheQueryUserLine(querysql,querypagesql,mkj,column)
				{
					...
					var link = urlstr1 + params;	
					var languageType = $("#languageType").val();
				 	$("#editgrid").setGridParam({url:'mainServlet.html?'+link+'&languageType='+languageType+'&str='+mkj+'&cloumn='+column}).trigger("reloadGrid");
				 	setTimeout($.unblockUI, 15);//关闭面板			
				}
				...
				********************************************************/
				...
				/***************** 外部话费导入 ******************************/
				/**********************************************************
				function name:    wbhfImport(tablename)
				function:		  显示要导出的数据列
				parameters:      tablename：表名,thiscol:要一行显示的字段数
				return:			  
				description:      
				********************************************************/
				function wbhfImport(ds,tsdcf,tablename,lantype,thiscol){
					$('#query').html('全选');
							var thevalue = "";						
							thevalue += "选择外部话费导入文件："
							thevalue += "&nbsp&nbsp<input type='file' id='mfile' name='mfile' value='' height='36px' width='300px'/>&nbsp;";
							$('#importform').html(thevalue);															
				 			autoBlockFormAndSetWH('theimportform',90,90,'importclose',"#ffffff",false,740,'');//弹出查询面板							 
						 	//	autoBlockFormAndSetWH(formId,top,left,closeId,bgColor,ifOverlayClose,width,height)
				}
				...
				**/
				...
				function wbImport()
				{
					var fileUrl = document.getElementById('mfile').value;
					fileUrl = fileUrl.substring(fileUrl.lastIndexOf(".")+1);
					if( fileUrl != "txt" )
					{
						jAlert("导入文件类型只能为 txt 或 csv 格式。","操作提示");
						return;
					}
