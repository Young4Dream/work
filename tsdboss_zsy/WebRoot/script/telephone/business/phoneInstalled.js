 		...
 		/////////////////////////////////////////////////////////////////////////////////////////////////////////////// 						  		  
			...
			********/
			function selectinserttype(){
				...
				if(str=="1"){
					...
					$("#Yhmc_hthdang").parent().prev().html("账户名称");
					$("#isk").show();
					$("#kuaquyu").show();
					...
					$("#Dh_hthdang").parent().show();
					$("#Dh_hthdang").parent().prev().show();
					//$("#Dhlx").val("固定电话");
					...
					$("#Dh_hthdang").parent().hide();
					$("#Yhmc_hthdang").parent().prev().html("合同号名称");
					$("#Dh_hthdang").parent().prev().hide();
			...
			function removenullhthcontent(){	 
				...
				$("#Yhxz_hthdang").val($("#usertype").val()==""?"":$("#usertype option:selected").text());
				getyhxzstr('p_setup');
			}					
        	...
        	********/
			function showNumInfo(phoneNum){
				...
				s+="</tr>";
				$("#tabbusiyxx").append(s);
				var res = fetchMultiArrayValue('patch.yxx',6,"&dh="+phoneNum,"patch");
				if(res != undefined && res!="" && res.length>0){
					...
					s+="<td>"+res[0][4]+"</td>";
					s+="</tr>";
				        autoBlockForm('divbusifee', 60, 'close', "#ffffff", false);
				}
           ...
           var cphonenum = $("#Dh_yhdang").val();
		    ...
		    var reszh = fetchFieldAlias('Hthdang',languageType);			 
			 ...
			 if(str=='1'){
	        	...
	        	if(resgfhth==undefined||resgfhth=='0'){
		        	//if($("#Yhlb_hthdang").val()=="公费"&&$("#Bm1_yhdang").val()=="")
		        	if($("#Yhlb_hthdang").val()==$("#getinternet0384").val()&&$("#Bm1_yhdang").val()=="")
		        	{
		        		//alert("当前用户类别为公费，请选择固话信息中的一级部门");
		        		alert($("#getinternet0033").val());
		        		$("#Bm1_yhdang").select();
		        		$("#Bm1_yhdang").focus();
		        		return false;
		        	}
			...
			*/
			...
			//一级部门
				...
				//if(!checkYCXFY()) return false;
		       	...
		       	//跨模块局  add 2016-01-06
		       	procparams+="&username="+encodeURIComponent($('#username').val());
		       	var mkj1 = $("#mokuaiju").val();
		       	procparams+='&mkj1='+tsd.encodeURIComponent(mkj1);
				...
				if(isk==true)
				{
					...
					procparams+='&mkjacross=1';
					procparams+='&mkj2='+tsd.encodeURIComponent(mkj2);
					procparams+='&mkj3='+tsd.encodeURIComponent(mkj3);
					procparams+='&mkj4='+tsd.encodeURIComponent(mkj4);
				}else
				{
					procparams+='&mkjacross=0';
				}
			...
			//营业区域
			var userareaid = $("#userareaid").val(); // 六铺炕~物资大厦~东联大厦~专网~金融街	
			if(userareaid=='全地区'){
				userareaid='全地区~丰和大厦~昆仑大厦~丰和园小区~六铺炕~物资大厦~东联大厦~昌平科技园~东直门~专网~金融街~在京单位(其他)~京外单位~海外公司~昌平科技园A34~昌平科技园A16~昌平科技园A29、A42、A45';
			}
			procparams += "&userarea=";
			procparams += tsd.encodeURIComponent(userareaid);
			//区域营业厅
			var uarea = $("#area").val();    //六铺炕
			procparams += "&uarea=";
			procparams += tsd.encodeURIComponent(uarea);	
			procparams +="&fsbm="+$("#fsbm").val();	
			//mingliang 2016.03.18添加套餐保存
			if("" != $("#hasSelectedPackageId").val()){
				procparams +="&packageid="+$("#hasSelectedPackageId").val();	
			} else {
				alert('请选择电话资费！');
				return false;
			}
			
			var startdz=$("#startdz").val();
			var enddz=$("#enddz").val();
			procparams +="&startdz="+tsd.encodeURIComponent(startdz);
			procparams +="&enddz="+tsd.encodeURIComponent(enddz);
...
//alert("==="+procparams);
        ...
        *********/
        function queryhthkey(){
        	...
        	var selectinserttypesss=$("#selectinserttype").val();
        	if(selecththcontent.length==0){
        		//alert("请输入查询条件！");
        		alert($("#getinternet0041").val());
        	...
        	if(selecththvalue==""){
        		...
        		if(selectinserttypesss=='1'){
        			querykey="dbsql_phonelnstalled.selecththkey_hth";
        		}else{
        			querykey="dbsql_phonelnstalled.selectnullhthqu";
        		}
        	...
        	$("#selecththkey").empty();//每次查询前将以前的数据清空
        	var res = fetchMultiArrayValue(querykey,6,"&hth="+tsd.encodeURIComponent(selecththcontent));
        	if(res[0]==undefined || res[0][0]==undefined)
			{	
				$("#selecththkey").append("<tr><td>"+$("#getinternet0366").val()+"</td></tr>");
				return false;
        	...
        	if(selectinserttypesss=='1'){
    			queryfangfa="getsavexzhth";
    		}else{
    			queryfangfa="setsavehth";
    		}
            var ify="";
            for(var i=0;i<res.length;i++){
                ify +="<tbody>";
                ify += "<tr onClick=\"getHTHXZ('"+res[i][0]+"')\" onDblClick=\""+queryfangfa+"('"+res[i][0]+"')\" id=\""+res[i][0]+"\">";					
				ify += "<td style=\"width: 250px;height: 20px\"><center>";
				ify += res[i][0];
				ify += "</center></td>";
        ...
        *********/
		function getsavexzhth(key){
			...
			//取HTHDANG的联系人、联系电话
			var lx = fetchMultiArrayValue("select.lxdhren",6,"&Hth="+str);
			$("#Bz8_yhdang").val(lx[0][0]);
			$("#Bz9_yhdang").val(lx[0][1]);
			//取htt号信息信息
       ...
       function getyhxzstr(key){
				...
				if(key=="p_setup"){
					...
					$("#Yhxz_hthdang").html($("#Yhxz_hthdang").html()+"<option value='' selected='true'>--"+$("#getinternet0365").val()+"--</option>");
					$("#UserTypeyhdang").html($("#UserTypeyhdang").html()+"<option value='' selected='true'>--"+$("#getinternet0365").val()+"--</option>");
				}		
	  					...
	  					if(op.value=="undefined"||op.value==undefined||op.value==""){
	  						...
	  						if(array[2]==$("#usertype").val()){
	  							if(key=="p_setup"){	  								
									...
									$("#Yhxz_hthdang").html($("#Yhxz_hthdang").html()+"<option value="+array[0]+">"+array[0]+"</option>");
	  								$("#UserTypeyhdang").html($("#UserTypeyhdang").html()+"<option value="+array[1]+">"+array[0]+"</option>");
	  							}	  							
	  						}
	  					}
	  				}
        ...
        *********/
		...
		{	
			...
			var urlstr=tsd.buildParams({packgname:'service',//java包
                 ...
                 $.ajax({
	              url:'mainServlet.html?'+urlstr+'&ywlx='+tsd.encodeURIComponent('p_setup')+"&typeid="+'50',
	              datatype:'xml',
	              cache:false,//如果值变化可能性比较大 一定要将缓存设成false
	              timeout: 1000,
	              async: false ,//同步方式
        ...
        *********/ 
        function Bytc_Saves() {
        	...
        	if(tctype=="" || tctype==undefined){
        		...
        		return false;
        	}else if(tctype == $("#hasSelectedPackageId").val()){//选择是否重复
        		alert("当前已选择该套餐，不能重复选取！");
        		return false;
        	}else{
        		//alert(/^\d{4}-\d{2}-\d{2}$/.test($("#TCStarttime").val()));
        		var dh = $("#Dh_yhdang").val();
        		...
        		var rlns = fetchMultiArrayValue("dbsql_phone.query_package_rlns",6,"&userid="+dh);//关系表的id和套餐id
        		if(rlns[0]==undefined || rlns[0][0]==undefined){//如果之前没选择套餐直接加载一个套餐
        			var res = fetchMultiArrayValue("dbsql_phone.query_package_detail",6,"&id="+tctype);
					...
					selectedPackageInfo += res[0][0];//名称
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"635\"><center>";
					if(1 == res[0][1]){
						res[0][1]="包年";
					}else if(2 == res[0][1]){
						res[0][1]="包月";
					...
					selectedPackageInfo += res[0][6];//类型
					selectedPackageInfo += "</center></td>";
					/*
					selectedPackageInfo += "<td width=\"100\"><center>";
					selectedPackageInfo += res[0][2];//固定费
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"150\"><center>";
					selectedPackageInfo += res[0][3];//免费期数
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"140\"><center>";
					selectedPackageInfo += res[0][4];//开始时间
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"140\"><center>";
					selectedPackageInfo += '2999-12-31';//结束时间
					selectedPackageInfo += "</center></td>"; */
					selectedPackageInfo += "<tr>";
        			
        		}else{//如果存在套餐判断是否和当前的一致
        			if(rlns[0][1] == tctype || (undefined != rlns[1] && rlns[1][1] == tctype)){
        				alert("当前已选择该套餐，不能重复选取！");
        				return false;
        			}else{
        				index = 2;
        ...
        *@param;
       	*@return;
        *********/ 
        function Bytc_Modify() {
        	var tctype = $("#Packagetypeshidden").val();
        	var selectedIds = $("#hasSelectedPackageId").val();
        	if(tctype=="" || tctype==undefined){
        		//alert("请选择包月套餐类型");
        		alert($("#getinternet0054").val());
        		return false;
        	}else if(selectedIds.indexOf(","+tctype+",") > -1){//选择是否重复
        		alert("当前已选择该套餐，不能重复选取！");
        		return false;
        	}else{
        		//alert(/^\d{4}-\d{2}-\d{2}$/.test($("#TCStarttime").val()));
        		...
        		var selectedPackageInfo = "";
        		var packageIds = ",";
        		var res = fetchMultiArrayValue("dbsql_phone.query_selected_rlns",6,"&Dh=" +dh);//查询当前已有的套餐取当前生效的那个，第二个取当前选择的
        		var hasPackage = false;//判断当前是否有套餐
        		if(undefined != res && "" != res && undefined != res[0] && "" != res[0]){
        			hasPackage = true;
        			packageIds += (res[0][8]+",");
        			selectedPackageInfo += "<tr>";
					selectedPackageInfo += "<td width=\"10\"><center>";
					...
					*/
					...
					selectedPackageInfo += res[0][1];//类型
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"100\"><center>";
					selectedPackageInfo += res[0][2];//固定费
					selectedPackageInfo += "</center></td>";
					...
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"140\"><center>";
					selectedPackageInfo += res[0][6];//结束时间变为月底最后一天
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<tr>";
				}
       			var res1 = fetchMultiArrayValue("dbsql_phone.query_package_detail",6,"&id="+tctype);
       			if(undefined != res1 && "" != res1 && undefined != res1[0] && "" != res1[0]){
       				packageIds += (tctype+",");
        			selectedPackageInfo += "<tr>";
					selectedPackageInfo += "<td width=\"10\"><center>";
					selectedPackageInfo += "<input type=\"checkbox\" id=\"v_bytctab_checks\"" + i + " />";
					...
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"170\"><center>";
					selectedPackageInfo += res1[0][0];//名称
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"105\"><center>";
					/**1、包年
					2、包月
					...
					*/
					...
					selectedPackageInfo += res1[0][1];//类型
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"100\"><center>";
					selectedPackageInfo += res1[0][2];//固定费
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"150\"><center>";
					selectedPackageInfo += res1[0][3];//免费期数
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"140\"><center>";
					if(hasPackage){//有套餐下月初生效
						selectedPackageInfo += res1[0][5];//开始时间下月初
					}else{//没有套餐立即生效
						selectedPackageInfo += res1[0][4];//开始时间当前时间
					}
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"140\"><center>";
					selectedPackageInfo += "2999-12-31";//结束时间
        	...
        	if(count<1)
        	{
        		...
        		if(undefined != res && "" != res && undefined != res[0] && "" != res[0]){
					...
					*/
					...
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<td width=\"140\"><center>";
					selectedPackageInfo += "2999-12-31";//结束时间变为月底最后一天
					selectedPackageInfo += "</center></td>";
					selectedPackageInfo += "<tr>";
        		}
				...
				$("#bytctabs").append(selectedPackageInfo);
	        	$("#hasSelectedPackageId").val(packageIds);//选中套餐id集
        	}
        	$("#Packagetypeshidden").val("");
        	$("#Packagetypes tr").css('background-color','#f7f7f7');
        	//Bytc_Refreshs('');
          ...
          var languageType = $("#languageType").val();  
	        $.ajax({
					...
					{	
					    ...
					    var b = xml.substring(xml.indexOf("<dhlx=")+6,xml.indexOf("dhlx/>"));
					     b.substring(b.indexOf('</option>')+9,b.length);//huoshuai 2014-06-30 add ge bie chu li
					    var sdhlx = b.replace("<option lvalue='领士电话' value='领士电话'>领士电话</option>","<option lvalue='' value=''>--请选择--</option><option lvalue='领士电话' value='领士电话'>领士电话</option>");
					    var sdhlxls= sdhlx.replace('<option lvalue="领示电话" value="领示电话">领示电话</option>');
					    ...
					    var a = xml.substring(xml.indexOf("<yhdang=")+8,xml.indexOf("yhdang/>"));
					    var tableyhdang=a.replace("<td class='wenzi'>代办人名称</td><td class='wenzix'><input type='text' name=Bz1_yhdang id=Bz1_yhdang  style='width: 150px;' maxlength=50  onpropertychange='TextUtil.NotMax(this)'/></td><td class='wenzi'>与户主关系</td><td class='wenzix'><input type='text' name=Bz2_yhdang id=Bz2_yhdang  style='width: 150px;' maxlength=50  onpropertychange='TextUtil.NotMax(this)'/></td><td class='wenzi'>代办证件号码</td><td class='wenzix'><select name=Bz5_yhdang id=Bz5_yhdang  style='width: 150px;'/></td></tr><tr><td class='wenzi'>查询密码</td><td class='wenzix'><input type='text' name=Mima_yhdang id=Mima_yhdang  style='width: 150px;' maxlength=16  onpropertychange='TextUtil.NotMax(this)'/></td><td class='wenzi'>代办证件类型</td><td class='wenzix'><select name=Bz4_yhdang id=Bz4_yhdang  style='width: 150px;'/></td>","");
					   // var tableyhdangxdanwei=tableyhdang.replace("<td class='wenzi'>代办单位名称</td><td class='wenzix'><input type='text' name=Bz10_yhdang id=Bz10_yhdang  style='width: 150px;' maxlength=50  onpropertychange='TextUtil.NotMax(this)'/></td>","");
					    //var tableyhdangxdanweidh=tableyhdangxdanwei.replace("<td class='wenzi'>代办人电话</td><td class='wenzix'><select name=Bz3_yhdang id=Bz3_yhdang  style='width: 150px;'/></td>","");
					    /* var c = b.replace("区域二","安装区域二");
					    var d = c.replace("区域三","安装区域三");
					    var tableyhdang = d.replace("区域四","安装区域四");*/
					    /*alert("嘿嘿="+tableyhdang.substring(tableyhdang.indexOf("证件号码")));*/
					   ...
					   // $("#Dhlx").html(sdhlx);	
						...
						var mfjbyhdang = fetchMultiArrayValue("dbsql_phone.querytabledate",6,"&tablename=Free_Grade&cloum=free_code,cname&key=1=1"); 
						if(mfjbyhdang!=undefined){
							var strinfo="<option value=''>--请选择--</option>";
							for(var i=0;i<mfjbyhdang.length;i++){
									strinfo+="<option value='"+mfjbyhdang[i][0]+"'>"+mfjbyhdang[i][1]+"</option>";
							}	
							$("#Mfjb_yhdang").html(strinfo);
						}
						var mokuaijuydang = fetchMultiArrayValue("dbsql_phone.querytabledate",6,"&tablename=mokuaiju&cloum=mokuaiju,mokuaiju&key=1=1"); 
						if(mokuaijuydang!=undefined){
							var strinfo="<option value=''>--请选择--</option>";
							for(var i=0;i<mokuaijuydang.length;i++){
									strinfo+="<option value='"+mokuaijuydang[i][0]+"'>"+mokuaijuydang[i][1]+"</option>";
							}	
							$("#Mokuaiju_yhdang").html(strinfo);
						}
						/*
						...
						*/	
						...
						var resbutetype = fetchMultiArrayValue("dbsql_phone.querytabledate",6,"&tablename=teltype&cloum=typename,typename&key=1=1");
						...
						var resservicearea = fetchMultiArrayValue("dbsql_phone.querytabledate",6,"&tablename=sys_servicearea&cloum=id,areaname&key=1=1");
						var servicearea_text="";
						if(resservicearea!=undefined){
							servicearea_text="<option value=''>--请选择--</option>";
							for(var i=0;i<resservicearea.length;i++){
									servicearea_text+="<option value='"+resservicearea[i][0]+"'>"+resservicearea[i][1]+"</option>";
							}	
							$("#servicearea_yhdang").html(servicearea_text);
						}						
						//如果是专线业务页面执行以下语句
						if($("#dedicatedflag").val()=="1"){
							
							$("#Dhlx").find("option").each(function(){
								if($(this).text()=='E1'){
									return false;
								}else{
									$("#Dhlx option[text='"+$(this).text()+"']").remove();
								}
							});
						}
						lodingset();//字段属性加载完执行该方法进行对编辑框操作
						
						//add by wcy 2016-09-10，中石油业务电话装机页面点开后，自动联动放在这里，不用settimeout延迟的方法，不规范
						$('#Dhlx').val('局内电话');
                        $('#Dhlx').change();
                        $('#usertype').val('办公用户');
                        $('#usertype').change();
...
//            }
        ...
        *********/
        ...
        function savesBm(){
        	...
        	$("#Yhmc_hthdang").val(sbmname); 
        	$("#Yhmc_hthdang").attr("disabled","");
		...
		****/
        function addsBmsihth(){          
		 ...
		 function c_p_address_fun()
		 {
			...
			address_tab += "<tr><td>"+$("#getinternet0375").val()+"</td><td><select id=\"c_p_address_xq\"><option value=\"\">--"+$("#getinternet0365").val()+"--</option></select></td><td>"+$("#getinternet0376").val()+"</td><td><select id=\"c_p_address_ld\"><option value=\"\">--"+$("#getinternet0365").val()+"--</option></select></td><td>"+$("#getinternet0377").val()+"</td><td><select id=\"c_p_address_dy\"><option value=\"\">--"+$("#getinternet0365").val()+"--</option></select></td><td>门牌号</td><td><select id=\"c_p_address_mp\"><option value=\"\">--"+$("#getinternet0365").val()+"--</option></select></td></tr>";
        ...
        ********/
        function getPackages()
        {
        	var urlstr=tsd.buildParams({
				...
				tsdoper:'query',//操作类型 
				datatype:'xmlattr',//返回数据格式 
				tsdpk:'phone.getPkg'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
 			});	
 			
			$.ajax({
				url:'mainServlet.html?'+urlstr+"&sumfields=package_name,id,service_type&tablename=ocs_package_main&initialization=1=1&fusearchsql=package_status=2",					
				datatype:'xml',
				cache:false,//如果值变化可能性比较大 一定要将缓存设成false
				timeout: 1000,
				async: false ,//同步方式
				success:function(xml){
					var ghzfzfx = '';
					$(xml).find('row').each(function(){
						if($(this).attr("package_name")!=undefined){
							var packageName = $(this).attr("package_name");
							var packageId = $(this).attr("id");
							var service_type = $(this).attr("service_type");
							if (service_type == '电话') {
                 				ghzfzfx +="<tr onclick='choosePackage(this)' id=\""+packageId+"\"><td>";
                 				ghzfzfx +="☞"+packageName;
                 				ghzfzfx +="</td></tr>";
                 			}
                 		}	
					});
					$("#Packagetypes").append(ghzfzfx);
				}
			});
        }
        ...
        *点击选中套餐
        ********/
        function choosePackage(obj){
        	$("#Packagetypes tr").css('background-color','#f7f7f7');
        	$(obj).css('background-color',"#0080FF");
			$("#Packagetypeshidden").val($(obj).attr("id"));
        	//根据套餐id加载套餐详细信息
        	//var res = fetchMultiArrayValue("dbsql_phone.query_package_detail",6,"&id="+$(obj).attr("id"));
        	//alert(res[0][0]);
        }
        	...
        	var urlstr=tsd.buildParams({ 
				...
				$.ajax({
					...
					success:function(xml){
							$(xml).find('row').each(function(){
								if($(this).attr("feetype")!=undefined){									
		                 			ghzfzfx +="<tr onClick=\"getGHfeetr_hth('"+$(this).attr("feetype")+"');\" id=\""+$(this).attr("feetype")+"\"><td>";
		                 			ghzfzfx +="☞"+$(this).attr("feetype");
		                 			ghzfzfx +="</td></tr>";
		                 		}	
							});
	...
	function choiceMokuaiju()
	{
		...
		{
			loadmokuaiju();
			autoBlockForm('mokuaijupanl', 60, 'exitmkj', "#ffffff", false);
		}else
		{
			$('#mkj2').val('');
			$('#mkj3').val('');
	...
	function loadmokuaiju(){
	    ...
	    $("#levelthree").empty();
	    $("#levelfour").empty();
	    var res = fetchMultiArrayValue("phonelnstalled.query",6,"");
	    var queryBASsel="";
    ...
    *******************/
    var TextUtil = new Object(); 
       ...
       *@param;
      	*@return;
       ********/
       function selecththkh(key){
    	   var selectinserttypess=$("#selectinserttype").val();
    	   if(selectinserttypess=="1"){
       	...
       	if(utype==1)
       	{
       		//sql='dbsql_phonelnstalled.selecththkeys';
       		sql='dbsql_phonelnstalled.selectnullhth';
       	}else
       	{
       		//sql='dbsql_phonelnstalled.selecththkeyg';
       		sql='dbsql_phonelnstalled.selectnullhth';
       	}
			/*
       	var phone = $("#Dh_yhdang").val();
       	...
       	if(res[0]==undefined || res[0][0]==undefined)
			{
				$("#selecththkey").append("<tr><td>没有信息！！！</td></tr>");
				return false;
			}
           var ify="";
           for(var i=0;i<res.length;i++){
               ify +="<tbody>";
               ify += "<tr onClick=\"getHTHXZ('"+res[i][0]+"')\" onDblClick=\"setsavehth('"+res[i][0]+"')\" id=\""+res[i][0]+"\">";					
				ify += "<td style=\"width: 120px;height: 20px\"><center>";
				ify += res[i][0];
				ify += "</center></td>";
				ify += "<td width=\"120px;\"><center>";
				ify += res[i][1];
				ify += "</center></td>";
				ify += "<td width=\"180px;\"><center>";
				ify += res[i][2];
				ify += "</center></td>";
				ify +="</tr>";
				ify +="</tbody>";
