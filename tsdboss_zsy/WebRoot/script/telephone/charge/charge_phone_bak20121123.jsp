...
description:      
**********************************************************************/
function btnFindClick(pay_type,payMonth)
{
	...
	$.ajax({
	url:"charge_phone?func=find&dh="+dhNum+"&Sj_SfMonth="+payMonth+"&sSkfs="+encodeURIComponent($("#ghFFFs").val())+"&Kemu=" + encodeURIComponent(revenuetype) + (pay_type=="groupcustomer"?"&DKHFlag=Y":""),
	async:true,//异步
	cache:false,
...
{		
	...
	if($("row:first",xml).attr("sflx")==2){
		alert("该用户为预付用户,只能在主营业厅收取,请注意!");
...
function JiaoFei(pay_type)
{
	...
	//缴费
	$.ajax({
		url:"charge_phone?func=save&Version=200404&Kemu="+encodeURIComponent(revenuetype) + dkhjfparam +"&Bcss=" + ghss+ "&SkrID="+$("#skrid").val() + "&Skfs=" + encodeURIComponent($.trim($("#ghFFFs").val()))+"&loginfo="+loginfo+"&hbdy="+hbdy+"&prepay="+prepay+'&fewtype=' + fewtype + '&Skval=' + $('#tsdSkvalText').val(),
		async:true,//异步
		cache:false,
...
function ghKFLS(Hth)
{
	...
	temp += '<td><fmt:message key="charge_phone_js.hfmonth"/></td><td><fmt:message key="charge_phone_js.feeheji"/></td><td><fmt:message key="charge_phone_js.feeznj"/></td><td><fmt:message key="charge_phone_js.thfee"/></td><td><fmt:message key="charge_phone_js.monthfixfee"/></td><td><fmt:message key="charge_phone_js.adjustfee"/></td><td><fmt:message key="charge_phone_js.packagefee"/></td><td><fmt:message key="charge_phone_js.thismonthfee"/></td><td><fmt:message key="charge_phone_js.lastmonthfee"/></td><td><fmt:message key="charge_phone_js.skr"/></td>';
	temp += "</tr>";
