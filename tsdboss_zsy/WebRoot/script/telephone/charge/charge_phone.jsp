...
function btnFindClick(pay_type,payMonth,cashmode)
{
   ...
   //立即结账	
    ...
    //如果输入无效电话直接退出返回 20151223 add by zxy
    if (strdhNum == '') {
 	$("#ghSearchBox").select().focus();
...
function ghClearInfoWithoutSearchBox()
{
	...
	//
	$("#multiHthDetail,#commonHthYfDetail").show();
		...
		if(cashmode == 'real' || cashmode =='multi' || cashmode =='multiOne'){
			...
			if(isCeilCharge == 'Y'){			
				if(integerType=="floor"){
					...
					//yjff = Math.ceil(yjff);  // 20151225 modify by zxy 中石油项目递交最小金额为角 暂时暂时先注释掉
				}				
			}
			//$("#ghSs").val(yjff);
			//2012-9-25 yhy start
...
description:      
**********************************************************************/
function onSave(pay_type,ssyes)
{
	//取应交金额的值
	var yj = $("#ghYJ").val();
	yj=parseFloat(yj,10);
...
description:      
**********************************************************************/
function JiaoFei(pay_type,ssyes)
{
	...
	//电话缴费实收金额
	//如果是退费，则转换金额 modify donglei 2015-10-09
	if($("#ghInfo").data("tuifei")=="T"){
		$("#ghSs").val(-ssyes);
	}
	var ghss = $("#ghSs").val();
...
{	
	...
	{	
	    ...
	    //过程执行结果
	   	...
	   	var ssyes=$("#ghInfo").data("ssye");//huoshuai add 20150811
		...
		//"确定要对 "																	//" 执行退费操作吗？当前可退费金额 "										" 元，点击确定将完成退费"
			if(confirm('<fmt:message key="charge_phone_js.confirmfee"/>' + $("#ghInfo").data("hth") + '<fmt:message key="charge_phone_js.confirmcancel"/>' + ssyes + '<fmt:message key="charge_phone_js.confirmcomplete"/>'))
			{
