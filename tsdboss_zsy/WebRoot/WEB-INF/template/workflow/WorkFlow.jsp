				...
				if(xml!=''){
					...
					if($('#iComplete').text()=='2'){//当前业务类型的工单是否全部提交完毕
						...
						if(ywlxCount!=''){
							...
							//$('#menudiv').css('height',sLines*50+'px');
							$('#menudiv').css('height','0px');
						}
...
function WorkFlowManager(operflag){
	...
	var jobid = res;
	//alert("jobid=" + res + ", busiclass=" + busiclass + ", ywlx=" + thisYwlx);
	params += '&jobid='+res;
	...
	if(operflag==1){//接收工单
		...
		if(confirm('确定要'+sTips+'工单吗？')){
			...
			if(canCommit[0][0]==0){
				...
				if (busiclass == '53') {
				var ps = "&jobid=" + jobid;
				var uuid = fetchSingleValue('WorkFlow.getRadJobBusiId', 6, ps, 'workflow');
				if (uuid == undefined || res == '') {
					alert('unknown error: ' + ps);
					return false;
				}
				ps = "&radacc=" + radacc + "&jobid=" + jobid;
				executeNoQuery('WorkFlow.updateRadJobAcc', 6, ps, 'workflow');
				ps = "&radacc=" + radacc + "&radpwd=" + radpwd + "&busiid=" + uuid;
				executeNoQuery('WorkFlow.updateRadAcc', 6, ps, 'workflow');
				}
