...
function  getJobList(){
	...
	if('<%=session.getAttribute("userid") %>'=='admin'){
		str = '';
	}else if(userright=='true' || userright==''){
		str = '0';
	}
	...
	var urlstr=tsd.buildParams({ 	packgname:'service',//java包
			 					...
			 					tsdoper:'query',//操作类型 
			 					datatype:'xml',//返回数据格式 
			 					tsdpk:'112jobmanager.queryjoblist'+str,//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
			 					tsdpkpagesql:'112jobmanager.queryjoblistpage'+str//依赖tsdpk 用于计算分页的sql配置名称应在tsdcf配置的文件中指定
			 					});
var column  = '';
...
jQuery("#editgrid").jqGrid({
	url:'mainServlet.html?'+urlstr+'&cloumn='+column+'&userid=<%=session.getAttribute("userid") %>&mqbm='+tsd.encodeURIComponent('<%=session.getAttribute("departname") %>')+'&area='+tsd.encodeURIComponent('<%=session.getAttribute("userarea") %>'),
	datatype: 'xml', 
	...
	colModel:thisdata.colModels, 
	       	...
	       	multiselect: true,
			multiselectWidth: 20,
			height:document.documentElement.clientHeight-145, //高
			 ...
			 ondblClickRow : function (ids) 
        	{
        		if (ids != null) 
            	{
					...
					var ids = $('#editgrid').getCell(ids,'Dh');
...
function getSelectGzList()
{
    ...
    $.ajax(
    {
        url : 'mainServlet.html?' + urlstr, datatype : 'xml', cache : false, 
...
function getFirstDepart()
{
    ...
    $.ajax(
    {
        url : 'mainServlet.html?' + urlstr + '&area='+tsd.encodeURIComponent('<%=session.getAttribute("userarea") %>'), datatype : 'xml', cache : false,
...
function saveRgsl(){
	...
	params += '&mqbm=' + tsd.encodeURIComponent(getFirstDepart());
...
function reloadData()
{
    ...
    $("#editgrid").setGridParam({
        url : 'mainServlet.html?' + urlstr + '&cloumn=' + thecolumn +'&mqbm='+tsd.encodeURIComponent('<%=session.getAttribute("departname") %>')+'&area='+tsd.encodeURIComponent('<%=session.getAttribute("userarea") %>')
    }).trigger("reloadGrid");
...
function  sendJobs(flag){
	...
	for(var i = 0 ; i < arr.length  ; i++){
		idarr.push($('#editgrid').getCell(arr[i],'Bz'));
	...
	if(flag=='send'){
		...
		if(confirm('确定要发送'+str+'工单吗?')){
			...
			if(idarrs.length>0){
				var disnextbmarr = disnextbm.split('@');
				send112Jobs(idarrs,disnextbmarr[1]);				
				var pgbz = $('#pgbz').val();
				for(var i = 0 ; i < idarrs.length;i++){
					var pgid = getNextBmPgid(idarrs[i]);
					savePglc(idarrs[i],pgid,disnextbmarr[0],pgbz);
				}
			}else{
		...
		if(disnextbm==''){
			...
			if(disnextbm.indexOf('~')!=-1){
				...
				displayinfo+='<input type="radio" id="slxbm0" name="astr" checked="checked"/><span id="lxbm0">'+disnextbmarr[0]+'</span>';
			}
...
function getNextDepart(key)
{
    ...
    var urlstr = tsd.buildParams(
    {
        ...
        tsdoper : 'query', //操作类型 
        datatype : 'xmlattr', //返回数据格式 
        tsdpk : '112jobmanager.send112jobs_nextdept'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
    });
    var disuserid = '<%=session.getAttribute("userid") %>';
    var disdept = '';
    if(disuserid!='admin'){
    	disdept = '<%=session.getAttribute("departname") %>';
    }else{
    	disdept = key;
    }
    var params = '&area='+tsd.encodeURIComponent('<%=userarea %>') + '&department='+tsd.encodeURIComponent(disdept);
    $.ajax(
    {
...
function getNextBmPgid(key)
{
    ...
    $.ajax(
    {
        url : 'mainServlet.html?' + urlstr + '&id='  + key, datatype : 'xml', cache : false, 
        //如果值变化可能性比较大 一定要将缓存设成false
        timeout : 3000, async : false , //同步方式
        success : function (xml)
        {
...
function savePglc(ID,PgID,nextbm,pgbz){
    ...
    $.ajax(
    {
        url : 'mainServlet.html?' + urlstr+params, cache : false, //如果值变化可能性比较大 一定要将缓存设成false
        timeout : 3000, 
        async : false , //同步方式
        success : function (msg)
        {
...
function  fhJobs(flag){
	...
	if(flag=='fh'){
		...
		$('#fhgzxx').val(fhgzxx);
		$('#testdatafuhe').val(fetchSingleValue('112jobmanager.querytestdata',6,'&id='+$('#editgrid').getCell(arr[0],'ID')));
...
function pxUserList(userarea)
{
    ...
    $.ajax(
    {
        url : 'mainServlet.html?' + urlstr + '&userarea=' + tsd.encodeURIComponent(userarea) + '&departname='+tsd.encodeURIComponent(departname), datatype : 'xml', cache : false, //如果值变化可能性比较大 一定要将缓存设成false
        timeout : 3000, async : false , //同步方式
        success : function (xml)
        {
            $(xml).find('row').each(function ()
...
function  pxJobs(flag){
	...
	if(flag=='px'){
		...
		var disbm = $('#editgrid').getCell(arr[0],'Bz');
		/**
		...
		*/
		savePxjg(pxid,pxbz,pxuserid,pxarea,disbm);//故障派修
	}else{
		...
		if(ispxflag==true){
			...
			$('#testdatapaixiu').val(fetchSingleValue('112jobmanager.querytestdata',6,'&id='+$('#editgrid').getCell(arr[0],'ID')));
...
//往工单流程表里插入数据
function savePxjg(ID,Pxrz,wxry,ywarea,disbm){
	//ID,PgID,Fsbm,Fsry,Fsrq,Bm,Bz
	var urlstr = tsd.buildParams(
    {
        packgname : 'service', //java包
    ...
    params += '&id=' + ID;
    params += '&Pxry=' + tsd.encodeURIComponent('<%=session.getAttribute("userid") %>');
    params += '&Pgrz=' + tsd.encodeURIComponent(disPxrz);
    params += '&Wxbz=' + tsd.encodeURIComponent(ywarea+'外线班');
...
function getPxrz(key)
{
    ...
    $.ajax(
    {
        url : 'mainServlet.html?id='+ key + urlstr , datatype : 'xml', cache : false, //如果值变化可能性比较大 一定要将缓存设成false
        timeout : 3000, async : false , //同步方式
        success : function (xml)
        {
                $(xml).find('row').each(function ()
...
function saveEnd(ID,wxjg,bz){
    ...
    params += '&Wxjg=' + tsd.encodeURIComponent(wxjg);//维修结果
    params += '&bz=' + tsd.encodeURIComponent(bz);//维修备注
    //params += '&Wxls=' + tsd.encodeURIComponent(disWxls);//维修历时
...
function changeJobWxls(key){
    ...
    $.ajax(
    {
        url : 'mainServlet.html?id=' + key + urlstr, cache : false, //如果值变化可能性比较大 一定要将缓存设成false
        timeout : 3000, 
        async : false , //同步方式
        success : function (msg)
        {
	...
	if(arr.length==0){
		...
		var urlstr=tsd.buildParams({ 	packgname:'service',//java包
				...
				$.ajax({
							...
							url:'mainServlet.html?' + urlstr + '&id=' +$('#editgrid').getCell(arr[0],'ID') +'&tsdtemp='+Math.random(),
							datatype:'xml',
							cache:false,//如果值变化可能性比较大 一定要将缓存设成false
							timeout: 5000,
							async: false ,//同步方式
...
function thisInfo(ids, mqbm, key)
{
    ...
    $.ajax(
    {
        url : 'mainServlet.html?dh=' + ids + urlstr + '&tsdtemp=' + Math.random(), datatype : 'xml', 
        cache : false, //如果值变化可能性比较大 一定要将缓存设成false
...
function exeUp112bz(id,bz,dh)
{
    ...
    $.ajax( 
    {
        url : 'mainServlet.html?bz=' + tsd.encodeURIComponent(bz) + '&id=' + id + urlstr, cache : false, //如果值变化可能性比较大 一定要将缓存设成false
        timeout : 3000, async : false , //同步方式
        success : function (msg) 
        {
            if (msg == "true") 
...
function getDhs(jobids)
{
    ...
    $.ajax(
    {
        url : 'mainServlet.html?id=' + jobids + urlstr + '&tsdtemp=' + Math.random(), datatype : 'xml', cache : false, 
        //如果值变化可能性比较大 一定要将缓存设成false
        timeout : 5000, async : false , //同步方式
        success : function (xml)
        {
...
.btn_2k3 {
	height:30px;BORDER-RIGHT: #87CEFA 1px solid; PADDING-RIGHT: 5px; BORDER-TOP: #87CEFA 1px solid; PADDING-LEFT: 5px; FONT-SIZE: 12px; FILTER: progid:DXstyle/images/publicTransform.Microsoft.Gradient(GradientType=0,StartColorStr=#FFFFFF, EndColorStr=#87CEFA); BORDER-LEFT: #87CEFA 1px solid; CURSOR: hand;COLOR: black; PADDING-TOP: 2px; BORDER-BOTTOM: #87CEFA 1px solid
}
</style>
  </head>
  ...
  <body>
		...
		<div id="navBar">
 	 	...
 	 	<!-- 用户操作标题以及放一些快捷按钮-->
		<div id="buttons">
			<button type="button" id="rgsl" onclick="openRgsl()">
				人工受理
			</button>
			<button type="button" id="deloper" onclick="deleteRows()">
				删除
			</button>
			<button type="button" id="sendjobs" onclick="sendJobs()">
				发送工单
			</button>
			<button type="button" onclick="printDisplay()" style="display: none">
				工单打印
			</button>
			...
			<button type="button" id="endjobs" disabled="disabled" onclick="endJobs()">
				确认完工
			</button>
			<button type="button" id="readjobs" onclick="readJobs()">
				查看流程
			</button>
					   ...
					   <form action="#" name="fault112fhform" id="fault112fhform" onsubmit="return false;">
						<table width="100%" id="tdiv" border="0" cellspacing="0" cellpadding="0">
								...
								<tr>
									...
									<td colspan="5">
										<textarea rows="6" id="testdatafuhe" cols="72" style="margin-left: 5px" disabled="disabled"></textarea>
									</td>
								</tr>
							</table>
						</form>
					   ...
					   <form action="#" name="fault112pxform" id="fault112pxform" onsubmit="return false;">
						<table width="100%" id="tdiv" border="0" cellspacing="0" cellpadding="0">
								...
								<tr>
									...
									<td >
										<textarea rows="6" id="testdatapaixiu" cols="72" style="margin-left: 5px" disabled="disabled"></textarea>
									</td>
								</tr>
		...
		<!-- 112障碍故障确认完工 -->
		<div class="neirong" id="fault112end" style="display: none;width: 720px">
				<div class="top">
					<div class="top_01" id="thisdrag">
							<div class="top_01left">
								<div class="top_02"><img src="style/images/public/neibut01.gif" /></div>
								<div class="top_03" id="fault112div">完工确认</div>
								<div class="top_04"><img src="style/images/public/neibut03.gif" /></div>
							</div>
					...
					<div class="midd" >
					   <form action="#" name="fault112endform" id="fault112endform" onsubmit="return false;">
						<table width="100%" id="tdiv" border="0" cellspacing="0" cellpadding="0">
								<tr>
									<td class="dflabelclass">
										维修结果
									...
									<td width="300px">
										<textarea rows="5" id="wxjg" cols="38" style="margin-left: 5px;margin-top: 5px;margin-bottom: 5px"></textarea>
										<font style="color: red">*</font>
									</td>
									<td class="dflabelclass">
										完工备注
									...
									<td width="300px">
										<textarea rows="5" id="wgbz" cols="38" style="margin-left: 5px;margin-top: 5px;margin-bottom: 5px"></textarea>
										<font style="color: red">*</font>
									</td>
								</tr>
							</table>
					...
					<div class="midd_butt">
						<button type="submit" class="tsdbutton" onclick="endJobs('end')">
							确认完工
						</button>
						<button type="submit" class="tsdbutton" id="fault112endclose" style="margin-left: 10px">
							关闭
						</button>
					</div>
		</div>
		...
		<div class="neirong" id="displayjobinfos" style="display: none;width:750px">
				<div class="top">
					...
					<div class="midd_butt">
						<button class="tsdbutton" id="infoclose" style="width: 70px;margin-left: 10px">
		...
		<div class="neirong" id="detailInfo"
				...
				<form action="#" name="detailInfoform" id="detailInfoform"
					...
					<table width="100%" id="tdiv" border="0"
						...
						<tr id='hx5'>
							...
							<td class="tdstyle" colspan="5">
								<textarea rows="3" cols="115" style="margin-top: 2px;"
			...
			<div class="midd_butt">
				<button type="button" class=btn_2k3 id="submithxinfo"
					onclick="addLineInfo()" style="width: 70px; margin-left: 10px">
					提交
				</button>
		...
		<div class="neirong" id="choosePrint" style="display: none;width:650px">
				<div class="top">
					...
					<div class="midd" style="background-color: #FFFFFF;height: 60px">
							<br />
					  	 	<button id="printDirect" class="tsdbutton" onclick="jobPrint(1)"><fmt:message key="broadband.zhijiePrint" /></button>
							<button id="printInDirect" class="tsdbutton" onclick="jobPrint(0)"><fmt:message key="broadband.yulanPrint" /></button>
							<br/>
					</div>	
					<div class="midd_butt">
