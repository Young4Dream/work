...
function getTabInfo()
{
	...
	params += '&skrs='+tsd.encodeURIComponent($('#cUserStatus').text());
            ...
            if(thisFlag == false){
            	//已最大业务类型个数生成数据
	           	...
	           	for(var i = 0 ; i < iLength ; i++){
	                ghTab += '<tr>';
	           		if(0 == i){
	           			ghTab += '<td rowspan="'+iCount+'"><fmt:message key="duty_new.dh"/></td>';//电话
	           		}
	           		//电话营收
	           		var sFee = sDhFee[i];
	            ...
	            if( '' != ghTab){//电话营收和业务流水统计
	            	...
	            	var sNav 	= '<tr class="trcss"><td style="width: 80px"><fmt:message key="duty_new.statetype"/></td><td style="width: 120px"><fmt:message key="duty_new.feetype"/></td><td style="width: 40px"><fmt:message key="duty_new.feecount"/></td>'+
	         		...
	         		var sTotal 	= '<tr class="trcss">'+
	         						'<td ><fmt:message key="duty_new.sumxiaoji"/></td><td>'+(sDhFeeCount/2)+'</td><td>'+(sDhFees.toFixed(2)/2)+'</td><td ><fmt:message key="duty_new.sumxiaoji"/></td>'+//小计
				    				'<td>'+sDhBusiCount+'</td><td>'+sDhBusiFees.toFixed(2)+'</td></tr>'+
				    			  '<tr style="font-weight: bold;">'+
				    				'<td colspan="4" ><fmt:message key="duty_new.sumheji"/></td><td>'+iTotalCount+'</td><td>'+iTotalFee.toFixed(2)+'</td></tr>';//总计 	
...
function dutyMx(rptName,ids,jobtype){
	...
	params += "&userid="+$('#cUserStatus').text();//阶段营收统计工号处理 
...
function addOperator(){
	...
	params += '&tend='+tEnd;
	//autoBlockForm('submitloading',20,'submitloadingclose',"#FFFFFF",false);
	var operators = fetchMultiArrayValue('Duty.queryOperators',6,params);
	//alert(operators);
...
//获取选择的工号
function getTheCheckedVals(name)
{
...
function chkSelected(){
	var res = getTheCheckedVals('useridchk');
	if('' == res){
				...
				<div class="title">
					<table cellspacing="4px" class="maintitle" align="left">
							<tr>
						    	...
						    	<td><fmt:message key="duty_new.selectstatusdate"/></td><!-- 请选择统计范围  -->
						    </tr>
					</table>
				</div>
	</div>	
	<div id="sIn" style="margin-top: 0;font-size: 14px">
		&nbsp;&nbsp;<!-- 时间范围 -->
		...
		<button id="dutybuttons" class="btn" onclick="printReport('<%=basePath %>','Duty')" style="display: none">
