...
<script type="text/javascript">
		...
		*/
		jQuery(document).ready(function () {
			loadNodeTable(1,'','');//加载端子面板
		});
		/**
		* 加载端子面板
		* @param type 1：初始化加载    2：查询加载
		* @param nodename ：端子名称  用于进行模糊查询
		* @param dh ：电话号码  用于进行模糊查询
		* @return 
		*/
		function loadNodeTable(type,nodename,dh){
			$('#pagered1').html("");
					
			//var NodeState = 'free';+'&NodeState='+NodeState    ;
			...
			if(type==1){
				...
				queryparams += encodeURIComponent("NodeName like '%") + nodename + encodeURIComponent("%'");
				queryparams += " and "+encodeURIComponent("dh like '%") + dh + encodeURIComponent("%'");
			}
			
			params =urlstr+queryparams+'&DeviceNo='+tsd.encodeURIComponent(DeviceNo)+'&devicetype='+tsd.encodeURIComponent(devicetype)+'&NodeState='+NodeState;			
		...
		*/
		function selectAndClose(){
			...
			var pdeviceno = $('#DeviceNo').val()//父节点编码
			if(NodeName!=''&&null!=NodeName){
				window.dialogArguments.getTheSelected(NodeName,DeviceNo,pdeviceno);  //增加参数存储父级设备编号
				window.close();
			}else{
				alert('请选择设备!');
			}
		...
		*/
		function queryNode(){
			var nodenameQ = $("#nodenameQ").val();
			var dhQ = $("#dhQ").val();
			if(nodenameQ=='' && dhQ==''){
				alert("请输入查询条件。");
				return false;
			}
			loadNodeTable(2,nodenameQ,dhQ);	//加载端子面板
		}
	...
	<body style="width: 99.5%">
		...
		<div id="querydiv" style="margin-left: 60px;margin-bottom: 10px;float: left;">
			<label>端子名称：</label>
			<input type="text" id="nodenameQ" name="nodenameQ"/>
			<lable>电话号码：</lable>
			<input type="text" id="dhQ" name="dhQ"/>
			<button type="button" style="margin-left: 10px;margin-bottom: 0px;height: 22px;" class="tsdbutton"
				onclick="queryNode()">
				查询
			</button>
		</div>
