<%------------------------------------------ 
    File Name:  trunk/boss-bs/src/tsdboss/WebRoot/WEB-INF/template/telephone/phonectrl/WaitBusinesRes.jsp
    Function:   电话待控制业务用户
    Author:     wenxuming
    Date:       2011-04-25
    Description: 
...
<html>
	<head>
		...
		jQuery(document).ready(function()
		{
		    ...
		    getywslUserPower();
		    Dat = loadData("dhyw_waitctl","zh",1,'选择操作');		
		    Dat.setHidden(['Ywdm']); 
		    gettabletitle();
		    getJqgridDate();
		    //gettabledate();
		...
		function gettabledate(query){
			...
			if(query=="query"){
				params="&key="+$("#fusearchsql").val()+"and d.jhj_id=j.jhjid and d.CtlType=t.tident and t.tsection='ctltype'";
			}else{
				params="&key=1=1 and d.jhj_id=j.jhjid and to_char(d.CtlType)=t.tident and t.tsection='ctltype'";
			}
			fieldname=fieldname.replace('Jhj_ID','j.jhjname');
			fieldname=fieldname.replace('CtlType','t.tvalues');
			...
			var res = fetchMultiArrayValue("dbsql_phone.querytabledate",6,"&cloum="+fieldname+params+"&tablename=dhyw_waitctl d,jhjid j,tsd_ini t");
			if(res==""||res==undefined||res=="undefined"){alert("没有数据信息！");return;}
			...
			for(var i=0;i<res.length;i++){
				...
				querystr += "<input type=\"radio\" id='v_bytctab_check"+res[i][1]+"' name='v_wsr_radio'  onClick=\"getradiovalue();\"  value='"+res[i][1]+","+res[i][10]+","+res[i][3]+","+res[i][6]+"' style='width:20px;'/>";
				querystr += "</center></td>";
				for(var j=0;j<Dat.FieldName.length;j++){
					if(Dat.FieldName[j]=="Ywdm"){
		...
		function getradiovalue(){
			...
			$("#radioYwdm").val(array[1]);//业务代码
			$("#radioCtlType").val(getCtlTypename(array[2]));///控制类型
			$("#radioCtlMan").val(array[3]);//控制者
		}
		...
		*/
		function BatchOpensave(){			
			...
			for(var ii=0;ii<Batchall.length;ii++)
			{
				...
				var tmp_Ywdm = $("#editgrid").getCell(Batchall[ii],"Ywdm");
				var tmp_CtlType = $("#editgrid").getCell(Batchall[ii],"CtlType");
				var tmp_CtlMan = $("#editgrid").getCell(Batchall[ii],"CtlMan");
				if(tmp_CtlType=="开通"){
					tmp_CtlType=1;
				}else if(tmp_CtlType=="取消"){
					tmp_CtlType=0;
				}
				params="&Dh="+tsd.encodeURIComponent(tmp_dh)+"&Ywdm="+tsd.encodeURIComponent(tmp_Ywdm)+"&CtlType="+tsd.encodeURIComponent(tmp_CtlType)+"&CtlMan="+tsd.encodeURIComponent(tmp_CtlMan)+"&userid="+tsd.encodeURIComponent($("#userid").val());
				//alert(params);
				var insertRes = executeNoQuery("dbsql_phone.insertDhywWaitctlTmp",6,params);
        		...
        		{
        			var resboolean = executeNoQuery("dbsql_phone.insertDhywWaitctlTmp",6,params);
        		}
				if(null==tmp_dh)
				{
			...
			var result = fetchMultiPValue("WaitBusinesRes.dhyw_Adjust_Ok",6,paramsStr);
		...
		function dcfield(){
			thisDownLoad('tsdBilling','mssql','dhyw_waitctl',$("#languageType").val(),5);
		}
		...
		function DownSure()
		{
			getTheCheckedFields('tsdBilling','mssql','dhyw_waitctl');
		}
		
		function saveupdate(){
			var dh = $("#radiophone").val();//电话dh
			if(dh==""){alert("请选择一个电话！");return;}
			var Ywdm = $("#radioYwdm").val();//业务代码
			var CtlType = $("#radioCtlType").val();//控制类型			
			var CtlMan = $("#radioCtlMan").val();//控制者
			var userid = $("#userid").val();
			var sfOperType = $("#sfOperType").val();
			var bz = $("#Bz").val().replace(/(^\s*)|(\s*$)/g,"");
			if(sfOperType==""){alert("请选择操作类型！");return;}
			if(bz==""){alert("请输入备注信息！");return;}
			var params="";
			params +="&opentype=Singleopen";
			params +="&Dh="+dh;
			params +="&Ywdm="+tsd.encodeURIComponent(Ywdm);//业务代码
			params +="&CtlType="+tsd.encodeURIComponent(CtlType);//控制类型
			params +="&CtlMan="+tsd.encodeURIComponent(CtlMan);//控制者
			...
			var result = fetchMultiPValue("WaitBusinesRes.dhyw_Adjust_Ok",6,params);
			if(result[0]==undefined || result[0][0]==undefined || result[0][0]=='-1')
			{
				alert("操作失败！");
				return;
		...
		function getCtlTypename(key){
			var res = fetchSingleValue("dbsql_phone.querytabledate",6,"&tablename=tsd_ini&cloum=TIDENT&key=tsection='ctltype' AND tvalues='"+tsd.encodeURIComponent(key)+"'");
			return res;
		}
	...
	<script type="text/javascript">
		function getJqgridDate(query){
			...
			var queryphone = $("#queryphone").val();
			if(query=="query"){
				params="&key="+$("#fusearchsql").val()+"  or dh in(select phys_phone from hmzy_physical)";
			}else{
				params="&key=1=1";
			}
			if($("#areajhj_idright").val()=="false"){
			...
			jQuery("#editgrid").jqGrid({
						url:'mainServlet.html?'+urlstr + "&cloum=Dh,"+fieldname+params+"&tablename=dhyw_waitctl",
						datatype: 'xml',
						...
						loadComplete:function(){
							var ids = $("#editgrid").getDataIDs();
								for(var i=0;i<ids.length;i++)
								{
									...
									var CtlType = $("#editgrid").getCell(ids[i],"CtlType");
									var CtlTypename = gettsd_ini(CtlType);
									if(CtlTypename!=undefined)
									{
										$("#editgrid").setRowData(ids[i],{"CtlType":CtlTypename});
									}
								}
							addRowOperBtn('editgrid',"<span style='color:red'>单选操作</span>",'Operation','Dh','click',1,'Ywdm','CtlType','CtlMan');			        											        	
		...
		function gettsd_ini(key){
			var res = fetchSingleValue("dbsql_phone.querytabledate",6,"&tablename=tsd_ini&cloum=TVALUES&key=tsection='ctltype' AND TIDENT='"+tsd.encodeURIComponent(key)+"'");
			return res;
		}
		
		function Operation(DH,Ywdm,CtlType,CtlMan){
			//alert(DH+","+Ywdm+","+getCtlTypename(CtlType)+","+CtlMan);
			$("#radiophone").val(DH);//电话dh
			$("#radioYwdm").val(Ywdm);//业务代码
			$("#radioCtlType").val(getCtlTypename(CtlType));///控制类型
			$("#radioCtlMan").val(CtlMan);//控制者
			savefrom();
		}
	</script>
  </head>
  <body>
	...
	<div class="neirong" id="saveupdate" style="width:478px;display: none">
			...
			<div class="midd" style="background-color:#FFFFFF;height:50px;">
				<div id="inputtext">
					<table border="0" cellpadding="0" bordercolor="" id="" style="width: 466px;"  cellspacing="3">
						<tr>
	...
	<!-- 导出数据 -->
	<div class="neirong" id="thefieldsform" style="display: none;width:760px">
		<div class="top">
			<div class="top_01" id="thisdrag">
					<div class="top_01left">
						<div class="top_02"><img src="style/images/public/neibut01.gif" /></div>
		...
		<div class="midd_butt" style="width:743px">
	...
	<input type="hidden" id="radioYwdm"/>
	<input type="hidden" id="radioCtlType"/>
	<input type="hidden" id="radioCtlMan"/>
