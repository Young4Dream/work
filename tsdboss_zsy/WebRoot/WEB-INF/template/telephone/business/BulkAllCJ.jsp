...
<html>
	<head>
		...
		**********/
		function cxJob()
		{
			...
			{
				var res = fetchMultiPValue("PhoneService.BulkallCJ",6,params);							
	...
	/**************
		 function getshow(){
			if($("#phoneshowcheck").attr("checked")){
				$("#phonevalue").show();
		...
		function getphonevalue(key){
			var res = fetchMultiArrayValue("dbsql_phone.bulkallcj_queryphone_dh",6,"&dhvalue="+key,'business');
			...
			if(res==""||res==undefined){
				...
				$("#phone_dh").html(res[0][0]);				
				$("#phone_yhmc").html(res[0][1]);
				$("#phone_bm1").html(res[0][2]);
				$("#phone_reg_date").html(res[0][3]);
				$("#phone_tjbz").html(res[0][4]);
				$("#phone_usertype").html(res[0][5]);
				$("#phone_bz8").html(res[0][6]);
				$("#phone_bz9").html(res[0][7]);
				$("#phone_dhgn").html(res[0][8]);
				$("#phone_yhdz").html(res[0][9]);
				$("#phone_bz3").html(res[0][10]);
			}
			autoBlockForm('queryphonefrom',5,'close',"#ffffff",false);//弹出查询框 
		}
	*************/
	  ...
	  function getdhcountvalue(){		
			...
			$("#dhcount").text("当前拆机电话数量："+rescount+"个");
		}
	</script>
	</head>
	<body>
	   ...
	   <div id="input-Unit" style="margin-bottom:0px;">			
		...
		<table id="tdiv" border="0"
			...
			<tr>
				...
				<td width="20%" class="tdstyle">
					<input type="text" id="phoneBalance" style="width:150px;" disabled/> 
				</td>
				<td class="labelclass">
					<label for="sarea">
						<span id="">实时余额</span>
				...
				<td width="20%" class="tdstyle">
					<input type="text" id="mothshuafei" style="width:150px;" disabled/> 
				</td>
				<td class="labelclass">
				</td>
				<td width="20%" class="tdstyle">
	...
	<div id="buttons" style="text-align:center">
		<button id="kdcx" style="width:120px;" onClick="cxJob()" disabled>批量拆机</button><button id="kdback" style="width:120px;" onClick="respage()"><fmt:message key="AddUser.Reset" /></button>
	</div>		
