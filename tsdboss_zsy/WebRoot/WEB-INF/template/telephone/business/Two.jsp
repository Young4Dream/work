...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>    
	...
	<style type="text/css">
		...
		#left_k{width:175px;height:330px;float:left;clear:both;overflow-x:scroll;border:1px solid #99ccff;}	
		#right_k{width:515px;height:330px;float:right;overflow-x:hidden;border:1px solid #99ccff;}
	...
	*/	
	...
	$(function(){
		//见装机页面填写的标头放到空号选好权限中		
		var nhead = parseUrl(document.location.search,"DhHead","");
		$("#kgquery").val(nhead);		
		...
		Dat.setWidth({Dh:153,Dh_Level:152});
		setUserRight();
		//loadNullGrid("","");
		getData();
		...
		**********/
		function getRefresh(){
			...
			var urlstrto=tsd.buildParams({ packgname:'service',//java包
									...
									datatype:'xml',//返回数据格式 
									tsdpname:'PhoneKaiHu.QuKongHao'
									});						
			$("#editgrid").setGridParam({url:'mainServlet.html?'+urlstrto+params}).trigger("reloadGrid");		
		}
		...
		**********/
		function getData()
		{
			...
			var urlstr=tsd.buildParams({packgname:'service',//java包
		                   ...
		                   $.ajax({
					              url:'mainServlet.html?'+urlstr+'&userid=' + "<%=userid%>" + '&area='+tsd.encodeURIComponent($("#area").val()),
					              datatype:'xml',
		...
		**********/	
		...
		{	
			...
			params = params + "&Dh_Level_Min=" + $("#dj_d").val();
			params = params + "&Dh_Level_Max=" + $("#dj_g").val();
  ...
  <body>
		<div id="wrapper">
			...
			<div id="bottom_t">
			  <ul>
				<li>
						<fmt:message key="phone.getinternet0441" /><!-- 电话等级 -->
						<!-- <select name="dj_d" id="dj_d" onchange="getRefresh('1')"></select> -->
						<select name="dj_d" id="dj_d" onchange="getRefresh();"></select>
					</li>
					<li>
						--
					</li>
					<li>
						...
						<select name="dj_g" id="dj_g" onchange="getRefresh();"></select>
					</li>
					<li>
						<fmt:message key="phone.getinternet0442" /><!-- 选号限制 -->
						<input type="text" style="width:100px" id="kgquery"/>
					</li>					
					<li>
