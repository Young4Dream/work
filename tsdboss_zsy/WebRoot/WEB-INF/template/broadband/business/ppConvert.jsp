<%-- 
	File Name:  ppConvert.jsp
    Function:   宽带业务受理（公私互转）
    Author:     wenxuming
    Date:       2009-01-01
    Description: 宽带公私互转
    Modify:     2010-03-21  
...
<html>
	<head>
		...
		<!-- 页面国际化 -->
		<script type="text/javascript"
			src="script/broadband/business/Internationalization.js"></script>
		<script type="text/javascript"
...
.neirong .midd #queryHTHdw tr{line-height:18px;}
		...
		function lastApplyInfo(UserName)
		{
			...
			else
			{
	            ...
	            function userChoose(UserName)
	            {
                  ...
                  if(param>0){
                    alert("<fmt:message key="broadband.gyhwwgzbnblcyw" />");
                    return false;
                  }
                  /*由于客户需要可以办理此业务了
                  var par=isnotnewbusi(UserName);    
                  if(par==1){
                    alert("该用户已经办理了套餐业务["+packagenameto+"],不能在办理公私互转业务！");
                    return false;
                  }*/
        	...
        	var urlstr=tsd.buildParams({ packgname:'service',
							...
							$.ajax({
								url:'mainServlet.html?'+urlstr+"&busiclass="+'broadband'+"&nubmer="+UserName+"&busitype="+'privatetopublic',
								datatype:'xml',
								cache:false,//如果值变化可能性比较大 一定要将缓存设成false
								timeout: 1000,
								async: false ,//同步方式
        	...
        	function jobPrint(flag)
        	{
        		...
        		var urll = "/tsdboss/ReportServer?reportlet=/com/tsdreport/broadband_jobadduser.cpt"+params;
        		//alert(lsz);
        		if(flag=="1")
        		{
        	...
        	function lsPrint(flag)
        	{
        		...
        		var urll = "/tsdboss/ReportServer?reportlet=/com/tsdreport/broadband_sf.cpt"+params;
        		//alert(lsz);
        		if(flag=="1")
        		{
        		...
        		{
        			window.showModalDialog(urll,window,"dialogWidth:800px; dialogHeight:409px; center:yes; scroll:no");
        		}
        		setTimeout($.unblockUI,15);
        	}
			...
			var res = fetchMultiPValue("kd.Lock",0,"&userid="+$("#userid").val()+"&account="+UserName+"&businesstype=privatetopublic&flag=in"+"&realname="+tsd.encodeURIComponent(rel));
	...
	///////////////////////////////////////////////////////////////// 
	      ...
	      var urlstr=tsd.buildParams({ 	packgname:'service',//java包
			...
			$.ajax({
					...
					success:function(xml){
							...
							$("#iiFeeType").html("<option value=''></option>");
			...
			$.ajax({
					...
					success:function(xml){
						...
						$(xml).find('row').each(function(){						 
							...
							$("#speedyc").val(speed);//将没转换的带宽加载带隐藏域中
							...
							//显示计费规则（更改后）的下拉菜单
		                    aQueryObj(sDh1);
							sDh1 = parseInt(sDh1,10);
							if(sDh1==1){
							  //私费用户
							  var sdh1 = getusertype(sDh1);
							  $("#sDh1").val(sdh1);
							  $("#y_h_l_x").val(sdh1);
							  $("#y_w_l_x").val("私费转公费");
							  $("#GFtype").show();
							  $("#GFtype").html("<option value='"+'2'+"' selected=true>"+'公费用户'+"</option>");
							  var gft = "<option value="+'4'+">"+'公寓用户'+"</option>";
							  $("#GFtype").html($("#GFtype").html()+gft);
							  $("#GFtypename").text("公费类型");
							  $("#GFtypename").show();
							  $("#etid").hide();
			                  $("#effecttime").hide();
			                  $("#effecttime").empty();
							  $("#closesave").removeAttr("disabled");
							}else if(sDh1==2||sDh1==4){
							  //公费用户
							  var sdh1 = getusertype(sDh1);
							  $("#sDh1").val(sdh1);
							  $("#y_h_l_x").val(sdh1);
							  ...
							  $("#etid").show();
							  $("#effecttime").show();
							  $("#effecttime").html("<option value='"+'1'+"' selected=true>"+'次月生效'+"</option>");
							    var ist = "<option value="+'2'+">"+'立即生效'+"</option>";
							    $("#effecttime").html($("#effecttime").html()+ist);
							  $("#closesave").removeAttr("disabled"); 
							  $("#GFtype").hide();
							  $("#GFtype").empty();
							   $("#GFtypename").hide();
	          ...
	          function getusertype(sDh1){
	            var res = fetchMultiArrayValue("telJobbroadband.usertype",0,"&TypeID="+sDh1);
	            return res;
	          }
	          ...
	          function isnotdecment(DeptName){	  
	           ...
	           var result="";
	               var urlstr=tsd.buildParams({packgname:'service',//java包
						 					...
						 					tsdoper:'query',//操作类型 
						 					datatype:'xmlattr',
						 					tsdpk:'ppConvert.shifoupublie'//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
						 				});
			                $.ajax({
					              url:'mainServlet.html?'+urlstr+'&UserName='+UserName,
					              datatype:'xml',
	           ...
	           */
	           ...
	           var params=publicshifou();
	           if(params>0){
	              alert("<fmt:message key="broadband.gyhjblbnzcblcyw"/>");
	              $("#kdSearchBox").val("");
	              clearText('form1');
	              $("#kdSearchBox").select();
	              $("#kdSearchBox").focus();
	            ...
	            //执行修改操作
		           ...
		           var sbm2 = $("#sBm2").val();
				 	if($('#sBm2 option').size()>1&&sbm2==""&&sbm1!=""){
	               ...
	               var pat=/[@#\$%\^&\*]+/g; 
			        if(pat.test(jobmemo)) 
			        { 
			         alert("<fmt:message key="broadband.srdbzzbnyffzf"/>"); //输入的备注中不能有非法字符!
			         $("#jobmemo").select();
			         $("#jobmemo").focus();
			         return false; 
			        }
	               var funcccc = "";
	               if($("#y_w_l_x").val()=="公费转私费")
	               {
	               		funcccc = "publictoprivate";
	               }
	               else
	               {
	                   ...
	                   fee5 = parseInt(fee5,10);
	               	   if((reamainfee-fee5)!=0){
	               		  alert("<fmt:message key="broadband.qxdyyckjsfy"/>");
	               		  $("#kdSearchBox").select();
	               		  $("#kdSearchBox").val("");
	               		  clearText('form1');
	               		  $("#iiFeeType").attr("disabled","disabled");
				   ...
				   var speed1 = $("#speedto").val();//当前带宽
	               var feei = $("#feei").val();
	               if(tsd.Qualified()==false){return false;}
	               //if(iiFeeType.length==0){alert("请选择新计费规则");$("#kdSearchBox").select();return false;};
	               var iiFeeTypename = $("select[id='iiFeeType'] :selected").html();
	               var ift="";
	               if(iiFeeType!=""){
	                 ift = "且计费规则更新为："+iiFeeTypename;
	               }else{
	               	 iiFeeType=toiFeeType;
	               }
	               //付费方式
				   ...
				   var hthbm = $("#hth").val();
	               var gftype = $("#GFtype").val();//私费转公费的选择的公费类型
	               if(confirm("<fmt:message key="gjh.nqdj"/>用户["+username+"]办理此业务"+ift+"")){
	               ...
	               sBm2 = tsd.encodeURIComponent(sBm2);
	               sBm3 = tsd.encodeURIComponent(sBm3);
	               sBm4 = tsd.encodeURIComponent(sBm4);
	               var paymode = tsd.encodeURIComponent(payitem);
				   var ispay = tsd.encodeURIComponent(ispay);
				   var hth = tsd.encodeURIComponent(zzhth);
				   ifurgent = tsd.encodeURIComponent(ifurgent);
				   jobmemo = tsd.encodeURIComponent(jobmemo);
				   var hthbm = tsd.encodeURIComponent(hthbm);
				   gftype = tsd.encodeURIComponent(gftype);
	               var urlstr=tsd.buildParams({packgname:'service',//java包
						 					   clsname:'Procedure',//类名
						 					   method:'exequery',//方法名
						 					   ds:'broadband',//数据源名称 对应的WEB-INF/classes/ApplicationContent.xml中配置的数据源
						 					   tsdpname:'ppConvert.radpublicprivate',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
						 					   tsdExeType:'query',
						 					   datatype:'xmlattr',
									 	       tsdUserId:'true'
						 				      });					 					
			       $.ajax({
					url:'mainServlet.html?'+urlstr+'&fee='+feei+'&userid='+userid+'&area='+area+'&depart='+depart+'&username='+username+'&prefeetype='+toiFeeType+'&newfeetype='+iiFeeType+'&func='+funcccc+'&afeets='+bafee+'&ameno='+bameno+'&sfeedk='+bsfee+'&smemo='+bsmemo+'&newspeed='+speed1+'&feename='+queryFeeInfo()+"&sbm="+sBm+"&sbm2="+sBm2+"&sbm3="+sBm3+"&sbm4="+sBm4+"&fecttime="+effecttime+"&paymode="+paymode+"&ispay="+ispay+"&hth="+hth+"&ifurgent="+ifurgent+"&descrpt="+jobmemo+"&hthbm="+hthbm+"&gftype="+gftype,
					datatype:'xml',
				    cache:false,//如果值变化可能性比较大 一定要将缓存设成false
				    timeout: 1000,
					async: false ,//同步方式
					...
					$(xml).find('row').each(function(){
						...
						if(result==null){
						   ...
						   $("#printfeeee").val(feei);
						      $.ajax({
								       url:'jobcode.html?'+"&telJobType="+funcccc+"&docId="+result+"&userid="+$("#userid").val()+"&transitionName=",
								       datatype:'xml',
								       cache:false,//如果值变化可能性比较大 一定要将缓存设成false
								       timeout: 1000,
								       async: false ,//同步方式
								       success:function(xml){
									        $(xml).find("rows>row").each(function(i){
									          var tsdto = $(this).find("admin").text();
						  ...
						  var timetype;
						  if(funcccc=="publictoprivate"){
						     if(effecttime=='1'){
						       timetype="次月生效";
						     }else if(effecttime=='2'){
						       timetype="立即生效";
						     }
						  }else{
						      timetype='';
						  ...
						  loginfo += username;
						  loginfo += ";";
						  loginfo += tsd.encodeURIComponent("原用户类型:");
						  ...
						  loginfo += ";";
						  loginfo += tsd.encodeURIComponent("生效日期:");
						  loginfo += tsd.encodeURIComponent(timetype);
						  loginfo += ";";
						  ...
						  $("#kdSearchByUserName").select();
			              $("#kdSearchByUserName").focus();
			              $("#iiFeeType").attr("disabled","disabled");
			              $("#closesave").attr("disabled","disabled");
			              $("#etid").hide();
		                  $("#effecttime").hide();
		...
		var urlstr=tsd.buildParams({packgname:'service',//java包
		                  ...
		                  $.ajax({
					              url:'mainServlet.html?'+urlstr+'&bname='+tsd.encodeURIComponent('publicprivet'),
					              datatype:'xml',
					              cache:false,//如果值变化可能性比较大 一定要将缓存设成false
					              timeout: 1000,
					              async: false ,//同步方式
           ...
           var TextUtil = new Object(); 
            TextUtil.NotMax = function(oTextArea){ 
                ...
                if(oTextArea.value.length > maxText){ 
                        ...
                        alert("<fmt:message key="broadband.ccxz"/>"); //超出限制
                } 
            }
    </script>
	...
	<body>
						...
						<tr>
							<td>
								...
								<select id="kdSearchWay" onchange="closequery()">
									<option value="0">
										<fmt:message key="broadband.account" /><!-- 账号 -->
									</option>
									<option value="1">
										<fmt:message key="broadband.phone" /><!-- 电话 -->
									</option>
									<option value="2">
										<fmt:message key="broadband.username" /><!-- 用户名 -->
									</option>
								</select>
							</td>
							<td>
							...
							<td>
								<button class="tsdbutton" id="kdSearchByUserName"
									onclick="kdSearch()">
									<fmt:message key="Revenue.bSearch" /><!-- 查找 -->
								</button>
							</td>
						</tr>
					</table>
					...
					<div id="inputtext">
						...
						<div id="inputtext">
							<table cellspacing="8px">
								<tr>
									<td>
										<fmt:message key="broadband.dqyhlx" /><!-- 当前用户类型 -->
									</td>
									<td>
										<input type="text" name="y_h_l_x" id="y_h_l_x"
											disabled="disabled" style="width: 135px" />
									</td>
									<td>
										<fmt:message key="broadband.kblyw" /><!-- 可办理业务 -->
									</td>
									<td>
										<input type="text" name="y_w_l_x" id="y_w_l_x" disabled="disabled" style="width: 135px" />										
									</td>
									<td>
										<span id="GFtypename"></span>
									</td>
									<td>
										<select id="GFtype" style="width:130px"></select>
									</td>
									<td id="etid">
										<fmt:message key="broadband.sxtime" /><!-- 生效时间 -->
									</td>
									<td>
										<select name="effecttime" id="effecttime" style="width: 90px">
										</select>
									</td>
								</tr>
								<tr>
									...
									<td>
										<input type="text" name="iFeeType" id="iFeeType"
											disabled="disabled" style="width: 135px"></input>
									</td>
									<td>
										<fmt:message key="BillingBG.BillingGZGGH" />
									</td>
									<td colspan=2>
										<select name="iiFeeType" id="iiFeeType" onchange="setspeed()"
											style="width: 135px">
											<option value=""></option>
										</select>
									</td>
						...
						<div id="inputtext">
							...
							<tr>
								...
								<select name="sBm" id="sBm" style="width: 130px"
									onchange="addsBmer()" disabled="disabled">
								...
								<td>
								<select name="sBm2" id="sBm2" style="width: 130px"
									onchange="addsBmsan()" disabled="disabled">
								</select>
								</td>
							</tr>
							<tr>
							   ...
							   <td>
								<select name="sBm3" id="sBm3" style="width: 130px"
									onchange="addsBmsi()" disabled="disabled">
								</select>
							   </td>
							   <td>
							...
							<tr>
								...
								<td>合同号</td>
								<td>
								  <input type="text" name="hth" id="hth" disabled="disabled" style="width: 130px"/>
								</td>	
							</tr>
						  </table>	
						</div>
		...
		<div style="display: none">
			...
			<input type="hidden" id="speedyc" name="speedyc"/>			
			<!-- 是否可以不选一次性费用项 -->
				...
				<div class="top_01" id="thisdrag">
					<div class="top_01left">
						...
						<div class="top_03" id="titlediv">
							<fmt:message key="broadband.gshzUserNamequery" /><!-- 宽带公私互转用户信息查询 -->
						</div>
						<div class="top_04">
		...
		<div class="neirong" id="queryHTH"
			...
			<div style="width: 525px; height: 250px; overflow-y: scroll; overflow-x: scroll; border-top: 1px solid gray; border-left: 1px solid gray;">			   						
				<table border="1" cellpadding="0" bordercolor="#9AC0CD" id="queryHTHdw" style="width: 510px;">										
				</table>
			</div>
				<div id="buttons">
				<center>
