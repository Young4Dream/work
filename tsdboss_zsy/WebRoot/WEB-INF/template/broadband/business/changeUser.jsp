...
<%-- 
	File Name:  changeUser.jsp
    Function:   宽带业务受理（更名）
    Author:     wenxuming
    Date:       2009-01-01
    Description: 宽带更名
    Modify:    	2010-12-14：霍帅   打印方法改为通用方法"printThisReport1(...)"  
 --%>
<%
...
<!-- 导入的js文件 -->
	...
	/////////////////////////////////////////////////////////////////	 
		 ...
		 var urlstr=tsd.buildParams({ 	packgname:'service',//java包
			...
			$.ajax({
	           ...
	           //跟新地址变更旧地址
	           function userUpdate(){
	               if(username==undefined){alert('<fmt:message key="userchanges.qsrcxnr"/>');$("#kdSearchBox").select();$("#kdSearchBox").focus();quxiao();return false;}
	               ...
	               //当checkbox的name值一样的话来区分checkbox那个是否选中状态
					...
					if(checkedBox=="" && $("input[type='checkbox'][name='checkbox']").size()>0)
					{
						...
						{
							alert("<fmt:message key="broadband.qxzzsyxycxfy" />");//请至少选择一项一次性费用项！
							return false;
						}
						else if($("#ableForNoChoose").val()=="true")
						{
							if(confirm("<fmt:message key="broadband.sfqrbxycxfy"/>"))//是否确认不选一次性费用项
							{
								//确认不选
							}
							else
	               ...
	               srealname2 = $.trim(tsd.encodeURIComponent(srealname2));
	               ...
	               if(srealname2==""){alert("<fmt:message key="broadband.qsrxdyhm"/>");$("#sRealName2").select();$("#sRealName2").focus();return false;}
	               
	               if(srealname==srealname2){alert("<fmt:message key="broadband.xxmbngyxmxt"/>");$("#sRealName2").select();$("#sRealName2").focus();return false;}
	               //sAddress1 = tsd.encodeURIComponent(sAddress1);    
			 	   if(confirm("<fmt:message key="broadband.nqdjyxm"/>["+decodeURIComponent(srealname)+"]<fmt:message key="broadband.ggwxxm"/>["+decodeURIComponent(srealname2)+"]?")){
	               var urlstr=tsd.buildParams({ packgname:'service',//java包
							 					clsname:'Procedure',//类名
							 					method:'exequery',//方法名
							 					ds:'broadband',//数据源名称 对应的WEB-INF/classes/ApplicationContent.xml中配置的数据源
							 					tsdpname:'changeUser.gh',//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定
							 					tsdExeType:'query',
							 					datatype:'xmlattr',
										 	    tsdUserId:'true'
							 				   });//执行的主Sql语句的配置名，应选取tsdcf中配置的文件里指定		 					
			       $.ajax({
					url:'mainServlet.html?'+urlstr+'&userid='+userid+'&area='+area+'&depart='+depart+'&username='+username+'&oldrealname='+srealname+'&newrealname='+srealname2+'&fee='+feei+'&feename='+queryFeeInfo()+"&paymode="+paymode+"&ispay="+ispay+"&hth="+hth+"&ifurgent="+ifurgent+"&descrpt="+jobmemo,
					datatype:'xml',
				    cache:false,//如果值变化可能性比较大 一定要将缓存设成false
				    timeout: 1000,
					async: false ,//同步方式
					...
					$(xml).find('row').each(function(){
						...
						if(result==null){
						   alert('<fmt:message key="gjh.eiterfailure"/>')
							...
							$.ajax({
								   url:'jobcode.html?'+"&telJobType="+'changeuser'+"&docId="+result+"&userid="+$("#userid").val()+"&transitionName=",
								   datatype:'xml',
								   cache:false,//如果值变化可能性比较大 一定要将缓存设成false
								   timeout: 1000,
								   async: false ,//同步方式
						  ...
						  loginfo += username;
						  loginfo += ";";
						  loginfo += tsd.encodeURIComponent("原姓名:");
						  loginfo += srealname;
						  loginfo += ";";
						  loginfo += tsd.encodeURIComponent("新姓名:");
						  loginfo += srealname2;
						  loginfo += ";";
		...
		var urlstr=tsd.buildParams({packgname:'service',//java包
		                   ...
		                   $.ajax({
					              url:'mainServlet.html?'+urlstr+'&bname='+tsd.encodeURIComponent('changeuser'),
					              datatype:'xml',
					              cache:false,//如果值变化可能性比较大 一定要将缓存设成false
					              timeout: 1000,
					              async: false ,//同步方式
    ...
    <script language="javascript">
             ...
             function userChoose(UserName)
             {
                 ...
                 if(par==1){
                    alert("<fmt:message key="broadband.gyhyjblltcyw"/>["+packagenameto+"],<fmt:message key="broadband.bnzblghyw"/>");
                    return false;
                 }
        	...
        	var urlstr=tsd.buildParams({ packgname:'service',
							...
							$.ajax({
								url:'mainServlet.html?'+urlstr+"&busiclass="+'broadband'+"&nubmer="+UserName+"&busitype="+'changeuser'+"&usertype="+usertype,
								datatype:'xml',
								cache:false,//如果值变化可能性比较大 一定要将缓存设成false
								timeout: 1000,
								async: false ,//同步方式
          ...
          function quereyshiftj(){
			...
			var res = fetchMultiPValue("kd.Lock",0,"&userid="+$("#userid").val()+"&account="+UserName+"&businesstype=changeuser&flag=in"+"&realname="+tsd.encodeURIComponent(rel));
				...
				<form action="#" onsubmit="return false;" name="form1" id="form1" method="post">
						...
						<div id="input-Unit">
							<div id="inputtext">
								...
								<div id="inputtext">
									<table style="margin-left:10px;">
										<tr>
											<td width="80">
												<fmt:message key="BillingBG.name" />
												<!-- 用户姓名 -->
											</td>
											<td width="160">
											...
											<td width="80">
												<fmt:message key="broadband.newuser"/><!-- 新用户名 -->
											</td>
											<td width="250">
												<input type="text" name="sRealName2" id="sRealName2" disabled="disabled" style="width: 130px" />
											</td>
										</tr>
									</table>
								</div>
							...
							<div>
								<table cellpadding="5px" cellspacing="5px">
									<tr>
										...
										<td>
										  <input type='checkbox' name="transferHTHbox" id="transferHTHbox" style="width: 22px; padding: 0px;" onclick="isnotdanwei()" disabled="disabled"/>
										</td>
							...
							<div id="buttons">
								...
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<button id="closesave" onclick="userUpdate()"
									disabled="disabled">
									<!-- 确定 -->
				...
				<div class="top_01" id="thisdrag">
					<div class="top_01left">
						...
						<div class="top_03" id="titlediv">
							<fmt:message key="broadband.guohuUserNamequery" /><!-- 宽带过户用户信息查询 -->
						</div>
						<div class="top_04">
