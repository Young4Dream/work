...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <head>    
	...
	function queryRadUser_equalReal(fieldname,fieldvalue){
		...
		if (res != undefined && res!="" && res.length>0){
			var conval=$("#queryfield").val();
			if(conval=="internetacct"){
				$("#queryvalue").val(res[0][0]);
			}else if(conval=="username"){
				$("#queryvalue").val(res[0][1]);
			}else if(conval=="useraddr"){
				$("#queryvalue").val(res[0][2]);
			}else if(conval=="dh"){
				$("#queryvalue").val(res[0][20]);
			}else if(conval=="mobile"){
				$("#queryvalue").val(res[0][17]);
			}else if(conval=="area"){
				$("#queryvalue").val(res[0][24]);
			}
	...
	//根据查询结果数组，给控件赋值
		...
		function queryAuthor(){
			...
			var res = fetchMultiArrayValue_rad('dbsql_rad.queryAuthorinfo',8,"&username="+$("#kdinUserName").text(),'mysql');			
			if (res != undefined && res!="" && res.length>0){
				for(var i=0;i<res.length;i++){
					$("#ipaddr").text(res[i][0]);
					$("#vlanaddr").text(res[i][1]);	
					$("#macaddr").text(res[i][2]);	
					$("#iSimultaneous").text(res[i][3]);				
				}
			}
		}
		...
		function updateAuthorinfo(){
			...
			if(confirm("确定要清除绑定信息吗？")){
				var res = executeNoQuery('dbsql_rad.updateAuthorinfo',8,"&username="+$("#kdinUserName").text());
				if(res=="true"){
					alert("清除成功！");
					queryAuthor();
					return false;
		...
		function initRadUser_jiaofei(){
				...
				$("#editgrid2").jqGrid({
						datatype: 'xml', 
						colNames:['缴费时间','缴费流水','帐号','用户名称','缴费金额','套餐名称','受理人员','费用类型','付款方式','备注'], 
						colModel:colm, 
				       	rowNum:10, //默认分页行数
			...
			<div id="input-Unit" style="margin-bottom:0px;">
				<div id="">
					<div class="title" style="line-height:32px;">
						&nbsp;&nbsp;
						<img src="style/icon/45.gif" />
						绑定信息&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input  class="tsdbutton" style="width: 80px;" type="button" value="解除绑定" onclick="updateAuthorinfo();"/>
					</div>
				</div>
			</div>
			<table id="kdInfo" width="960" border="1" bordercolor="#eeeeee" cellspacing="0" cellpadding="1" style="background-color:#eeeeee;border-collapse:collapse" bgcolor="#eeeeee">
				<tr>
					<td width="100">绑定MAC地址</td><td width="200"><span id="macaddr"></span></td><td width="100">绑定IP</td><td width="200"><span id="ipaddr"></span></td><td width="100">绑定VLAN</td><td width="200"><span id="vlanaddr"></span></td>
				</tr>
				<tr>
					<td width="100">同时在线数</td><td width="200"><span id="iSimultaneous"></span></td><td width="100"></td><td width="200"><span id=""></span></td><td width="100"></td><td width="200"><span id=""></span></td>
				</tr>
			</table>
		</div>
		<div title="缴费流水">
