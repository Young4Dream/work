...
<!-- 导入css文件开始 -->
	...
	function searchHz()
	{
		...
		var params = "&tjtype=" + cjkEncode(tjtype) + "&timestart=" + cjkEncode(timebegin) + "&timeend=" + cjkEncode(timeend) + "&hzyf=" + cjkEncode(hzyf);
  ...
  <body>
	...
	<legend style="font-size:14px">业务收费汇总、明细</legend>
		...
		统计方式：<select id="tj_tjtype" style="display:inline;">
			<option value="byhzyf">按收费月份</option>
			<option value="bytime" selected>按时间范围</option>
		</select>
		...
		<div id="tj_paytype" style="display:block;">
			<br />
			<button class="tsdbutton" onclick="searchHz()">查询汇总表</button>
			<button class="tsdbutton" onclick="searchMx()">查询明细表</button>
		</div>
		<br />
	</fieldset>
