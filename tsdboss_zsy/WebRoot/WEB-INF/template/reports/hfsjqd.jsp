...
<!-- 导入css文件开始 -->
	...
	function getuser(){
		...
		res = fetchMultiArrayValue("dbsql_phone.querytabledate",6,"&cloum=userid,username&tablename=sys_user&key=defarea='"+tsd.encodeURIComponent(area)+"'");
		if(res[0]!=undefined)
		{
			var optHtml = "<option value=\"%\">全部</option>";
			for(var ii=0;ii<res.length;ii++)
	...
	function search()
	{
		...
		var sys_users = $("#sys_users").val();
		var params = "&hzyf=" + cjkEncode(hzyf) + "&tjtype=" + cjkEncode(tjtype) + "&tjway=" + cjkEncode(tjway) + "&area=" + cjkEncode(tjarea);
		params += "&tjstart=" + cjkEncode(tjstart) + "&tjend=" + cjkEncode(tjend) + "&paytype=" + cjkEncode(paytype) + "&userid=" + cjkEncode(userid) + "&areauserid=" + cjkEncode(sys_users);
  ...
  <!-- 
   ...
   -->
   	<fieldset style="margin-left:50px;margin-right:50px;margin-top:100px;font-size:14px;">
   	<legend style="font-size:14px">话费收缴清单</legend>
	<br />
	...
	<div id="tj_way_st" style="display:inline;">
		<select id="stations" name="stations" style="width:120px;" onchange="getuser();"></select>
	</div>
	<div style="display:inline;">
		<select id="sys_users" name="sys_users" style="width:120px;"></select>		
	</div>
