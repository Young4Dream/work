...
<!-- 导入css文件开始 -->
	...
	$(function(){
		setUserRight();
		var res = fetchMultiValue("ReportParam.stations",6,"");
		if(res[0]!=undefined)
		{
			var optHtml = "<option value=\"%\">全部</option>";
			for(var ii=0;ii<res.length;ii++)
			{
				optHtml = optHtml + "<option value=\"" + res[ii] + "\">" + res[ii] + "</option>";
			}
	...
	function search()
	{
		...
		params += "&tjstart=" + cjkEncode(tjstart) + "&tjend=" + cjkEncode(tjend) + "&paytype=" + cjkEncode(paytype) + "&userid=" + cjkEncode(userid);
  ...
  <!-- 
   ...
   -->
	...
	<div id="tj_way" style="display:inline;">
		<input type="radio" name="byarea" value="byself" checked />本人<input type="radio" name="byarea" value="byarea" />本站		
	</div>
	<div id="tj_way_st" style="display:none;">
		<select id="stations" name="stations" style="width:120px;"></select>
	</div>
