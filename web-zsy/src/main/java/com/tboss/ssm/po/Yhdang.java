package com.tboss.ssm.po;

import java.math.BigDecimal;
import java.util.Date;

public class Yhdang {
    private String dh;

    private String hth;

    private String jxh;

    private String yhmc;

    private String bm1;

    private String bm2;

    private String bm3;

    private String bm4;

    private String bmbh;

    private String yhdz;

    private String hwjb;

    private String hwjb0;

    private String hwjb1;

    private Date lastadjustdate;

    private String callsheduleno;

    private String adjustsheduleno;

    private String zlh;

    private String jflb;

    private String mfjb;

    private String mima;

    private String mima2;

    private Date regDate;

    private Date startdate;

    private Date enddate;

    private String tjbz;

    private Date tjdate;

    private String stopbz;

    private String delbz;

    private Date deldate;

    private String dhlx;

    private Short custtype;

    private Short jhjId;

    private String dhgn;

    private String bz1;

    private String bz2;

    private String bz3;

    private String bz4;

    private String bz5;

    private String bz6;

    private String bz7;

    private String bz8;

    private String bz9;

    private String bz10;

    private String mokuaiju;

    private String modual;

    private String port;

    private String typenum;

    private String jbkzlx;

    private String ywarea;

    private BigDecimal dhid;

    private String mobile;

    private Short creditgrade;

    private Integer creditpoint;

    private Long usertype;

    private Long servicearea;

    private String mkj2;

    private String mkj3;

    private String mkj4;

    private Integer mkjacross;

    private Date fjdate;

    private String startdz;

    private String enddz;

    private byte[] photo;

    public String getDh() {
        return dh;
    }

    public void setDh(String dh) {
        this.dh = dh == null ? null : dh.trim();
    }

    public String getHth() {
        return hth;
    }

    public void setHth(String hth) {
        this.hth = hth == null ? null : hth.trim();
    }

    public String getJxh() {
        return jxh;
    }

    public void setJxh(String jxh) {
        this.jxh = jxh == null ? null : jxh.trim();
    }

    public String getYhmc() {
        return yhmc;
    }

    public void setYhmc(String yhmc) {
        this.yhmc = yhmc == null ? null : yhmc.trim();
    }

    public String getBm1() {
        return bm1;
    }

    public void setBm1(String bm1) {
        this.bm1 = bm1 == null ? null : bm1.trim();
    }

    public String getBm2() {
        return bm2;
    }

    public void setBm2(String bm2) {
        this.bm2 = bm2 == null ? null : bm2.trim();
    }

    public String getBm3() {
        return bm3;
    }

    public void setBm3(String bm3) {
        this.bm3 = bm3 == null ? null : bm3.trim();
    }

    public String getBm4() {
        return bm4;
    }

    public void setBm4(String bm4) {
        this.bm4 = bm4 == null ? null : bm4.trim();
    }

    public String getBmbh() {
        return bmbh;
    }

    public void setBmbh(String bmbh) {
        this.bmbh = bmbh == null ? null : bmbh.trim();
    }

    public String getYhdz() {
        return yhdz;
    }

    public void setYhdz(String yhdz) {
        this.yhdz = yhdz == null ? null : yhdz.trim();
    }

    public String getHwjb() {
        return hwjb;
    }

    public void setHwjb(String hwjb) {
        this.hwjb = hwjb == null ? null : hwjb.trim();
    }

    public String getHwjb0() {
        return hwjb0;
    }

    public void setHwjb0(String hwjb0) {
        this.hwjb0 = hwjb0 == null ? null : hwjb0.trim();
    }

    public String getHwjb1() {
        return hwjb1;
    }

    public void setHwjb1(String hwjb1) {
        this.hwjb1 = hwjb1 == null ? null : hwjb1.trim();
    }

    public Date getLastadjustdate() {
        return lastadjustdate;
    }

    public void setLastadjustdate(Date lastadjustdate) {
        this.lastadjustdate = lastadjustdate;
    }

    public String getCallsheduleno() {
        return callsheduleno;
    }

    public void setCallsheduleno(String callsheduleno) {
        this.callsheduleno = callsheduleno == null ? null : callsheduleno.trim();
    }

    public String getAdjustsheduleno() {
        return adjustsheduleno;
    }

    public void setAdjustsheduleno(String adjustsheduleno) {
        this.adjustsheduleno = adjustsheduleno == null ? null : adjustsheduleno.trim();
    }

    public String getZlh() {
        return zlh;
    }

    public void setZlh(String zlh) {
        this.zlh = zlh == null ? null : zlh.trim();
    }

    public String getJflb() {
        return jflb;
    }

    public void setJflb(String jflb) {
        this.jflb = jflb == null ? null : jflb.trim();
    }

    public String getMfjb() {
        return mfjb;
    }

    public void setMfjb(String mfjb) {
        this.mfjb = mfjb == null ? null : mfjb.trim();
    }

    public String getMima() {
        return mima;
    }

    public void setMima(String mima) {
        this.mima = mima == null ? null : mima.trim();
    }

    public String getMima2() {
        return mima2;
    }

    public void setMima2(String mima2) {
        this.mima2 = mima2 == null ? null : mima2.trim();
    }

    public Date getRegDate() {
        return regDate;
    }

    public void setRegDate(Date regDate) {
        this.regDate = regDate;
    }

    public Date getStartdate() {
        return startdate;
    }

    public void setStartdate(Date startdate) {
        this.startdate = startdate;
    }

    public Date getEnddate() {
        return enddate;
    }

    public void setEnddate(Date enddate) {
        this.enddate = enddate;
    }

    public String getTjbz() {
        return tjbz;
    }

    public void setTjbz(String tjbz) {
        this.tjbz = tjbz == null ? null : tjbz.trim();
    }

    public Date getTjdate() {
        return tjdate;
    }

    public void setTjdate(Date tjdate) {
        this.tjdate = tjdate;
    }

    public String getStopbz() {
        return stopbz;
    }

    public void setStopbz(String stopbz) {
        this.stopbz = stopbz == null ? null : stopbz.trim();
    }

    public String getDelbz() {
        return delbz;
    }

    public void setDelbz(String delbz) {
        this.delbz = delbz == null ? null : delbz.trim();
    }

    public Date getDeldate() {
        return deldate;
    }

    public void setDeldate(Date deldate) {
        this.deldate = deldate;
    }

    public String getDhlx() {
        return dhlx;
    }

    public void setDhlx(String dhlx) {
        this.dhlx = dhlx == null ? null : dhlx.trim();
    }

    public Short getCusttype() {
        return custtype;
    }

    public void setCusttype(Short custtype) {
        this.custtype = custtype;
    }

    public Short getJhjId() {
        return jhjId;
    }

    public void setJhjId(Short jhjId) {
        this.jhjId = jhjId;
    }

    public String getDhgn() {
        return dhgn;
    }

    public void setDhgn(String dhgn) {
        this.dhgn = dhgn == null ? null : dhgn.trim();
    }

    public String getBz1() {
        return bz1;
    }

    public void setBz1(String bz1) {
        this.bz1 = bz1 == null ? null : bz1.trim();
    }

    public String getBz2() {
        return bz2;
    }

    public void setBz2(String bz2) {
        this.bz2 = bz2 == null ? null : bz2.trim();
    }

    public String getBz3() {
        return bz3;
    }

    public void setBz3(String bz3) {
        this.bz3 = bz3 == null ? null : bz3.trim();
    }

    public String getBz4() {
        return bz4;
    }

    public void setBz4(String bz4) {
        this.bz4 = bz4 == null ? null : bz4.trim();
    }

    public String getBz5() {
        return bz5;
    }

    public void setBz5(String bz5) {
        this.bz5 = bz5 == null ? null : bz5.trim();
    }

    public String getBz6() {
        return bz6;
    }

    public void setBz6(String bz6) {
        this.bz6 = bz6 == null ? null : bz6.trim();
    }

    public String getBz7() {
        return bz7;
    }

    public void setBz7(String bz7) {
        this.bz7 = bz7 == null ? null : bz7.trim();
    }

    public String getBz8() {
        return bz8;
    }

    public void setBz8(String bz8) {
        this.bz8 = bz8 == null ? null : bz8.trim();
    }

    public String getBz9() {
        return bz9;
    }

    public void setBz9(String bz9) {
        this.bz9 = bz9 == null ? null : bz9.trim();
    }

    public String getBz10() {
        return bz10;
    }

    public void setBz10(String bz10) {
        this.bz10 = bz10 == null ? null : bz10.trim();
    }

    public String getMokuaiju() {
        return mokuaiju;
    }

    public void setMokuaiju(String mokuaiju) {
        this.mokuaiju = mokuaiju == null ? null : mokuaiju.trim();
    }

    public String getModual() {
        return modual;
    }

    public void setModual(String modual) {
        this.modual = modual == null ? null : modual.trim();
    }

    public String getPort() {
        return port;
    }

    public void setPort(String port) {
        this.port = port == null ? null : port.trim();
    }

    public String getTypenum() {
        return typenum;
    }

    public void setTypenum(String typenum) {
        this.typenum = typenum == null ? null : typenum.trim();
    }

    public String getJbkzlx() {
        return jbkzlx;
    }

    public void setJbkzlx(String jbkzlx) {
        this.jbkzlx = jbkzlx == null ? null : jbkzlx.trim();
    }

    public String getYwarea() {
        return ywarea;
    }

    public void setYwarea(String ywarea) {
        this.ywarea = ywarea == null ? null : ywarea.trim();
    }

    public BigDecimal getDhid() {
        return dhid;
    }

    public void setDhid(BigDecimal dhid) {
        this.dhid = dhid;
    }

    public String getMobile() {
        return mobile;
    }

    public void setMobile(String mobile) {
        this.mobile = mobile == null ? null : mobile.trim();
    }

    public Short getCreditgrade() {
        return creditgrade;
    }

    public void setCreditgrade(Short creditgrade) {
        this.creditgrade = creditgrade;
    }

    public Integer getCreditpoint() {
        return creditpoint;
    }

    public void setCreditpoint(Integer creditpoint) {
        this.creditpoint = creditpoint;
    }

    public Long getUsertype() {
        return usertype;
    }

    public void setUsertype(Long usertype) {
        this.usertype = usertype;
    }

    public Long getServicearea() {
        return servicearea;
    }

    public void setServicearea(Long servicearea) {
        this.servicearea = servicearea;
    }

    public String getMkj2() {
        return mkj2;
    }

    public void setMkj2(String mkj2) {
        this.mkj2 = mkj2 == null ? null : mkj2.trim();
    }

    public String getMkj3() {
        return mkj3;
    }

    public void setMkj3(String mkj3) {
        this.mkj3 = mkj3 == null ? null : mkj3.trim();
    }

    public String getMkj4() {
        return mkj4;
    }

    public void setMkj4(String mkj4) {
        this.mkj4 = mkj4 == null ? null : mkj4.trim();
    }

    public Integer getMkjacross() {
        return mkjacross;
    }

    public void setMkjacross(Integer mkjacross) {
        this.mkjacross = mkjacross;
    }

    public Date getFjdate() {
        return fjdate;
    }

    public void setFjdate(Date fjdate) {
        this.fjdate = fjdate;
    }

    public String getStartdz() {
        return startdz;
    }

    public void setStartdz(String startdz) {
        this.startdz = startdz == null ? null : startdz.trim();
    }

    public String getEnddz() {
        return enddz;
    }

    public void setEnddz(String enddz) {
        this.enddz = enddz == null ? null : enddz.trim();
    }

    public byte[] getPhoto() {
        return photo;
    }

    public void setPhoto(byte[] photo) {
        this.photo = photo;
    }
}